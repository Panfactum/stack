  ...
  return {
    ...
    task: async (_, parentTask) => {
      ...
      if (eksHCLPaths.length === 0) {
        parentTask.skip(
          applyColors(
            ...
            {
              highlights: [{ phrase: "No clusters found", style: "subtle" }],
            }
          )
        );
        return;
      ...
      eksHCLPaths.forEach((eksHCLPath) => {
        ...
        subtasks.add({
          title: applyColors(`Retrieve cluster info ${clusterId}`, {
            highlights: [{ phrase: clusterId, style: "subtle" }],
          }),
          task: async (ctx, task) => {
      ...
      subtasks.add({
        title: applyColors(
          ...
          { highlights: [{ style: "subtle", phrase: clustersYAMLPath }] }
        ),
        task: async (ctx) => {
      ...
      ///////////////////////////////////////////////////////
      subtasks.add({
        title: applyColors(`Updating Kubeconfig ${kubeConfigPath}`, {
          highlights: [{ style: "subtle", phrase: kubeConfigPath }],
        }),
        task: async () => {
