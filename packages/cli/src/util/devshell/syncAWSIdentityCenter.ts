...
export async function syncAWSIdentityCenter(inputs: {
    ...
    const { context, subtaskConfig } = inputs;
    ...
    const taskList: Array<ListrTask<TaskContext>> = [
        {
            title: applyColors(
                `Getting outputs from AWS Identity Center Module ${MODULES.IAM_IDENTIY_CENTER_PERMISSIONS}`,
                 {highlights: [{phrase: MODULES.IAM_IDENTIY_CENTER_PERMISSIONS, style: "subtle"}]}
            ),
            task: async (ctx, task) => {
                ...
                if (await fileExists(configFilePath)) {
                    try {
                        ...
                        if(!originalAWSConfig["sso-session sso"]){
                            config["sso-session sso"] = await getSSOConfig(context, ssoRegion, task)
                        }
                    } catch (e) {
                    ...
                    config = {
                        ...configUpdate,
                        ["sso-session sso"]: await getSSOConfig(context, ssoRegion, task)
                    }
                }
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                ...
                if(subtaskConfig && subtaskConfig.onComplete){
                    subtaskConfig.onComplete()
                }
    ...
    if(subtaskConfig){
        return subtaskConfig.parentTask.newListr(taskList, {
            ctx: {
                profiles: {}
            },
        ...
        const tasks = new Listr<TaskContext>(taskList, {ctx: {profiles: {}}})
        try {
            await tasks.run()
        } catch(e){
            throw new CLIError("Failed to synchronize IAM Identity Center settings with the DevShell's AWS configuration", e)
        }
        return null
    }
}

async function getSSOConfig(context: PanfactumContext, ssoRegion: string, task: ListrTaskWrapper<TaskContext, typeof DefaultRenderer, typeof SimpleRenderer>){
