...
interface ExecInputs {
    ...
    stdin?: globalThis.Request | globalThis.ReadableStream | globalThis.File | globalThis.Blob | number | "inherit" | null
}
...
export async function execute(inputs: ExecInputs): Promise<ExecReturn> {
    ...
    for (let i = 0; i < retries + 1; i++) {
        ...
        if(onStdOutNewline){
            ...
            const stdoutProcessor = createTextOutputProcessor(stdoutReader, onStdOutNewline, i)
        ...
        if(onStdErrNewline){
            ...
            const stderrProcessor = createTextOutputProcessor(stderrReader, onStdErrNewline, i)
        ...
        logsBuffer += output + "\n"
        const stdout =  (await new globalThis.Response(stdoutForCapture).text()).trim()
        const stderr =  (await new globalThis.Response(stderrForCapture).text()).trim()
...
const createTextOutputProcessor = (reader: ReadableStreamDefaultReader , processLine: (line: string, runNum: number) => void, runNum: number) => {
    let buffer = ''; // Buffer to store incomplete lines
    const decoder = new globalThis.TextDecoder("utf-8");

    const processor = async (event: {done: true} | {done: false, value: Uint8Array}): Promise<void> => {
      if(!event.done) {
