...
const SETUP_STEPS: Array<{
  ...
  },
  {
    ...
    extraInfo: "⏰ NOTE: The cluster may take up to 20 minutes to be created after you answer a couple questions"
  },
  {
...
export class InstallClusterCommand extends PanfactumCommand {
  ...
  async execute() {
    ...
    this.context.logger.log(`Panfactum Config: ${JSON.stringify(config, null, 2)}`, {level: "debug"})

    const {environment, region, sla_target: slaTarget} = config;
