...
export const VPC_NAME = z.string()
  .min(3)
  .max(100)
  .regex(/^[a-zA-Z0-9_-]+$/, "Must only contain the letters a-z (case-insensitive), numbers 0-9, hyphens (-), and underscores (_)")
export const VPC_DESCRIPTION = z.string()
  .min(3)
  .max(255)
  .regex(/^[a-zA-Z0-9_\- .:/=+@]+$/, "Must only contain spaces, the letters a-z (case-insensitive), numbers 0-9, and the following characters: _.:/=+-@")
export const DOCKERHUB_USERNAME = z.string()
  .min(3)
  .max(63)
  .regex(/^[a-zA-Z0-9_-]+$/, "Must only contain the letters a-z (case-insensitive), numbers 0-9, hyphens (-), and underscores (_)")
  .regex(/^(?!aws:).*$/i, "Cannot start with 'AWS:' (case insensitive)")
export const GITHUB_USERNAME = z.string()
  .min(3)
  .max(63)
  .regex(/^[a-zA-Z0-9_-]+$/, "Must only contain the letters a-z (case-insensitive), numbers 0-9, hyphens (-), and underscores (_)")
  export const CLUSTER_NAME = z.string()
  .min(3)
  .max(63)
  .regex(/^[a-z0-9-]+$/, "Must only contain the letters a-z (lowercase), numbers 0-9, and hyphens (-)")
  .regex(/^[a-z0-9].*[a-z0-9]*$/, "Must start and end with a letter a-z (lowercase) or number 0-9")
export const CLUSTER_DESCRIPTION = z.string()
.min(3)
.max(255)
.regex(/^[a-zA-Z0-9_\- .:/=+@]+$/, "Must only contain spaces, the letters a-z (case-insensitive), numbers 0-9, and the following characters: _.:/=+-@")
.regex(/^(?!aws:).*$/i, "Cannot start with 'AWS:' (case insensitive)")
...
export class Checkpointer {
  ...
  async save(){
    try {
      await Bun.write(this.checkpointFile, JSON.stringify(this.checkpointData, null, 2));
    } catch (e){
      throw new CLIError(`Failed to save checkpoint data to ${this.checkpointFile}`, e)
    }
  }
  ...
  private async rawLoad(): Promise<unknown> {
    try {
      ...
      throw new CLIError(`Failed to load checkpoint data from ${this.checkpointFile}`, e)
    }
  }

  async getSavedInput(key: keyof z.infer<typeof SAVED_INPUTS>): Promise<z.infer<typeof SAVED_INPUTS>[typeof key]> {
  ...
  async updateSavedInput(key: keyof z.infer<typeof SAVED_INPUTS>, value: NonNullable<z.infer<typeof SAVED_INPUTS>[typeof key]>) {
    if(this.checkpointData === undefined){
