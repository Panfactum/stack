...
export async function buildKubeConfig({
  awsProfile,
  context,
}: {
  awsProfile: string;
  ...
  // I took this out to build the CLI to test
  ...
  if (awsClusterHCLs.length > 0) {
    ...
    await Promise.all(
      Array.from(glob.scanSync(environments_dir)).map(
        async (clusterTerragruntHcl) => {
          const directory = path.dirname(clusterTerragruntHcl);
          context.logger.log(
            `Adding cluster at ${getLastPathSegments(directory, 3)}...`,
            {
              indentLevel: 1,
            }
          );
          const moduleOutput = await terragruntOutput({
            ...
            context: {
              ...context,
              // stdout: createNullWriter() // I took this out to build the CLI to testz
            },
    ...
    await Bun.write(Bun.file(clusterInfoFilePath), clusterInfo.join("\n"));
