...
import SidebarContent from "@/components/layouts/primary/sidebar/SidebarContent";
import type { DocsMetadata } from "@/components/layouts/primary/types";
import { getBreadcrumbs } from "@/components/layouts/primary/util/getBreadcrumbs";
...
const InternalDocsSidebar: Component<DocSidebarProps> = (props) => {
  ...
  const sectionBasePath = createMemo(
    () =>
      `${isVersioned() ? `/${pathComponents().version}` : ""}${activeSection().path}`,
  );
