...
const FAQItem: ParentComponent<FAQItemProps> = (props) => {
  return (
    <Accordion.Item
      value={props.id}
      class={`
        border-b-2 border-gray-light-mode-500 py-4
        last:border-0
        dark:border-gray-dark-mode-700
      `}
    >
      <Accordion.Header class="[&[data-expanded]_svg]:rotate-45">
        <Accordion.Trigger
          class={`
            ...
            text-left text-display-sm font-semibold
          `}
        >
          {props.title}
      ...
      <Accordion.Content
        class={`
          faq-content flex flex-col gap-4 overflow-hidden text-display-xs
          text-secondary
          [&_a]:underline [&_a]:hover:cursor-pointer
        `}
      >
        {props.children}
      </Accordion.Content>
