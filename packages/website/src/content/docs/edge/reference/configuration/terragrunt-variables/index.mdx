...
Your environments folder at `environments_dir` **must** have three levels of scoping for the [configuration-as-code](/edge/concepts/iac#configuration-as-code):
...
*To apply modules that use the Authentik provider, you will also need to have `AUTHENTIK_TOKEN` set in [your .env file.](/edge/reference/configuration/dotenv)*
