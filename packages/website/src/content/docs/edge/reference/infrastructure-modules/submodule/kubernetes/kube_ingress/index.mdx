...
1. For all domains in `domains`, ensure that a DNS record is configured to point to the domain to this cluster's NGINX
   ingress controller (via [kube\_external\_dns](/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_dns))
   and provide the ingress controller a TLS certificate for the domains
   (via [kube\_cert\_manager](/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager)).
...
[kube\_cert\_issuers](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cert_issuers) provides a global
...
for performance and security improvements, see the [kube\_aws\_cdn](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_aws_cdn) module.
...
settings are described in more detail [here](/edge/reference/infrastructure-modules/submodule/aws/aws_cdn).
