...
| `sla_target`                  | `number` (`1`, `2`, or `3`) | `3`     | The [uptime SLA level](/docs/edge/guides/deploying-workloads/high-availability) that will be targeted in the module deployment. Trades off increased uptime for higher runtime costs.                                                                                                 |
| `panfactum_scheduler_enabled` | `bool`                      | `true`  | Whether to use the bin-packing scheduler provided by [kube\_scheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_scheduler). Using this will reduce cluster costs by approximately 25-33% with no additional impact, but requires the scheduler be deployed. |
