...
* Check out the source code indicated by `repo` using our [standard process](/edge/guides/cicd/checking-out-code).
...
`git_username` and `git_password` can be used for authenticating with a private `repo`. See our [documentation](/edge/guides/cicd/checking-out-code)
