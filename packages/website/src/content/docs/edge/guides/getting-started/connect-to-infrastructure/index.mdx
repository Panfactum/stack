...
[^1]:
    ...
    [bootstrapping guide.](/docs/edge/guides/bootstrapping/identity-provider)

If your organization's administrators have not already
[provisioned your account](/docs/edge/guides/user-management/provisioning-new-user), ask them now. You will need to
follow [this guide](/docs/edge/guides/user-management/setting-up-new-user) to set up your user for the first time.
...
2. Update the `config.user.yaml` to include the clusters you want to access and what AWS profile you want to use for
   that access. See the [reference docs](/docs/edge/reference/configuration/kubernetes#config.user.yaml) for the
   specific syntax.
