...
* Set the `repo_url` [repository variable](/docs/edge/reference/configuration/repo-variables). This will likely have already been set up when your system was first deployed.

* Set `GIT_USERNAME` and `GIT_PASSWORD` in your [personal `.env` file](/docs/edge/reference/configuration/dotenv). The values will be combined with `repo_url` to clone your IaC over HTTPS as follows:
...
<MarkdownAlert severity="info">
  ...
  by following [these instructions.](/docs/edge/modules/wf_tf_deploy/overview#authenticating-with-private-repositories)
</MarkdownAlert>
