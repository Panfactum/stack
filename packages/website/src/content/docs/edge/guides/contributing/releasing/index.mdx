...
- Copy all the unreleased documentation (under `packages/website/src/content/docs/main`) to the edge documentation
  (under `packages/website/src/content/docs/edge`), rewriting internal links and version placeholders.

- Copy the unreleased changelog (`changelog/main/log.yaml`) into a dated edge entry (e.g.,
  `changelog/edge/26-04-05/log.yaml`) and move any `upgrade.mdx` alongside it.
...
Every stable release occurs on a stable release "channel" which lives on a dedicated git branch. The base of each stable
branch is an edge release.
...
- Copy all the edge documentation (under `packages/website/src/content/docs/edge`) to the new stable release channel
  documentation (under `packages/website/src/content/docs/<stable_release_slug>`).
