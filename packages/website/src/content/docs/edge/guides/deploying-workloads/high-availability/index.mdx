...
Panfactum modules will adjust their default behavior based on the `sla_target` [Terragrunt variable.](/edge/reference/configuration/terragrunt-variables#metadata) [^1] [^80]
...
* Set `sla_target` to `1`.
* [Set up liveness probes](/edge/guides/deploying-workloads/basics#liveness-and-readiness-probes) for your workloads to ensure that they will be restarted if they enter a failure state.
