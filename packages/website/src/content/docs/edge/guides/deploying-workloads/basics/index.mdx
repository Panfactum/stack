...
2. [Create a first-party IaC module](/docs/edge/guides/iac/first-party-modules#writing-modules)
   ...
   | :----------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
   | [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) | [kube\_namespace](/docs/edge/modules/kube_namespace/overview)        | A logical container for Kubernetes resources. Most resources must be assigned a namespace.                              |
   | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)        | [kube\_deployment](/docs/edge/modules/kube_deployment/overview)      | Orchestrates stateless workloads such as webservers.                                                                    |
   | [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)      | [kube\_stateful\_set](/docs/edge/modules/kube_stateful_set/overview) | Orchestrates for stateful workloads such as databases.                                                                  |
   | [CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/)            | [kube\_cron\_job](/docs/edge/modules/kube_cron_job/overview)         | Periodically runs one-shot processes on a schedule such as system maintenance scripts.                                  |
   | [DaemonSet](https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/)          | [kube\_daemon\_set](/docs/edge/modules/kube_daemon_set/overview)     | Runs a pod on every tolerated node in the cluster.                                                                      |
   | [Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)                      | [kube\_job](/docs/edge/modules/kube_job/overview)                    | Runs a one-off process as a part of a module deployment. Useful for pre-/post-deployment scripts such as DB migrations. |
   | [Argo Workflow](https://argoproj.github.io/workflows/)                                     | [wf\_spec](/docs/edge/modules/wf_spec/overview)                       | An advanced pattern available through our [Workflow Engine Addon](/docs/edge/guides/addons/workflow-engine/installing). |
...
[^2]: In fact, our submodules utilize a shared interface for defining pods: [kube\_pod](/docs/edge/modules/kube_pod/overview).
...
<MarkdownAlert severity="info">
  ...
  We provide a submodule for easily referencing the various registry addresses: [aws\_ecr\_pull\_through\_cache\_addresses](/docs/edge/modules/aws_ecr_pull_through_cache_addresses/overview).
</MarkdownAlert>
...
* Every container has a ready-only root filesystem. If you need to write to disk, we recommend using temporary directories (see below)
  or a [kube\_stateful\_set](/docs/edge/modules/kube_stateful_set/overview). However, you
...
All of our controller submodules such as [kube\_deployment](/docs/edge/modules/kube_deployment/overview)
...
[^8]: Unless you are using [kube\_stateful\_set](/docs/edge/modules/kube_stateful_set/overview)
...
can utilize the `common_pod_labels` and `common_pod_annotations` inputs of the [kube\_policies](/docs/edge/modules/kube_policies/overview)
...
define precedence for pod preemption. Our [kube\_policies](/docs/edge/modules/kube_policies/overview)
...
(e.g., [kube\_deployment](/docs/edge/modules/kube_deployment/overview)).
