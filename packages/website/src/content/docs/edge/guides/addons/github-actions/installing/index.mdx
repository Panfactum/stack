...
guides for our recommended architecture [here](/docs/edge/guides/cicd/getting-started).

Additionally, GitHub Actions have many shortcomings compared to a more full-featured, Kubernetes-native workflow engine
such as Argo Workflows. See our detailed analysis and list of limitations [here](/docs/edge/concepts/background/cicd/gha).
...
We provide a module to deploy this controller: [kube_gha](/docs/edge/modules/kube_gha/overview).
...
To create runners, you will use our [kube_gha_runners](/docs/edge/modules/kube_gha_runners/overview) module.
...
2. Add a `terragrunt.hcl` and sops-encrypted `secrets.yaml` to that directory that looks like this:
   ...
   1. Follow
      ...
      [encrypted with sops.](/docs/edge/guides/iac/deploying-modules#using-secrets)
   ...
   2. The `runners` input is a key-value map that defines the different runner groups that your workflow jobs can
      ...
      [module documentation](/docs/edge/modules/kube_gha_runners/overview) for information on the various configuration
      values.
