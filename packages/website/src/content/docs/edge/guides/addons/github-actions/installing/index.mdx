...
We provide a module to deploy this controller: [kube\_gha](/docs/edge/modules/kube_gha/overview).
...
To create runners, you will use our [kube\_gha\_runners](/docs/edge/modules/kube_gha_runners/overview) module.
...
2. Add a `terragrunt.hcl` and sops-encrypted `secrets.yaml` to that directory that looks like this:
   ...
   2. The `runners` input is a key-value map that defines the different runner groups that your workflow jobs can target. The keys
      are the names of the groups and the values are their configuration. See the [module documentation](/docs/edge/modules/kube_gha_runners/overview)
