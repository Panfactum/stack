...
[^2]: Your administrator set this when [first installing Argo](/edge/guides/addons/workflow-engine/installing).
...
<MarkdownAlert severity="warning">
  The Panfactum [kube\_argo](/edge/reference/infrastructure-modules/direct/kubernetes/kube_argo) does not create an Ingress for the Argo Workflow Server, so the HTTP API is only available
...
[kube\_sa\_auth\_workflow](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_sa_auth_workflow)
...
Using our [Event Bus Addon](/edge/guides/addons/event-bus/installing), you can create an Argo [Sensor](https://argoproj.github.io/argo-events/concepts/sensor/)
...
For a real world example, see our guide on rolling deployments in your [CI / CD pipelines](/edge/guides/cicd/rolling-deployments).
