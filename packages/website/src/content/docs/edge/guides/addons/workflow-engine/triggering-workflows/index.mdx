...
<MarkdownAlert severity="warning">
  The Panfactum [kube\_argo](/docs/edge/modules/kube_argo/overview) does not create an Ingress for the Argo Workflow
  Server, so the HTTP API is only available from inside the cluster. Since authentication must be done via
  ServiceAccount token, it does not make sense to expose the server publicly. If you need to create a Workflow from an
  external event or trigger, see the next section regarding Argo Events.
</MarkdownAlert>
...
[kube_sa_auth_workflow](/docs/edge/modules/kube_sa_auth_workflow/overview) submodule to assign the necessary permissions
...
Using our [Event Bus Addon](/docs/edge/guides/addons/event-bus/installing), you can create an Argo
...
[CI / CD pipelines](/docs/edge/guides/cicd/rolling-deployments).
