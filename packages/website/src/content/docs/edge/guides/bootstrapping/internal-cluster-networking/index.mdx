...
<p class="subtitle">Install the basic Kubernetes cluster networking primitives via the [kube\_core\_dns](/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns)
and [kube\_cilium](/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium) modules.</p>
...
we recommend reviewing the [concept documentation](/edge/concepts/networking/cluster-networking) for more
information.
...
for deploying Cilium: [kube\_cilium](/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium).
...
We provide a module to deploy CoreDNS called [kube\_core\_dns](/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns).
...
5. If the deployment succeeds, you should see a `core-dns` deployment with either 1/2 or 2/2 pods running:
   ...
   you complete the [autoscaling section](/edge/guides/bootstrapping/autoscaling) of the bootstrapping guide.
