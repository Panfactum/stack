...
infrastructure modules](/docs/edge/reference/infrastructure-modules/overview) that comprise the Panfactum framework. [^1] We
...
By default, this is the `main` branch. If you need to change it, make sure it is set as the `repo_primary_branch` (per the [docs](/docs/edge/reference/configuration/repo-variables)).
...
[^66]: You can override this folder by setting `environments_dir` (per the [docs](/docs/edge/reference/configuration/repo-variables)).
...
it must be set up to follow [these conventions](/docs/edge/reference/configuration/terragrunt-variables).
