...
We provide both a module to deploy Authentik ([kube\_authentik](/docs/edge/modules/kube_authentik/overview)) and
many modules to connect Authentik to the various systems in the Panfactum stack (e.g., [authentik\_aws\_sso](/docs/edge/modules/authentik_aws_sso/overview)).
...
We provide a module to do this: [aws\_ses\_domain](/docs/edge/modules/aws_ses_domain/overview).
...
7. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record
   ...
   to set this up. You can use the [aws\_dns\_records](/docs/edge/modules/aws_dns_records/overview) module to
   deploy the record.
...
branding: [authentik\_core\_resources](/docs/edge/modules/authentik_core_resources/overview).
