...
We provide both a module to deploy Authentik ([kube\_authentik](/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)) and
many modules to connect Authentik to the various systems in the Panfactum stack (e.g., [authentik\_aws\_sso](/edge/reference/infrastructure-modules/direct/authentik/authentik_aws_sso)).
...
We provide a module to do this: [aws\_ses\_domain](/edge/reference/infrastructure-modules/direct/aws/aws_ses_domain).
...
7. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record
   ...
   to set this up. You can use the [aws\_dns\_records](/edge/reference/infrastructure-modules/direct/aws/aws_dns_records) module to
   deploy the record.
...
branding: [authentik\_core\_resources](/edge/reference/infrastructure-modules/direct/authentik/authentik_core_resources).
...
RBAC reference](/edge/reference/rbac) for more information.
