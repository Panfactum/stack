...
within this guide, but we do in our [concept documentation.](/edge/concepts/networking/cluster-networking)
...
We provide a module to deploy it: [kube\_external\_dns](/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
...
We provide a module to deploy it: [kube\_aws\_lb\_controller](/edge/reference/infrastructure-modules/direct/kubernetes/kube_aws_lb_controller)
...
We provide a module to deploy it: [kube\_ingress\_nginx](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress_nginx).
...
1. Return the `kube_vault` module you deployed in the [Vault guide section](/edge/guides/bootstrapping/vault).
...
7. Moreover, notice that NGINX properly secures the site in a standard way by setting security headers for the browser. You
   ...
   This is accomplished by our [kube\_ingress](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress)
...
We provide a bastion deployment module: [kube\_bastion](/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion).
...
4. Update the values to the correct values for your setup. See the [reference docs](/edge/reference/configuration/ssh) for
   more information.
