...
We provide a module to deploy this controller: [kube\_reloader](/edge/reference/infrastructure-modules/direct/kubernetes/kube_reloader).
...
While the pull-through image cache deployed in [earlier in this guide](/edge/guides/bootstrapping/kubernetes-cluster#configure-pull-through-cache)
...
Fortunately, we can provide a solution to this problem, the [kube\_node\_image\_cache\_controller](/edge/reference/infrastructure-modules/direct/kubernetes/kube_node_image_cache_controller).
...
[^22]: While many of our modules automatically specify images for the cache, you can specify additional images
    via the  [kube\_node\_image\_cache](/edge/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) submodule. This
    is a *submodule* so you can use it many times inside [your own first-party infrastructure modules.](/edge/guides/iac/first-party-modules)
...
We provide a module to deploy it: [kube\_pvc\_autoresizer](/edge/reference/infrastructure-modules/direct/kubernetes/kube_pvc_autoresizer).
...
We provide a module for the Descheduler: [kube\_descheduler](/edge/reference/infrastructure-modules/direct/kubernetes/kube_descheduler).
...
via the [kube\_aws\_ebs\_csi](/edge/reference/infrastructure-modules/direct/kubernetes/kube_aws_ebs_csi) module we installed in
[a prior section](/edge/guides/bootstrapping/storage-interfaces).
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_snapshotter).
...
We provide a module to deploy this functionality: [kube\_velero](/edge/reference/infrastructure-modules/direct/kubernetes/kube_velero).
...
<BootstrappingGuideNav backHref={"/docs/edge/guides/bootstrapping/inbound-networking"} forwardHref={"/docs/edge/guides/bootstrapping/databases"} stepNumber={17} />
