...
[concept docs on network cryptography](/docs/edge/concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube_cert_manager](/docs/edge/modules/kube_cert_manager/overview).
...
[kube_cert_issuers](/docs/edge/modules/kube_cert_issuers/overview).
...
3. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/docs/edge/guides/bootstrapping/dns), we configured DNS in the
      [aws_registered_domains](/docs/edge/modules/aws_registered_domains/overview) and
      [aws_delegated_zones](/docs/edge/modules/aws_delegated_zones/overview) modules under `production/global`. If you
