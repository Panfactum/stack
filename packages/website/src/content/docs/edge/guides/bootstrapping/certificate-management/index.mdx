...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/docs/edge/modules/kube_cert_manager/overview).
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/docs/edge/modules/kube_cert_issuers/overview).
...
3. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/docs/edge/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/docs/edge/modules/aws_registered_domains/overview)
      and [aws\_delegated\_zones](/docs/edge/modules/aws_delegated_zones/overview) modules
