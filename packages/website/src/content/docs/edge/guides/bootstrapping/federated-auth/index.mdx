...
<MarkdownAlert severity="info">
  ...
  * [aws\_eks](/edge/reference/infrastructure-modules/direct/aws/aws_eks) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encrypt\_key](/edge/reference/infrastructure-modules/submodule/aws/aws_kms_encrypt_key)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault) to allow SSO authentication
...
[authentik\_aws\_sso](/edge/reference/infrastructure-modules/direct/authentik/authentik_aws_sso).
...
to automatically set this up: [aws\_iam\_identity\_center\_permissions](/edge/reference/infrastructure-modules/direct/aws/aws_iam_identity_center_permissions).
...
3. For the `acccount_access_configuration`, we recommend setting this up according to our [RBAC recommendations](/edge/reference/rbac),
   ...
   3. The remaining values determine which Authentik groups get assigned which roles in the specified account. For descriptions
      of the roles, see this [reference document.](/edge/reference/rbac)
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](/edge/reference/infrastructure-modules/submodule/aws/aws_kms_encrypt_key) module
   * Vault clusters deployed with [kube\_vaults](/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault)
...
5. Set up `config.yaml` according to [these reference docs](/edge/reference/configuration/aws). Here is an example file:
...
<MarkdownAlert severity="info">
  In general, your [terragrunt variables](/edge/reference/configuration/terragrunt-variables)
  ...
  access. See [this guide](/edge/guides/iac/deploying-modules#local-deployments) for more information.
</MarkdownAlert>
...
[authentik\_vault\_sso](/edge/reference/infrastructure-modules/direct/authentik/authentik_vault_sso).
...
We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](/edge/reference/infrastructure-modules/direct/vault/vault_auth_oidc).
...
1. Remove your hardcoded `VAULT_ADDR` and `VAULT_TOKEN` token from your `.env` file. Make sure that you added the
   `vault_addr` in you `region.yaml` as covered in the [inbound networking section.](/edge/guides/bootstrapping/inbound-networking#deploy-the-vault-ingress)
