...
Before you being, please ensure you have reviewed the guide for [pinning the Panfactum version](/edge/guides/versioning/pinning).
...
2. Ensure that the terragrunt `version` for that environment is set to `local` in order
   to deploy the local changes from step 1 (review [these docs](/edge/guides/iac/first-party-modules)). [^2]
