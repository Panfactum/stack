...
While we detail how to manage the BuildKit installation in the [kube\_buildkit](/docs/edge/modules/kube_buildkit/overview)
...
8. Images are pulled from ECR by kubelets to run workloads on Kubernetes nodes. A *single* ECR is used to source
   ...
   the [aws\_ecr\_repos](/docs/edge/modules/aws_ecr_repos/overview) module.
