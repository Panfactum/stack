...
The Panfactum Stack's Cilium deployment ([kube\_cilium](/docs/edge/modules/kube_cilium/overview)) has a few enhancements over the default Cilium install:
...
You can see our implementation in the [kube\_linkerd](/docs/edge/modules/kube_linkerd/overview) terraform module.
...
In the Panfactum Stack, we use the Ingress-NGINX Controller ([kube\_ingress\_nginx](/docs/edge/modules/kube_ingress_nginx/overview))
...
We provide a submodule for defining Ingress manifests for our Ingress-NGINX controller: [kube\_ingress](/docs/edge/modules/kube_ingress/overview).
...
We provide a module for deploying CoreDNS to Panfactum Stack clusters, [kube\_core\_dns](/docs/edge/modules/kube_core_dns/overview),
which is set up in [the bootstrapping guide.](/docs/edge/guides/bootstrapping/internal-cluster-networking#deploy-coredns) [^20]
...
In the Panfactum Stack, we provide the [kube\_external\_dns](/docs/edge/modules/kube_external_dns/overview)
...
[kube\_bastion](/docs/edge/modules/kube_bastion/overview).
...
Stack via [kube\_cert\_manager](/docs/edge/modules/kube_cert_manager/overview).
...
[kube\_ingress](/docs/edge/modules/kube_ingress/overview) submodule.
