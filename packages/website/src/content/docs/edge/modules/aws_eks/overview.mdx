...
* A set of "controller" [node groups](https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html) with a static size for running cluster-critical controllers. Nodes use the [Bottlerocket](https://bottlerocket.dev/) distribution.
  Autoscaled nodes are deployed via our [kube\_karpenter](/docs/edge/modules/kube_karpenter/overview) module.
...
by the `vpc_id` input as these are the subnets created by the [aws\_vpc](/docs/edge/modules/aws_vpc/overview) module.
...
The SSO roles are installed into each account via [aws\_iam\_identity\_center\_permissions](/docs/edge/modules/aws_iam_identity_center_permissions/overview)
...
in the [kube\_policies](/docs/edge/modules/kube_policies/overview). AWS doesn't install
