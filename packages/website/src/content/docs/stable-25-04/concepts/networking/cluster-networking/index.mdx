...
The Panfactum Stack's Cilium deployment ([kube\_cilium](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_cilium)) has a few enhancements over the default Cilium install:
...
You can see our implementation in the [kube\_linkerd](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_linkerd) terraform module.
...
In the Panfactum Stack, we use the Ingress-NGINX Controller ([kube\_ingress\_nginx](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx))
...
We provide a submodule for defining Ingress manifests for our Ingress-NGINX controller: [kube\_ingress](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_ingress).
...
We provide a module for deploying CoreDNS to Panfactum Stack clusters, [kube\_core\_dns](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_core_dns),
...
In the Panfactum Stack, we provide the [kube\_external\_dns](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
module which will provision DNS records in [AWS Route 53.](https://aws.amazon.com/route53/) The specific instructions
for configuring this are covered in [the bootstrapping guide.](/stable-25-04/guides/bootstrapping/inbound-networking#deploy-externaldns) [^21]
...
[kube\_bastion](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_bastion).
...
For more information on SSH and why we chose this architecture over a VPN-based system, see [these concept docs.](/stable-25-04/concepts/networking/cryptography#vpns)
...
TLS and the associated [X.509 certificate infrastructure](/stable-25-04/concepts/networking/cryptography#x.509-certificates)
...
Stack via [kube\_cert\_manager](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
[kube\_ingress](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) submodule.

<MarkdownAlert severity="warning">
  This certificate provisioning process will only work if you have set up DNS in accordance with our
  [bootstrapping guide.](/stable-25-04/guides/bootstrapping/dns)
</MarkdownAlert>
