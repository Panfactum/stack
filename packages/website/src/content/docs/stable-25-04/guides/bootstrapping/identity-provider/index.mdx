...
A core security tenant is providing a single root directory for managing your organization's users and permissions. This
greatly simplifies identity and access management (IAM), improves the user experience, and reduces the probability of
access control mistakes.

Identity and access management is a broad topic which we will not cover in-depth in this guide. However, it is critical
to understand a few key terms:

- **Authentication (Authn)**: The process of _identifying_ a user (i.e., "logging in")

- **Authorization (Authz)**: The process of controlling what a user has access to

- **Identity Provider (IdP)**: A system that serves as the source of truth for information about users and group
  assignments; can provide this information to other systems in a secure manner

- **Service Provider (SP)**: Any system that your users utilize directly (e.g., AWS, Kubernetes, Vault); will often
  connect to an IdP for information about the users

- **Federated Auth**: The use of protocols such as
  [SAML](http://docs.oasis-open.org/security/saml/Post2.0/sstc-saml-tech-overview-2.0.html),
  [OAuth2](https://oauth.net/2/) / [OIDC](https://openid.net/developers/how-connect-works/), or
  [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol) to connect many SPs to a single IdP.

- **Single Sign-on**: Using a federated auth protocol to facilitate a _single_ means of _authentication_ for your users;
  does not necessarily facilitate _authorization_

- **Role-based Access Control (RBAC)**: An _authorization_ paradigm that assigns permissions to users based on which
  groups they are in
...
<MarkdownAlert severity="warning">
  You should only configure ONE root identity provider for your organization. Only complete this guide section ONCE for
  your production environment (or your most secure environment that contains a Kubernetes cluster).
</MarkdownAlert>

There are many managed IdPs available and you have likely heard of a few such as [Okta](https://www.okta.com/) or
[Microsoft Entra ID](https://learn.microsoft.com/en-us/entra/fundamentals/whatis). However, we _strongly_ recommend
self-hosting. Your IdP will control _all_ access to your systems, and leaving this control in the hands of a third-party
leaves you vulnerable to outages and security incidents (not to mention egregious pricing).

Fortunately, there are also many production-ready self-hosted IdPs that contain all the same functionality. In our
analysis, the most robust and well-integrated tool is [Authentik](https://goauthentik.io/).

We provide both a module to deploy Authentik
([kube_authentik](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_authentik)) and many
modules to connect Authentik to the various systems in the Panfactum stack (e.g.,
...
AWS provides a service called [Simple Email Service (SES)](https://aws.amazon.com/ses/) that facilitates email sending
via [SMTP](https://datatracker.ietf.org/doc/html/rfc5321) (the standard protocol for sending email).

Before using it, you must first configure SES to use the domain you want to use send email from (e.g., `panfactum.com`).
We provide a module to do this:
[aws_ses_domain](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_ses_domain).

Let's deploy the module now:

1. Choose the domain that you want to use for automated email sending. This can be the same domain that you use to send
   email normally.
...
3. Add a new `terragrunt.hcl` file that looks like this:
   ...
   ::: code-group labels=\[aws_ses_domain/terragrunt.hcl]
...
6. Log into the AWS web console and navigate to the SES service. Under "Get set up," notice that your SES configuration
   ...
   Depending on your needs, you may need to
   [request production access](https://docs.aws.amazon.com/ses/latest/dg/request-production-access.html) to authorize
   your account for more permissive email sending. AWS adds this extra gate-keeping step to guard against spam senders.

7. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record that
   covers all of your email sending systems. We recommend following
   [this guide](https://docs.aws.amazon.com/ses/latest/dg/send-email-authentication-dmarc.html) to set this up. You can
   use the [aws_dns_records](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_dns_records) module to
   deploy the record.
...
2. Add a new `terragrunt.hcl` file that looks like this:
   ...
   ::: code-group labels=\[kube_authentik/terragrunt.hcl]
...
7. Save the `akadmin_bootstrap_token` value in your `.env` as `AUTHENTIK_TOKEN`. Save the `authentik_url` as the
   `authentik_url` field in your `global.yaml`. This will be used by the
   [Authentik OpenTofu (Terraform) provider](https://registry.terraform.io/providers/goauthentik/authentik/latest/docs)
   in subsequent steps to configure Authentik.
...
8. Navigate to the `domain` you configured in step 2 in a web browser. You should receive a login prompt like this (may
...
10. After successful sign-in, click the "Admin Interface" button in the top-right (if not already in the admin
    dashboard):
...
12. Navigate to System > Brands. Edit the `authentik` brand. Disable the `Default` setting. This will conflict with the
    automated setup in the next guide section.
...
provide a module that augments this initial setup with more secure authentication flows, recovery flows, and branding:
[authentik_core_resources](/docs/stable-25-04/reference/infrastructure-modules/direct/authentik/authentik_core_resources).

This module also allows you to define user groups. Groups in Authentik will be assigned to roles in service providers as
the foundational mechanism for implementing RBAC across your ecosystem. See the
[Panfactum RBAC reference](/docs/stable-25-04/reference/rbac) for more information.
...
2. Add a new `terragrunt.hcl` file that looks like this:
   ...
   ::: code-group labels=\[authentik_core_resources/terragrunt.hcl]
...
Now that Authentik is bootstrapped, we will stop using the insecure bootstrapped user and switch to a personal user
account.
...
3. Reset the new user's password.
   ...
   4. In a minute or two, you should receive a recovery email at your user's email address: [^3]
      ...
      <MarkdownAlert severity="warning">
        If Authentik is configured to send emails from a domain that is different that your user's email domain, AWS SES
        will reject the email while in sandbox mode (see above).
   ...
   6. Next, you will need to set up your preferred MFA method. Since you are in `superusers`, you may need to use
      WebAuthn for which we recommend [Yubikeys](https://www.yubico.com/).
...
4. Provision a new API token.
   ...
   7. We recommend using a short-lived, expiring token as the credential will have root access to Authentik and thus
      _every_ system in your ecosystem. Generally, you will only need this for initially setting up the Authentik
      infrastructure modules which will not change frequently.

   8. Once the token has been created, copy it to your clipboard via the copy button on the far right of the token's row
      in the Tokens table.
...
6. (Recommended) Add a secondary MFA device in case you lose or damage your primary. If you skip this step, you increase
   your risk of becoming locked out of your entire ecosystem permanently.
...
[^3]:
    If you do not receive an email or if clicking on the link results in a "Request has been denied" error from
    Authentik, it is possible your email client is scanning the URLs which can either consume the one-time password
    reset token or even result in the email never being delivered at all. This is particularly prevalent with Microsoft
    Outlook SafeLinks. Ensure that you have whitelisted the Authentik domain as trustworthy.

## Next Steps

Your IdP is now fully configured and ready to be connected to the other systems set up in this guide such as AWS,
Kubernetes, and Vault.

<BootstrappingGuideNav
  backHref={"/docs/stable-25-04/guides/bootstrapping/databases"}
  forwardHref={"/docs/stable-25-04/guides/bootstrapping/federated-auth"}
  stepNumber={19}
/>
