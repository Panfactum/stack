...
Like internal networking, *inbound* networking has several moving parts. We won't cover them in detail within this
guide, but we do in our [concept documentation.](/docs/stable-25-04/concepts/networking/cluster-networking)
...
[kube\_external\_dns](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
...
As the cluster nodes are running in *private* subnet, the first step to providing inbound networking is deploying a
*public* gateway in your VPC for inbound traffic to connect with before it is forwarded onto your Kubernetes nodes. AWS
...
We provide a module to deploy it:
[kube\_aws\_lb\_controller](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_aws_lb_controller)
...
We provide a module to deploy it:
[kube\_ingress\_nginx](/docs/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_ingress_nginx).
...
While the NGINX-ingress controllers *are* successfully running, they will not process any traffic until you create an
...
1. Return the `kube_vault` module you deployed in the
   [Vault guide section](/docs/stable-25-04/guides/bootstrapping/vault).
...
6. Let's see that in action. Run `stern . -n ingress-nginx` to start capturing logs from all the NGINX servers. Now
   ...
   Notice that cert-manager has successfully provisioned a *public* TLS certificate and NGINX has picked it up to allow
...
7. Moreover, notice that NGINX properly secures the site in a standard way by setting security headers for the browser.
   ...
   [kube\_ingress](/docs/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) module.
   `kube_vault` uses it internally, and you can use it directly in your projects.
...
[^1]: We do not want to use `kubectl port-forward` for this purpose; that was just a stop-gap measure during the
    ...
    cluster. Finally, `kubectl port-forward` connects directly to a single *pod* which is prone to service disruptions
...
We provide a bastion deployment module:
...
4. Update the values to the correct values for your setup. See the
   [reference docs](/docs/stable-25-04/reference/configuration/ssh) for more information.
...
2. We will open a tunnel to the service bound to your `localhost:3030` that will route connections *through the
   bastion.* Run `pf-tunnel -b <bastion_name> -r nginx-status.ingress-nginx:18080 -l 3030`. Replace `<bastion_name>`
...
<BootstrappingGuideNav backHref={"/docs/stable-25-04/guides/bootstrapping/autoscaling"} forwardHref={"/docs/stable-25-04/guides/bootstrapping/maintenance-controllers"} stepNumber={16} />
