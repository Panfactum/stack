...
For more information on why the Panfactum stack includes a service mesh and why we chose Linkerd, check out our
[concept documentation.](/docs/stable-25-04/concepts/networking/cluster-networking#service-mesh)

## Deploy Linkerd

[Linkerd](https://linkerd.io/2.15/overview/) is likely the most popular service mesh for Kubernetes, and we provide a
module for deploying it:
[kube\_linkerd](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_linkerd)

This module not only deploys linkerd but also connects it to the certificate infrastructure that we provisioned in the
previous section. It also installs linkerd-specific observability tooling that we will review later.
...
<MarkdownAlert severity="info">
  Note that you may receive some warning about components not being up-to-date. Linkerd produces many minor releases and
  these warning are benign. You should stick with the default version included in the Panfactum stack unless you have a
  specific reason to deviate.
</MarkdownAlert>

### Verify Certificate Rotation

While linkerd may be working after the initial deploy, we also want to ensure it *continues* to work after its
...
Linkerd also comes with a handy debugging suite called `viz` that can be easily installed (temporarily) using the cli.
Run this now: `linkerd viz install | kubectl apply -f -`.
...
Linkerd works by injecting sidecar containers into your pods to connect them to the mesh. This happens on pod *creation*
so already existing pods will not have any sidecars injected. Let's restart them to connect them to the mesh:
...
2. Rotate a certificate to force cert-manager to connect with Vault:
...
3. The tap session should produce a response similar to this:
   ...
   Notice that the `src` IP address will be one of the cert-manager pods and the `dst` will be one of your Vault pods.
   Additionally, notice that `tls=true` indicates that **the request came over a TLS connection even though Vault itself
   was never configured with TLS.** This is the service mesh sidecar container in action.

The Linkerd documentation also provides even more granular methods that you can use to
[validate the security of your traffic](https://linkerd.io/2.15/tasks/validating-your-traffic/).

### Remove Viz

We will revisit observability tooling in a later section, so for now, let's remove the manually created `viz` to keep
the cluster clean of redundant resources:
...
we configure *external* networking, we will address autoscaling as the cluster is likely already running 75+ containers
and will soon become resource constrained.

<BootstrappingGuideNav backHref={"/docs/stable-25-04/guides/bootstrapping/certificate-management"} forwardHref={"/docs/stable-25-04/guides/bootstrapping/autoscaling"} stepNumber={14} />
