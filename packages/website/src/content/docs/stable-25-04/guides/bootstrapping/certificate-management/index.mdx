...
our [concept docs on network cryptography](/stable-25-04/concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_cert_issuers).
...
4. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/stable-25-04/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/stable-25-04/reference/infrastructure-modules/direct/aws/aws_registered_domains)
      and [aws\_delegated\_zones](/stable-25-04/reference/infrastructure-modules/direct/aws/aws_delegated_zones) modules
