...
1. [**Set up AWS Root Profile**](/stable-25-04/guides/bootstrapping/preparing-aws#create-access-keys-for-each-account): Ensure you have configured an AWS root profile that the script will use to interact with the necessary AWS services.

2. [**Update the `.kube/config.user.yaml`**](/stable-25-04/guides/bootstrapping/kubernetes-cluster#set-up-kubeconfig): Run `pf-update-kube` to ensure your Kubernetes configuration is up to date and ready for cluster operations.
