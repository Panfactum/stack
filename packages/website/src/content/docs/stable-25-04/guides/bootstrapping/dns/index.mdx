...
[registrar](https://en.wikipedia.org/wiki/Domain_name_registrar) (an organization that allows you to *lease* domain
...
[aws\_registered\_domains](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_registered_domains) module.
[^1]
...
1. When you purchased your domain name, AWS *may* have automatically set up a hosted zone for you. You should delete the
...
[^1]: The domains will have the following defaults you can override if desired:
    ...
    refer to the
    [module documentation](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_registered_domains).

[^2]: If the zone does not exist, you may skip this step. If you want to *keep* this zone, follow the import instructions
    above.
...
[^3]: Note that `@1.1.1.1` instructs `delv` to use Cloudflare's DNS servers instead of your network's default. Some
...
* Limits the permissions needed to be granted to users and services in each environment. To add records to the subdomain
  `dev.panfactum.com`, a user would not need *any* access to records in other environments. This would not be possible
  if all records were stored in a single, global hosted zone.

This is accomplished via a concept called
[subdomain delegation](/docs/stable-25-04/concepts/networking/subdomain-delegation). Panfactum's
[aws\_delegated\_zones](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_delegated_zones) will provide
the relevant setup for this to work.
...
3. Create a `terragrunt.hcl` in that directory that looks like this: [^4]
   ...
   ::: code-group labels=\[global/aws\_delegated\_zones\_production/terragrunt.hcl,
   global/aws\_delegated\_zones\_development/terragrunt.hcl]
...
4. Create a `module.yaml` in that directory:
   ...
   ::: code-group labels=\[global/aws\_delegated\_zones\_production/module.yaml,
   global/aws\_delegated\_zones\_development/module.yaml]
...
[^4]: Note that this is the first time we will take advantage of terragrunt's
...
If you have static records to deploy, you can use our
[aws\_dns\_records](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws_dns_records) module to deploy them
to the appropriate hosted zone.
...
<BootstrappingGuideNav backHref={"/docs/stable-25-04/guides/bootstrapping/infrastructure-as-code"} forwardHref={"/docs/stable-25-04/guides/bootstrapping/aws-networking"} stepNumber={6} />
