...
* [Guide for writing and deploying IaC modules](/stable-25-04/guides/iac/first-party-modules)

## Overview

In the Panfactum Stack, your code will run on the Kubernetes clusters you created in the
[bootstrapping guide](/stable-25-04/guides/bootstrapping/overview). [^1]
...
1. Build a container image containing the code your wish to deploy. We recommend using our
   [BuildKit addon](/stable-25-04/guides/addons/buildkit/installing) to build from Dockerfiles. [^10]

2. [Create a first-party IaC module](/stable-25-04/guides/iac/first-party-modules#writing-modules)
   ...
   | :----------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
   | [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) | [kube\_namespace](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_namespace)        | A logical container for Kubernetes resources. Most resources must be assigned a namespace.                              |
   | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)        | [kube\_deployment](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)      | Orchestrates stateless workloads such as webservers.                                                                    |
   | [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)      | [kube\_stateful\_set](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set) | Orchestrates for stateful workloads such as databases.                                                                  |
   | [CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/)            | [kube\_cron\_job](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job)         | Periodically runs one-shot processes on a schedule such as system maintenance scripts.                                  |
   | [DaemonSet](https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/)          | [kube\_daemon\_set](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_daemon_set)     | Runs a pod on every tolerated node in the cluster.                                                                      |
   | [Argo Workflow](https://argoproj.github.io/workflows/)                                     | [wf\_spec](/stable-25-04/reference/infrastructure-modules/submodule/workflows/wf_spec)                       | An advanced pattern available through our [Workflow Engine Addon](/stable-25-04/guides/addons/workflow-engine/installing). |

3. [Deploy the IaC module](/stable-25-04/guides/iac/first-party-modules#deploying-modules) by configuring it in the appropriate environment directory and running `terragrunt apply`.

4. (Optional) Connect the module deployment to our [CI / CD addon.](/stable-25-04/guides/cicd/getting-started)
...
[^2]: In fact, our submodules utilize a shared interface for defining pods: [kube\_pod](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_pod).
...
<MarkdownAlert severity="info">
  ...
  as a [step in the bootstrapping guide](/stable-25-04/guides/bootstrapping/kubernetes-cluster#configure-pull-through-cache)).
  We provide a submodule for easily referencing the various registry addresses: [aws\_ecr\_pull\_through\_cache\_addresses](/stable-25-04/reference/infrastructure-modules/submodule/aws/aws_ecr_pull_through_cache_addresses).
</MarkdownAlert>
...
[module docs](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_pod) for more information).
...
* Every container has a ready-only root filesystem. If you need to write to disk, we recommend using temporary directories (see below)
  or a [kube\_stateful\_set](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set). However, you
...
See our [high availability guide](/stable-25-04/guides/deploying-workloads/high-availability) for more information.
...
[bin-packing scheduler.](/stable-25-04/guides/bootstrapping/autoscaling#deploy-bin-packing-kubernetes-scheduler) This
...
All of our controller submodules such as [kube\_deployment](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)
...
* `common_secrets`: A key-value mapping of secret values. The values will be stored in a Kubernetes
  ...
  reference](/stable-25-04/reference/rbac)).
...
[^8]: Unless you are using [kube\_stateful\_set](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set)
...
Pod created by Panfactum submodules will automatically be labeled with our [standard resource tags](/stable-25-04/reference/resource-tags).
...
can utilize the `common_pod_labels` and `common_pod_annotations` inputs of the [kube\_policies](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_policies)
...
define precedence for pod preemption. Our [kube\_policies](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_policies)
...
[here](/stable-25-04/guides/deploying-workloads/permissions).
...
We provide a [custom controller](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_node_image_cache_controller) that will automatically cache images on nodes before the pods are scheduled
...
[kube\_node\_image\_cache](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) submodule.
...
(e.g., [kube\_deployment](/stable-25-04/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)).
