...
* You have prior experience working with infrastructure tools such as
  OpenTofu (Terraform) and Terragrunt. If you are not, please review the [concept docs](/docs/stable-25-04/concepts/iac)
  before proceeding.

* You have completed the necessary [repository setup guide](/docs/stable-25-04/guides/bootstrapping/configuring-infrastructure-as-code).
...
The [configuration-as-code](/docs/stable-25-04/concepts/iac#configuration-as-code) for each deployed module is stored in your repository's
...
[^1]: This folder lives in your stack repository, is defined by the repo variable `environments_dir`,
    and was created when running `pf-update-terragrunt`. For more information, see our [repository setup guide](/docs/stable-25-04/guides/bootstrapping/configuring-infrastructure-as-code)
...
While Terragrunt should have been properly configured when completing the [bootstrapping guide](/docs/stable-25-04/guides/bootstrapping/configuring-infrastructure-as-code),
you can review the standard Panfactum configuration [rules here](/docs/stable-25-04/reference/configuration/terragrunt-variables).

### Local Deployments

When working with modules locally, it can be helpful to override your organization's values in the [standard terragrunt
configuration files.](/docs/stable-25-04/reference/configuration/terragrunt-variables) For example, you may want to use a different provider configuration to pull in your specific
...
(set by the `repo_primary_branch` [repo variable](/docs/stable-25-04/reference/configuration/repo-variables)) are the values
...
We provide two [terragrunt variables](/docs/stable-25-04/reference/configuration/terragrunt-variables) for setting the module version:

* `pf_stack_version`: the [Panfactum module](/docs/stable-25-04/reference/infrastructure-modules/overview) version to use; used
...
We use the following [terragrunt variables](/docs/stable-25-04/reference/configuration/terragrunt-variables) to construct `include.panfactum.locals.pf_stack_source`:

* `module`: The name of the module to deploy. **Defaults to the directory name if not provided.** Can be overridden in `module.yaml`.

* `pf_stack_version`: Check [here](/docs/stable-25-04/reference/releases) for the full list of available releases. This should
...
We provide ready-made configuration for many providers (see the [full list here](/docs/stable-25-04/reference/configuration/terragrunt-variables#providers)).
...
configuration variables, see the [reference docs](/docs/stable-25-04/reference/configuration/terragrunt-variables#providers).
...
that integrates with terragrunt. See our [guide for setting up sops](/docs/stable-25-04/guides/bootstrapping/infrastructure-as-code).
...
For a list of all the common input variables see this [reference list.](/docs/stable-25-04/reference/infrastructure-modules/overview#common-variables)
...
You can easily do this for any set of modules by manipulating a few [Terragrunt variables](/docs/stable-25-04/reference/configuration/terragrunt-variables#module-source):
