...
GitHub Actions are **not** the recommended method for setting up CI / CD in Panfactum clusters. We provide a full set of
guides for our recommended architecture [here](/docs/stable-25-04/guides/cicd/getting-started).

Additionally, GitHub Actions have many shortcomings compared to a more full-featured, Kubernetes-native workflow engine
such as Argo Workflows. See our detailed analysis and list of limitations [here](/docs/stable-25-04/concepts/cicd/gha).

## Install the Runner Controller

To begin using self-hosted runners, you must first install the
[Actions Runner Controller (ARC)](https://github.com/actions/actions-runner-controller) which is a Kubernetes operator
that orchestrates and scales the runners based on Kubernetes custom resources.

We provide a module to deploy this controller:
[kube_gha](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_gha).
...
2. Add a `terragrunt.hcl` to that directory that looks like this:
   ...
   ::: code-group labels=[kube_gha/terragrunt.hcl]
...
To create runners, you will use our
[kube_gha_runners](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_gha_runners) module.
...
2. Add a `terragrunt.hcl` and sops-encrypted `secrets.yaml` to that directory that looks like this:
   ...
   ::: code-group labels=[kube_gha_runners/terragrunt.hcl, kube_gha_runners/secrets.yaml]
   ...
   1. Follow
      [GitHub's documentation](https://docs.github.com/en/actions/hosting-your-own-runners/managing-self-hosted-runners-with-actions-runner-controller/authenticating-to-the-github-api#authenticating-arc-with-a-personal-access-token-classic)
      to provision a Classic Personal Access Token (PAT) with the correct permissions. Provide this token as an input
      via `github_token`. Make sure that the token is
      [encrypted with sops.](/docs/stable-25-04/guides/iac/deploying-modules#using-secrets)

   2. The `runners` input is a key-value map that defines the different runner groups that your workflow jobs can
      target. The keys are the names of the groups and the values are their configuration. See the
      [module documentation](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_gha_runners) for
      information on the various configuration values.

      <MarkdownAlert severity="warning">
        The names of your runners must be globally unique to your organization. So if you install runners in multiple
        clusters, they cannot have identical names.
      </MarkdownAlert>

   3. The only required configuration value is the `github_config_url` which defines where in GitHub your runners will
      be registered for use. You can register runners to either (a) a single repository, (b) an entire organization, or
      (c) an entire enterprise. The `github_config_url` is simply the URL to one of those entities (e.g.,
      `https://github.com/Panfactum/stack`). We recommend registering runners to your organization.

3. Run `pf-tf-init` in the directory to enable the required providers.

4. Run `terragrunt apply`.

Assuming you have registered runners to your organization, you should now be able to see your runners in your GitHub
...
4. Open the GitHub repository in a web browser. Navigate to the Actions tab. You should see the "Example" workflow in
...
6. Run the workflow. Within a minute, the workflow job should begin, and you should see the runner pod in your cluster.
   ...
   <MarkdownAlert severity="info">
     If the workflow is stuck in a pending state, double check that your runner is available on the repository by
     visiting Actions > Runners > Self-hosted runners. Additionally, make sure that the `runs-on` value in your
