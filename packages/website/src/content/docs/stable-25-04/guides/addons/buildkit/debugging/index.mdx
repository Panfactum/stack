...
BuildKit uses your local client's credentials to push and pull images from image registries like ECR. If you are not
authorized to interact with a registry, you may receive an HTTP 401 error like below:
...
- You may be trying to interact with other private registries such as ghcr.io. See
  [these docs](/docs/stable-25-04/guides/addons/buildkit/building-images#authenticating-with-private-registries) for
  information on how to properly set up authentication.
...
Occasionally, an intermediate system might break the connection (e.g., an SSH bastion restarting) or the BuildKit server
itself might be closed since they run on spot instances.
...
If you have configured your ECR repo to have
[image immutability](https://docs.aws.amazon.com/AmazonECR/latest/userguide/image-tag-mutability.html), that means that
new images cannot be pushed to a tag that has already been used. [^1]
...
Generally, this error is desirable; you should avoid accidentally changing build artifacts such as images once they are
created. This can impact the behavior of live systems that depend on these artifacts.

If you _really_ need to replace the image, a superuser can use the AWS API (either via the web console or CLI) to delete
the image.
