...
<ModuleHeader
  ...
  sourceHref="https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_STABLE_25_04__/packages/infrastructure/aws_registered_domains"
  status="stable"
  type="direct"
/>
...
Description: The type of the contact. Defaults to PERSON if admin_organization_name is not provided. Otherwise, defaults
to COMPANY.
...
Description: The type of the contact. Defaults to PERSON if registrant_organization_name is not provided. Otherwise,
defaults to COMPANY.
...
Description: The type of the contact. Defaults to PERSON if tech_organization_name is not provided. Otherwise, defaults
to COMPANY.
...
This module does **not** register domains but rather adopts domains that you have already registered and updates their
information. Additionally, destroying this module does un-register the domains; it only removes them from the terraform
state.
...
These people will be contacted in the case of billing issues, abuse, or required changes. Their information is **not**
publicly available, but will be made available to the registrar (AWS).
