...
<ModuleHeader name="aws_eks" sourceHref="https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_STABLE_25_04__/packages/infrastructure/aws_eks" status="stable" type="direct"/>
...
- A set of "controller" [node groups](https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html) with a static size for running cluster-critical controllers. Nodes use the [Bottlerocket](https://bottlerocket.dev/) distribution.
  Autoscaled nodes are deployed via our [kube\_karpenter](/docs/stable-25-04/reference/infrastructure-modules/kubernetes/kube\_karpenter) module.
...
by the `vpc_id` input as these are the subnets created by the [aws\_vpc](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws\_vpc) module.
...
For an [SLA target of level 2 or above](/docs/stable-25-04/guides/deploying-workloads/high-availability), you MUST provide **at least 3** subnets (each in a different AZ).
...
This module is intended to be installed as a part of [this guide](/docs/stable-25-04/guides/bootstrapping/kubernetes-cluster) which includes manual steps
...
The SSO roles are installed into each account via [aws\_iam\_identity\_center\_permissions](/docs/stable-25-04/reference/infrastructure-modules/direct/aws/aws\_iam\_identity\_center\_permissions)
...
in the [kube\_policies](/docs/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube\_policies). AWS doesn't install
