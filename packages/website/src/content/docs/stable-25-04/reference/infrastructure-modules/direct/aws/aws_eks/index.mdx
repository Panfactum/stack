...
* A set of "controller" [node groups](https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html) with a static size for running cluster-critical controllers. Nodes use the [Bottlerocket](https://bottlerocket.dev/) distribution.
  Autoscaled nodes are deployed via our [kube\_karpenter](/stable-25-04/reference/infrastructure-modules/kubernetes/kube_karpenter) module.
...
by the `vpc_id` input as these are the subnets created by the [aws\_vpc](/stable-25-04/reference/infrastructure-modules/direct/aws/aws_vpc) module.
...
For an [SLA target of level 2 or above](/stable-25-04/guides/deploying-workloads/high-availability), you MUST provide **at least 3** subnets (each in a different AZ).
...
This module is intended to be installed as a part of [this guide](/stable-25-04/guides/bootstrapping/kubernetes-cluster) which includes manual steps
...
The SSO roles are installed into each account via [aws\_iam\_identity\_center\_permissions](/stable-25-04/reference/infrastructure-modules/direct/aws/aws_iam_identity_center_permissions)
...
in the [kube\_policies](/stable-25-04/reference/infrastructure-modules/direct/kubernetes/kube_policies). AWS doesn't install
