...
<ModuleHeader
  name="aws_iam_identity_center_permissions"
  sourceHref="https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_STABLE_25_04__/packages/infrastructure/aws_iam_identity_center_permissions"
  status="stable"
  type="direct"
/>
...
2. `admins`: Maps to a role that has admin access to most items, but attempts to block permissions that could create
   obvious security problems or delete key infrastructure.
...
When using tools like the AWS CLI, AWS sessions will automatically be renewed _so long as your SAML authentication is
not expired_.

As a result, the SAML authentication expiration time is going to be the value that most impacts your user ergonomics. To
adjust this value, you will need to follow
[this guide](https://docs.aws.amazon.com/singlesignon/latest/userguide/configure-user-session.html#user-session-duration-how-to-configure).

Since it automatically renews, we recommend keeping the AWS session expiration time fairly short (1 hour). This provides
a small improvement to security as this decreases the useful lifetime of stolen session tokens. You can adjust the value
via the `session_duration_hours` input.
...
Description: The number of hours that AWS sessions will last. This is NOT the time before needing to re-authenticate
with your IdP, but rather the amount of time that AWS session tokens last before expiring.
