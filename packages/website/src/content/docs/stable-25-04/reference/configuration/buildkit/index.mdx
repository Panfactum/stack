...
1. Instructs the `pf-update-buildkit --build` command how to generate the registry
   [credential helpers](https://github.com/docker/docker-credential-helpers) used by BuildKit. This command must be run
   by a superuser with root access to the BuildKit environment.
...
<div class="h-4" />
::: code-group labels=[.buildkit/config.yaml] ```yaml bastion: production-primary module:
production/us-east-2/kube_buildkit ``` :::
