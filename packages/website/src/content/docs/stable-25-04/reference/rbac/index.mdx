...
For every system, Panfactum modules configure the following default roles. **These are scoped to the specific system /
environment.** They are not global and a user can (and will) have different roles in different environment.
...
These are the default groups provisioned by the
[authentik_core_resources](/docs/stable-25-04/reference/infrastructure-modules/direct/authentik/authentik_core_resources).
**These are global.**

The below table charts their _recommended_ role by environment; however, by default they do not confer any permissions
as this depends on your environment configuration. [^1]
