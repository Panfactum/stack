...
* `kube_cert_manager` and `kube_cert_issuers` modules have been consolidated into a single `kube_certificates` module in order to address various race conditions
  ...
  1. Create a `terragrunt.hcl` file in the `kube_certificates` directory with the following contents:
  ...
  1. From the **region** directory, run the following:
  ...
  1. Update the version of the Panfactum framework you are using.

  1. Navigate to the `kube_certificates` directory.

      1. Run `terragrunt init`.

      1. Run `terragrunt state push state.json && rm state.json`.

      1. Run `terragrunt apply` and review the changes. There should be only a few resources that will be replaced as we fixed a typo in the name.

  1. Remove the `kube_cert_issuers` and `kube_cert_manager` directories.

  1. Replace any references to `kube_cert_issuers` and `kube_cert_manager` with `kube_certificates` in your code.
