...
2. [Create a first-party IaC module](/docs/main/guides/iac/first-party-modules#writing-modules)
   ...
   | :----------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
   | [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) | [kube\_namespace](/docs/main/modules/kube_namespace/overview)        | A logical container for Kubernetes resources. Most resources must be assigned a namespace.                              |
   | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)        | [kube\_deployment](/docs/main/modules/kube_deployment/overview)      | Orchestrates stateless workloads such as webservers.                                                                    |
   | [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)      | [kube\_stateful\_set](/docs/main/modules/kube_stateful_set/overview) | Orchestrates for stateful workloads such as databases.                                                                  |
   | [CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/)            | [kube\_cron\_job](/docs/main/modules/kube_cron_job/overview)         | Periodically runs one-shot processes on a schedule such as system maintenance scripts.                                  |
   | [DaemonSet](https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/)          | [kube\_daemon\_set](/docs/main/modules/kube_daemon_set/overview)     | Runs a pod on every tolerated node in the cluster.                                                                      |
   | [Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)                      | [kube\_job](/docs/main/modules/kube_job/overview)                    | Runs a one-off process as a part of a module deployment. Useful for pre-/post-deployment scripts such as DB migrations. |
   | [Argo Workflow](https://argoproj.github.io/workflows/)                                     | [wf\_spec](/docs/main/modules/wf_spec/overview)                       | An advanced pattern available through our [Workflow Engine Addon](/docs/main/guides/addons/workflow-engine/installing). |
...
[^2]: In fact, our submodules utilize a shared interface for defining pods: [kube\_pod](/docs/main/modules/kube_pod/overview).
...
<MarkdownAlert severity="info">
  ...
  We provide a submodule for easily referencing the various registry addresses: [aws\_ecr\_pull\_through\_cache\_addresses](/docs/main/modules/aws_ecr_pull_through_cache_addresses/overview).
</MarkdownAlert>
...
* Every container has a ready-only root filesystem. If you need to write to disk, we recommend using temporary directories (see below)
  or a [kube\_stateful\_set](/docs/main/modules/kube_stateful_set/overview). However, you
...
All of our controller submodules such as [kube\_deployment](/docs/main/modules/kube_deployment/overview)
...
[^8]: Unless you are using [kube\_stateful\_set](/docs/main/modules/kube_stateful_set/overview)
...
can utilize the `common_pod_labels` and `common_pod_annotations` inputs of the [kube\_policies](/docs/main/modules/kube_policies/overview)
...
define precedence for pod preemption. Our [kube\_policies](/docs/main/modules/kube_policies/overview)
...
We provide a [custom controller](/docs/main/modules/kube_node_image_cache_controller/overview) that will automatically cache images on nodes before the pods are scheduled
...
[kube\_node\_image\_cache](/docs/main/modules/kube_node_image_cache/overview) submodule.
...
(e.g., [kube\_deployment](/docs/main/modules/kube_deployment/overview)).
