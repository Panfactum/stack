...
[^1]: The IdP itself should have already been provisioned in the [bootstrapping guide.](/main/guides/bootstrapping/identity-provider)

If your organization's administrators have not already [provisioned your account](/main/guides/user-management/provisioning-new-user), ask them now. You
will need to follow [this guide](/main/guides/user-management/setting-up-new-user) to set up your user for the first time.
...
2. Update the `config.user.yaml` to include the clusters you want to access and what AWS profile you want to use for that access. See
   the [reference docs](/main/reference/configuration/kubernetes#config.user.yaml) for the specific syntax.
...
* [Learn how to deploy infrastructure using infrastructure-as-code](/main/guides/iac/deploying-modules)

* [Customize your organization's devShell](/main/guides/development-shell/customizing)

* [Connect to databases](/main/guides/networking/database-connections)
