...
<MarkdownAlert severity="warning">
  The Panfactum [kube\_argo](/docs/main/modules/kube_argo/overview) does not create an Ingress for the Argo Workflow Server, so the HTTP API is only available
...
[kube\_sa\_auth\_workflow](/docs/main/modules/kube_sa_auth_workflow/overview)
