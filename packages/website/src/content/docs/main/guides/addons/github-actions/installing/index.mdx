...
provide a full set of guides for our recommended architecture [here](/main/guides/cicd/getting-started).

Additionally, GitHub Actions have many shortcomings compared to a more full-featured, Kubernetes-native workflow
engine such as Argo Workflows. See our detailed analysis and list of limitations [here](/main/concepts/cicd/gha).
...
We provide a module to deploy this controller: [kube\_gha](/main/reference/infrastructure-modules/direct/kubernetes/kube_gha).
...
To create runners, you will use our [kube\_gha\_runners](/main/reference/infrastructure-modules/direct/kubernetes/kube_gha_runners) module.
...
2. Add a `terragrunt.hcl` and sops-encrypted `secrets.yaml` to that directory that looks like this:
   ...
   1. Follow [GitHub's documentation](https://docs.github.com/en/actions/hosting-your-own-runners/managing-self-hosted-runners-with-actions-runner-controller/authenticating-to-the-github-api#authenticating-arc-with-a-personal-access-token-classic) to provision a
      ...
      as an input via `github_token`. Make sure that the token is [encrypted with sops.](/main/guides/iac/deploying-modules#using-secrets)
   ...
   2. The `runners` input is a key-value map that defines the different runner groups that your workflow jobs can target. The keys
      are the names of the groups and the values are their configuration. See the [module documentation](/main/reference/infrastructure-modules/direct/kubernetes/kube_gha_runners)
