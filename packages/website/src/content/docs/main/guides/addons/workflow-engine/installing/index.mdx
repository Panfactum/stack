...
<p class="subtitle">
  Install the management controllers and Web dashboard for [Argo Workflows](https://argoproj.github.io/workflows/) and
  [Argo Events](https://argoproj.github.io/argo-events/).
</p>

## Background

The [Argo Project](https://argoproj.github.io/) is a popular collection of Kubernetes controllers. Each controller
operates independently, but they do have integrations with one another.

Two of these controllers work together to enable sophisticated workflow logic: Argo Workflows and Argo Events.

### Argo Workflows

[Argo Workflows](https://argoproj.github.io/workflows/) provide a mechanism for defining
[DAGs](https://en.wikipedia.org/wiki/Directed_acyclic_graph) that orchestrate the execution of pods and containers
...
- Manged via declarative IaC (through
  [Kubernetes custom resources for Argo](https://argo-workflows.readthedocs.io/en/latest/fields/)).

- Provides an integrated web dashboard that (a) provides granular execution and debugging information, (b) the ability
  for non-technical users to create and manage workflows, and (c) integrations into our SSO framework.
...
- Deep integrations with Kubernetes, allowing it to benefit from all the other utilities installed in the Panfactum
  Stack.
...
[^1]:
    Argo Events can also be used to trigger many more actions than just Argo Workflows. See our
    [Event Bus Addon](/docs/main/guides/addons/event-bus/installing) guide.

Events can be inbound webhooks, messages in a message queue, or any other
[EventSource](https://argoproj.github.io/argo-events/concepts/event_source/) that Argo Events supports.

Because you will often want these types of external triggers, we install both Argo Workflows _and_ Argo Events in this
guide even though they are separate projects.

## Installing

We provide a module, [kube_argo](/docs/main/modules/kube_argo/overview), that deploys the controllers for both Argo
Workflows and Argo Events.

Let's deploy it now:

1. In the region with the cluster that you wish to deploy the workflow engine to, add a new directory called
   `kube_argo`.
...
In the `kube_argo` module, we create a
[WorkflowTemplate](https://argo-workflows.readthedocs.io/en/latest/workflow-templates/) called `test` in the `argo`
namespace. [^2] We will use this to create a Workflow and test if Argo Workflows is functioning correctly.

[^2]:
    A WorkflowTemplate is used to create Workflows. A Workflow is a single run of an execution graph (i.e., every
...
1. In a web browser, navigate to the domain you supplied for `argo_domain`. You should be met with a screen that looks
   like this:
...
3. Select "Workflow Templates", the second menu item on the left-hand navigation. On this page, you will see all the
   WorkflowTemplates that can be used to create new Workflows:

   <MarkdownImage src={WorkflowTemplatesImg} alt="Workflow templates" />

   You should see the `test` template. If you do not see any templates, ensure you have selected `argo` in the namespace
   filter.
...
5. The Workflow that gets generated from the `test` template will simply run a container that prints whatever string you
   supply as the `message` parameter. Adjust the `message` parameter as desired and click "+ Submit".
...
Let's now attempt to create a Workflow from the `test` template using the argo CLI that comes bundled in the Panfactum
devShell:
...
4. Run `argo logs -n argo <workflow-name>` where `<workflow-name>` is the generated name from step 2. Verify that your
...
- For a brief overview of all the capabilities of Argo Workflows, we strongly recommend working through
  [these Argo Workflow tutorials.](https://argo-workflows.readthedocs.io/en/latest/walk-through/)

- To start creating your own custom Workflows, take a look at
  [our documentation](/docs/main/guides/addons/workflow-engine/creating-workflows).

- Deploy some of [our pre-built Workflows](/docs/main/guides/addons/workflow-engine/prebuilt-workflows) for common tasks
  in the Panfactum Stack

- Learn how to use Workflows for your CI / CD system [here.](/docs/main/guides/cicd/getting-started)

- Learn more about how to leverage Argo Events [here.](/docs/main/guides/addons/event-bus/installing)
