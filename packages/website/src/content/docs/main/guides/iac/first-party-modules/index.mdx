...
Set the `iac_dir` repo variable to that directory per the [reference docs](/main/reference/configuration/repo-variables).
...
All the rules in the [main module deployment guide](/main/guides/iac/deploying-modules#defining-a-module-for-deployment) apply.
...
* Set the `repo_url` [repository variable](/main/reference/configuration/repo-variables). This will likely have already been set up when your system was first deployed.

* Set `GIT_USERNAME` and `GIT_PASSWORD` in your [personal `.env` file](/main/reference/configuration/dotenv). The values will be combined with `repo_url` to clone your IaC over HTTPS as follows:
...
<MarkdownAlert severity="info">
  ...
  by following [these instructions.](/main/reference/infrastructure-modules/submodule/workflow/wf_tf_deploy#authenticating-with-private-repositories)
</MarkdownAlert>
...
<MarkdownAlert severity="info">
  Even thought the infrastructure-as-code versions will differ based on the `version` value, the [configuration-as-code](/main/concepts/iac#configuration-as-code) (i.e., module versions, module inputs, etc.) for **all** module deployments will **always** be defined
  by the values in the primary integration branch. This is set by the `repo_primary_branch` [repo variable.](/main/reference/configuration/repo-variables)
...
In addition to the infrastructure modules that you deploy directly, we provide [many submodules](/main/reference/infrastructure-modules/overview)
...
and `pf_module_ref`. Whenever you change the `pf_stack_version` [Terragrunt variable](/main/reference/configuration/terragrunt-variables#module-source), these variables will
...
<MarkdownAlert severity="info">
  If you are [using a local copy of the Panfactum submodules](/main/guides/iac/deploying-modules#using-a-local-copy-of-panfactum-modules),
  ...
  by setting the `pf_stack_local_use_relative` [Terragrunt variable](/main/reference/configuration/terragrunt-variables#module-source) to `false`.
...
to **you**. For all available settings, see the [reference documentation.](/main/reference/configuration/terragrunt-variables)
