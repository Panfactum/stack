...
* You have prior experience working with infrastructure tools such as
  OpenTofu (Terraform) and Terragrunt. If you are not, please review the [concept docs](/main/concepts/iac)
  before proceeding.

* You have completed the necessary [repository setup guide](/main/guides/bootstrapping/configuring-infrastructure-as-code).
...
The [configuration-as-code](/main/concepts/iac#configuration-as-code) for each deployed module is stored in your repository's
...
[^1]: This folder lives in your stack repository, is defined by the repo variable `environments_dir`,
    and was created when running `pf-update-terragrunt`. For more information, see our [repository setup guide](/main/guides/bootstrapping/configuring-infrastructure-as-code)
...
While Terragrunt should have been properly configured when completing the [bootstrapping guide](/main/guides/bootstrapping/configuring-infrastructure-as-code),
you can review the standard Panfactum configuration [rules here](/main/reference/configuration/terragrunt-variables).

### Local Deployments

When working with modules locally, it can be helpful to override your organization's values in the [standard terragrunt
configuration files.](/main/reference/configuration/terragrunt-variables) For example, you may want to use a different provider configuration to pull in your specific
...
(set by the `repo_primary_branch` [repo variable](/main/reference/configuration/repo-variables)) are the values
...
However, this is only the [configuration-as-code](/main/concepts/iac#configuration-as-code). The
infrastructure modules (i.e., infrastructure-as-code) may be pinned to specific versions via the `terraform.source`
field.

We provide two [terragrunt variables](/main/reference/configuration/terragrunt-variables) for setting the module version:

* `pf_stack_version`: the [Panfactum module](/main/reference/infrastructure-modules/overview) version to use; used
  by `include.panfactum.locals.pf_stack_source`. (See below)

* `version`: the [first-party module](/main/guides/iac/first-party-modules) version to use; used by
  `include.panfactum.locals.source`.

### Deploying Panfactum Modules

If you are deploying a [Panfactum module](/main/reference/infrastructure-modules/overview), use
...
We use the following [terragrunt variables](/main/reference/configuration/terragrunt-variables) to construct `include.panfactum.locals.pf_stack_source`:

* `module`: The name of the module to deploy. **Defaults to the directory name if not provided.** Can be overridden in `module.yaml`.

* `pf_stack_version`: Check [here](/main/reference/releases) for the full list of available releases. This should
...
We provide ready-made configuration for many providers (see the [full list here](/main/reference/configuration/terragrunt-variables#providers)).
...
configuration variables, see the [reference docs](/main/reference/configuration/terragrunt-variables#providers).
...
that integrates with terragrunt. See our [guide for setting up sops](/main/guides/bootstrapping/infrastructure-as-code).
...
All Panfactum modules deploy resources with [standard tags / labels.](/main/reference/resource-tags).
...
For a list of all the common input variables see this [reference list.](/main/reference/infrastructure-modules/overview#common-variables)
...
You can easily do this for any set of modules by manipulating a few [Terragrunt variables](/main/reference/configuration/terragrunt-variables#module-source):
...
[^6]: For submodules, this works via the `pf_module_source` input provided to your modules by Terragrunt. See the
    [first-party IaC development documentation for more information.](/main/guides/iac/first-party-modules#using-panfactum-submodules)
