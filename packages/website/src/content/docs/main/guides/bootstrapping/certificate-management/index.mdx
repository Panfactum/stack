...
our [concept docs on network cryptography](/main/concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/main/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/main/reference/infrastructure-modules/direct/kubernetes/kube_cert_issuers).
...
3. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/main/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/main/reference/infrastructure-modules/direct/aws/aws_registered_domains)
      and [aws\_delegated\_zones](/main/reference/infrastructure-modules/direct/aws/aws_delegated_zones) modules
