...
We provide a module to deploy this controller: [kube\_reloader](/main/reference/infrastructure-modules/direct/kubernetes/kube_reloader).
...
While the pull-through image cache deployed in [earlier in this guide](/main/guides/bootstrapping/kubernetes-cluster#configure-pull-through-cache)
...
Fortunately, we can provide a solution to this problem, the [kube\_node\_image\_cache\_controller](/main/reference/infrastructure-modules/direct/kubernetes/kube_node_image_cache_controller).
...
[^22]: While many of our modules automatically specify images for the cache, you can specify additional images
    via the  [kube\_node\_image\_cache](/main/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) submodule. This
    is a *submodule* so you can use it many times inside [your own first-party infrastructure modules.](/main/guides/iac/first-party-modules)
...
We provide a module to deploy it: [kube\_pvc\_autoresizer](/main/reference/infrastructure-modules/direct/kubernetes/kube_pvc_autoresizer).
...
We provide a module for the Descheduler: [kube\_descheduler](/main/reference/infrastructure-modules/direct/kubernetes/kube_descheduler).
...
via the [kube\_aws\_ebs\_csi](/main/reference/infrastructure-modules/direct/kubernetes/kube_aws_ebs_csi) module we installed in
[a prior section](/main/guides/bootstrapping/storage-interfaces).
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](/main/reference/infrastructure-modules/direct/kubernetes/kube_external_snapshotter).
...
We provide a module to deploy this functionality: [kube\_velero](/main/reference/infrastructure-modules/direct/kubernetes/kube_velero).
