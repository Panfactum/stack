...
[concept documentation](/docs/main/concepts/background/networking/aws-primitives) before proceeding.
...
[SLA target level](/docs/main/guides/deploying-workloads/high-availability#sla-levels) when deploying each environment.
The configured SLA target changes how Panfactum modules behave. Higher targets will improve uptime but also increase
...
To choose a level, set the [Terragrunt variable](/docs/main/reference/configuration/terragrunt-variables#metadata)
...
[^91]:
    ...
    [the reference docs](/docs/main/reference/configuration/terragrunt-variables#metadata).

## Create your VPC

We provide an [aws_vpc](/docs/main/modules/aws_vpc/overview) module to deploy the VPC. This includes not only the VPC
but also configuration of subnets, routing tables, flow logs, etc. See the module reference docs for a more
comprehensive overview.

### Deploy the AWS VPC Module

We will now deploy the [aws_vpc](/docs/main/modules/aws_vpc/overview) module via terragrunt:
...
6. Ensure that your [NAT nodes](/docs/main/concepts/background/networking/nat) are running and healthy:
...
<MarkdownAlert severity="warning">
  If you deviate from the default settings for the [aws\_vpc](/docs/main/modules/aws_vpc/overview) module, you will need
  to follow the instructions inside the module documentation for further instructions on how to configure AWS networking
  to support Panfactum infrastructure.
</MarkdownAlert>
...
<BootstrappingGuideNav
  backHref={"/docs/main/guides/bootstrapping/dns"}
  forwardHref={"/docs/main/guides/bootstrapping/kubernetes-cluster"}
  stepNumber={7}
/>
