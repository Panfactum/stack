...
When using Panfactum, you will need to choose an [SLA target level](/docs/main/guides/deploying-workloads/high-availability#sla-levels) when deploying each environment. The configured SLA
...
We provide an [aws\_vpc](/docs/main/reference/infrastructure-modules/direct/aws/aws_vpc) module to deploy the VPC.
...
2. Add a `terragrunt.hcl` that looks like [this](https://github.com/Panfactum/reference-infrastructure/blob/__PANFACTUM_VERSION_MAIN__/environments/production/us-east-2/aws_vpc/terragrunt.hcl). Replace
