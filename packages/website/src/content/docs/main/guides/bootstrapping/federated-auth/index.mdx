...
<MarkdownAlert severity="info">
  ...
  * [aws\_eks](/docs/main/modules/aws_eks/overview) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encrypt\_key](/docs/main/modules/aws_kms_encrypt_key/overview)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](/docs/main/modules/kube_vault/overview) to allow SSO authentication
...
[authentik\_aws\_sso](/docs/main/modules/authentik_aws_sso/overview).
...
to automatically set this up: [aws\_iam\_identity\_center\_permissions](/docs/main/modules/aws_iam_identity_center_permissions/overview).
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](/docs/main/modules/aws_kms_encrypt_key/overview) module
   * Vault clusters deployed with [kube\_vaults](/docs/main/modules/kube_vault/overview)
...
[authentik\_vault\_sso](/docs/main/modules/authentik_vault_sso/overview).
...
We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](/docs/main/modules/vault_auth_oidc/overview).
