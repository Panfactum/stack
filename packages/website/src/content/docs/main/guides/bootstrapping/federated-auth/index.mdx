...
- [aws_eks](/docs/main/modules/aws_eks/overview) to allow SSO authentication with the cluster KMS encryption key.

- sops keys (deployed with [aws_kms_encrypt_key](/docs/main/modules/aws_kms_encrypt_key/overview)) to allow SSO
  authentication with the sops KMS encryption key.

- [kube_vault](/docs/main/modules/kube_vault/overview) to allow SSO authentication with the Vault KMS unseal key.
</MarkdownAlert>
...
[authentik_aws_sso](/docs/main/modules/authentik_aws_sso/overview).
...
set this up: [aws_iam_identity_center_permissions](/docs/main/modules/aws_iam_identity_center_permissions/overview).
...
3. For the `acccount_access_configuration`, we recommend setting this up according to our
   [RBAC recommendations](/docs/main/reference/rbac), but you have the flexibility to choose what works best for your
   organization.
   ...
   3. The remaining values determine which Authentik groups get assigned which roles in the specified account. For
      descriptions of the roles, see this [reference document.](/docs/main/reference/rbac) You must have already allowed
      these groups access to AWS in the `authentik_aws_sso` module above.
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply **every** module
   ...
   - sops keys deployed with the [aws_kms_encrypt_key](/docs/main/modules/aws_kms_encrypt_key/overview) module

   - Vault clusters deployed with [kube_vaults](/docs/main/modules/kube_vault/overview)
...
5. Set up `config.yaml` according to [these reference docs](/docs/main/reference/configuration/aws). Here is an example
   file:
...
<MarkdownAlert severity="info">
  In general, your [terragrunt variables](/docs/main/reference/configuration/terragrunt-variables)
  ...
  access. See [this guide](/docs/main/guides/iac/deploying-modules#local-deployments) for more information.
</MarkdownAlert>
...
[authentik_vault_sso](/docs/main/modules/authentik_vault_sso/overview).
...
[vault_auth_oidc](/docs/main/modules/vault_auth_oidc/overview).
...
1. Remove your hardcoded `VAULT_ADDR` and `VAULT_TOKEN` token from your `.env` file. Make sure that you added the
   ...
   [inbound networking section.](/docs/main/guides/bootstrapping/inbound-networking#deploy-the-vault-ingress)
...
<BootstrappingGuideNav
  backHref={"/docs/main/guides/bootstrapping/identity-provider"}
  forwardHref={"/docs/main/guides/bootstrapping/next-steps"}
  stepNumber={20}
/>
