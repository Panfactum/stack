...
[concept documentation.](/docs/main/concepts/background/networking/cluster-networking#service-mesh)

## Deploy Linkerd

[Linkerd](https://linkerd.io/2.15/overview/) is likely the most popular service mesh for Kubernetes, and we provide a
module for deploying it: [kube_linkerd](/docs/main/modules/kube_linkerd/overview)
...
2. Add a `terragrunt.hcl` to that directory that looks like this:
   ...
   ::: code-group labels=\[kube_linkerd/terragrunt.hcl]
...
While linkerd may be working after the initial deploy, we also want to ensure it _continues_ to work after its
...
Linkerd works by injecting sidecar containers into your pods to connect them to the mesh. This happens on pod _creation_
...
we configure _external_ networking, we will address autoscaling as the cluster is likely already running 75+ containers
and will soon become resource constrained.

<BootstrappingGuideNav
  backHref={"/docs/main/guides/bootstrapping/certificate-management"}
  forwardHref={"/docs/main/guides/bootstrapping/autoscaling"}
  stepNumber={14}
/>
