...
<p class="subtitle">
  ...
  using our [aws\_eks](/docs/main/modules/aws_eks/overview) module.
</p>
...
[^1]:
    This cost is well worth it. Even if you were to self-manage the control plane using a tool like
...
<MarkdownAlert severity="info">
  ...
  Guide](/docs/main/guides/bootstrapping/suspend-kubernetes-cluster) to suspend resources and resume as you are setting
  up your cluster.
</MarkdownAlert>
...
configured to download the image from the public registry _only if it does not already contain the image in its cache_.
In this way, most images will only ever need to be downloaded from a public source _once_ during the initial deployment.

We provide a module for configuring this behavior:
[aws_ecr_pull_through_cache](/docs/main/modules/aws_ecr_pull_through_cache/overview).
...
[^9]:
    We store the config file in your repo and not in the typical location (`~/.kube/config`) so that it does not
...
1. Add a `config.user.yaml` file that looks like this: [^7]
   ...
   [^7]:
       This file is specific to every user as different users will have different access levels to the various clusters.
...
clusters. More information about this file can be found [here](/docs/main/reference/configuration/kubernetes).

<MarkdownAlert severity="warning">
  *Every* one of your users will need to set up their own `config.user.yaml` file, and we provide instructions for them
  [here](/docs/main/guides/getting-started/connect-to-infrastructure).
</MarkdownAlert>
...
additional [terragrunt variables](/docs/main/reference/configuration/terragrunt-variables).
...
<BootstrappingGuideNav
  backHref={"/docs/main/guides/bootstrapping/aws-networking"}
  forwardHref={"/docs/main/guides/bootstrapping/internal-cluster-networking"}
  stepNumber={8}
/>
