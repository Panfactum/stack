...
We will use the [kube\_vault](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_vault) infrastructure module to deploy
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/reference-infrastructure/blob/__PANFACTUM_VERSION_MAIN__/environments/production/us-east-2/kube_vault/terragrunt.hcl).

3. For now, set `wait` to `false`. This will enable `terragrunt apply` to succeed even though the Vault pods will not be ready. See below for more details.
...
7. Note that this deployment **will not succeed,** and you should see three vault instances which are in the unready state:
...
via the [vault\_core\_resources](/docs/main/reference/infrastructure-modules/direct/vault/vault_core_resources) module.
...
Let's now deploy [vault\_core\_resources](/docs/main/reference/infrastructure-modules/direct/vault/vault_core_resources):

1. Before proceeding, set `wait` to `true` in the `kube_vault` module. It is now expected that future `kube_vault` deployments should succeed.

2. Create a new directory adjacent to your `kube_vault` module called `vault_core_resources`.

3. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/reference-infrastructure/blob/__PANFACTUM_VERSION_MAIN__/environments/production/us-east-2/vault_core_resources/terragrunt.hcl).
...
in the [prior section](/docs/main/guides/bootstrapping/storage-interfaces).
...
<BootstrappingGuideNav backHref={"/docs/main/guides/bootstrapping/storage-interfaces"} forwardHref={"/docs/main/guides/bootstrapping/certificate-management"} stepNumber={12} />
