...
We will use the [kube_vault](/docs/main/modules/kube_vault/overview) infrastructure module to deploy Vault in a highly
available manner using
...
[^1]:
    Vault only keeps unencrypted data in memory and this includes the encryption key. As a result, when pods restart,
...
[^6]:
    The recovery keys are not the _unseal keys_. We do not utilize static unseal keys in the Panfactum setup as we rely
...
[^2]:
    This is highly dependent on organization size, but we recommend the following: `1` if you are a solo operator, `2`
...
[^3]:
    It can be tempting to make this a high number, but you will need these keys fairly regularly (expect about once per
...
[vault_core_resources](/docs/main/modules/vault_core_resources/overview) module.
...
Let's now deploy [vault_core_resources](/docs/main/modules/vault_core_resources/overview):
...
3. Add a `terragrunt.hcl` to that directory that looks like this:
   ...
   ::: code-group labels=\[vault_core_resources/terragrunt.hcl]
...
[prior section](/docs/main/guides/bootstrapping/storage-interfaces).
...
<BootstrappingGuideNav
  backHref={"/docs/main/guides/bootstrapping/storage-interfaces"}
  forwardHref={"/docs/main/guides/bootstrapping/certificate-management"}
  stepNumber={12}
/>
