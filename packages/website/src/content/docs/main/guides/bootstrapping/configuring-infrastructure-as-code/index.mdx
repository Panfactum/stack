...
<MarkdownAlert severity="warning">
  ...
  [concepts documentation](/main/concepts/iac) before continuing.
</MarkdownAlert>

The Panfactum framework uses the open-source Terraform fork, [OpenTofu](https://opentofu.org/), to define the [100+
infrastructure modules](/main/reference/infrastructure-modules/overview) that comprise the Panfactum framework. [^1] We
use a [configuration-as-code](/main/concepts/iac#configuration-as-code) manager for Terraform / OpenTofu called [Terragrunt](https://terragrunt.gruntwork.io/)
...
By default, this is the `main` branch. If you need to change it, make sure it is set as the `repo_primary_branch` (per the [docs](/main/reference/configuration/repo-variables)).
...
[^66]: You can override this folder by setting `environments_dir` (per the [docs](/main/reference/configuration/repo-variables)).
...
it must be set up to follow [these conventions](/main/reference/configuration/terragrunt-variables).
...
[^2]: See the [reference documentation](/main/reference/configuration/terragrunt-variables) for more information on the available values.
