...
Before you being, please ensure you have reviewed the guide for [pinning the Panfactum version](/main/guides/versioning/pinning).
...
1. Update the `pf_stack_version` [terragrunt variable](/edge/reference/configuration/terragrunt-variables#variables)
...
2. Ensure that the terragrunt `version` for that environment is set to `local` in order
   to deploy the local changes from step 1 (review [these docs](/main/guides/iac/first-party-modules)). [^2]
