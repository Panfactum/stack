...
1. For all domains in `domains`, ensure that a DNS record is configured to point to the domain to this cluster's NGINX
   ingress controller (via [kube\_external\_dns](/docs/main/modules/kube_external_dns/overview))
   and provide the ingress controller a TLS certificate for the domains
   (via [kube\_cert\_manager](/docs/main/modules/kube_cert_manager/overview)).
...
[kube\_cert\_issuers](/docs/main/modules/kube_cert_issuers/overview) provides a global
...
for performance and security improvements, see the [kube\_aws\_cdn](/docs/main/modules/kube_aws_cdn/overview) module.
