...
The Panfactum Stack's Cilium deployment ([kube\_cilium](/main/reference/infrastructure-modules/direct/kubernetes/kube_cilium)) has a few enhancements over the default Cilium install:
...
You can see our implementation in the [kube\_linkerd](/main/reference/infrastructure-modules/direct/kubernetes/kube_linkerd) terraform module.
...
In the Panfactum Stack, we use the Ingress-NGINX Controller ([kube\_ingress\_nginx](/main/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx))
...
We provide a submodule for defining Ingress manifests for our Ingress-NGINX controller: [kube\_ingress](/main/reference/infrastructure-modules/submodule/kubernetes/kube_ingress).
...
We provide a module for deploying CoreDNS to Panfactum Stack clusters, [kube\_core\_dns](/main/reference/infrastructure-modules/direct/kubernetes/kube_core_dns),
...
In the Panfactum Stack, we provide the [kube\_external\_dns](/main/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
module which will provision DNS records in [AWS Route 53.](https://aws.amazon.com/route53/) The specific instructions
for configuring this are covered in [the bootstrapping guide.](/main/guides/bootstrapping/inbound-networking#deploy-externaldns) [^21]
...
[kube\_bastion](/main/reference/infrastructure-modules/direct/kubernetes/kube_bastion).
...
For more information on SSH and why we chose this architecture over a VPN-based system, see [these concept docs.](/main/concepts/networking/cryptography#vpns)
...
TLS and the associated [X.509 certificate infrastructure](/main/concepts/networking/cryptography#x.509-certificates)
...
Stack via [kube\_cert\_manager](/main/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
[kube\_ingress](/main/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) submodule.

<MarkdownAlert severity="warning">
  This certificate provisioning process will only work if you have set up DNS in accordance with our
  [bootstrapping guide.](/main/guides/bootstrapping/dns)
</MarkdownAlert>
