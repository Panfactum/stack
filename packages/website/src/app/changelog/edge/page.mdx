...
* By default, PVCs created by controllers such as StatefulSets can not be updated through their controller as their template (`volumeClaimTemplates`)
  ...
  The following StatefulSets need to be deleted **in this order** AND **with `kubectl delete --cascade=orphan`** AND **immediately restored**
  with a subsequent `terragrunt apply` to their defining module:

  * The Vault StatefulSet created by `kube_vault`
  * The Redis cluster for Authentik created by `kube_authentik`
  ...
  * Any Redis clusters you have provisioned with [kube\_redis\_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)
  ...
  After completing this operation, you need to delete the backing PVCs from each module one at a time by deleting
  ...
  to take advantage of the new functionality. As long as you proceed one at a time, this will not cause any downtime.
