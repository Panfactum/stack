...
- The `persistence_enabled` option was removed from [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/kubernetes/kube_redis_sentinel).
...
- Adds [kube_scheduler](/docs/edge/reference/infrastructure-modules/kubernetes/kube_scheduler), an alternative Kubernetes
...
use the scheduler provided by [kube_scheduler](/docs/edge/reference/infrastructure-modules/kubernetes/kube_scheduler)
instead of the less-efficient EKS scheduler.

- If `panfactum_scheduler_enabled` is `true`, the [kube_descheduler](/docs/edge/reference/infrastructure-modules/kubernetes/kube_descheduler)
will automatically remove pods from low utilization nodes to allow the [kube_scheduler](/docs/edge/reference/infrastructure-modules/kubernetes/kube_scheduler)
to bin-pack them on other nodes.

### Fixed

- Addresses a bug in the previous release that left [kube_karpenter](/docs/edge/reference/infrastructure-modules/kubernetes/kube_karpenter)
not deployable.
...
- Updates [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/kubernetes/kube_pg_cluster) with many new variables
...
to the [aws_account](/docs/edge/reference/infrastructure-modules/aws/aws_account) module.

- Adds the [kube_open_cost](/docs/edge/reference/infrastructure-modules/kubernetes/kube_open_cost) module
for calculating the cost of workloads running on Kubernetes.

### Fixed

- Addressed issue in [aws_vpc](/docs/edge/reference/infrastructure-modules/aws/aws_vpc) where NAT nodes wouldn't restart
if NAT setup failed with an exit code other than `1`.

- Increased the memory floor of the Authentik server in [kube_authentik](/docs/edge/reference/infrastructure-modules/kubernetes/kube_authentik)
to avoid OOM issues.

- Updates [kube_authentik](/docs/edge/reference/infrastructure-modules/kubernetes/kube_authentik) to allow showing
[Gravatar](https://gravatar.com/) profile images.

- Updates [kube_authentik](/docs/edge/reference/infrastructure-modules/kubernetes/kube_authentik) to provide the necessary Permissions-Policy headers to allow
use of WebAuthn devices.

- Correctly applies pod labels in [kube_aws_lb_controller](/docs/edge/reference/infrastructure-modules/kubernetes/kube_aws_lb_controller).

- Removes node preferences defaults from [kube_workload_utility](/docs/edge/reference/infrastructure-modules/kubernetes/kube_workload_utility)
...
- Fixes incorrect SCIM property mapping in [authentik_aws_sso](/docs/edge/reference/infrastructure-modules/authentik/authentik_aws_sso).

- Aligns pod labels, affinities, topologySpreadConstraints, and tolerations in
[kube_linkerd](/docs/edge/reference/infrastructure-modules/kubernetes/kube_linkerd) to conventions used in all
other modules.
...
- Updates [aws_vpc](/docs/edge/reference/infrastructure-modules/aws/aws_vpc) to support new command `pf-vpc-network-test` that
...
[aws_vpc](/docs/edge/reference/infrastructure-modules/aws/aws_vpc) module.
...
- Addressed issue in [aws_vpc](/docs/edge/reference/infrastructure-modules/aws/aws_vpc) that caused a temporary, harmless error to crash
...
[kube_core_dns](/docs/edge/reference/infrastructure-modules/kubernetes/kube_core_dns) module.
