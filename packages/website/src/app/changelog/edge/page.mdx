...
**`kube_fledged` must be removed from your clusters before upgrading to the ***next*** version** as it will no longer be available in the next release.
...
**`kube_reflector` must be removed from your clusters before upgrading to the ***next*** version** as it will no longer be available in the next release.
...
* Resolved issue with [kube\_pg\_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) where `pg_cpu_millicores` could not be increased above `1000`.
