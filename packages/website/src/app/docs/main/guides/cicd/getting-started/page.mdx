...
<MarkdownAlert severity="info">
  ...
  we do offer alternative modules for [self-hosted GitHub Actions](/docs/main/guides/addons/github-actions/installing).
</MarkdownAlert>
...
We provide a complete example the IaC of a fully functional CI / CD system [here](https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_MAIN__/packages/reference/infrastructure/demo_cicd).
...
module "tf_deploy_prod_us_east_2" {
  source = "github.com/Panfactum/stack.git//packages/infrastructure/wf_tf_deploy?ref=__PANFACTUM_VERSION_MAIN__" #pf-update
...
1. Adjust the `dependencies` array of your Event Bus Sensor to match the specific webhook events that you are interested in. In the
   ...
   ```hcl
   module "sensor" {
     source = "github.com/Panfactum/stack.git//packages/infrastructure/kube_argo_sensor?ref=__PANFACTUM_VERSION_MAIN__" #pf-update  ...
     ...
     dependencies = [
       {
         name = "push-to-main"
