...
import VersioningImg from './versioning.svg'
...
Set `iac_dir` to that directory per the [reference docs](/docs/main/reference/configuration/repo-variables).
...
This takes care of common issues around versioning and caching that can often cause confusing results.
...
* The [configuration-as-code](/docs/main/concepts/iac#configuration-as-code) (e.g., module versions, module inputs, etc.) will **always** be defined
  by the primary integration branch (set by `repo_primary_branch` in `panfactum.yaml`), in this case `main`.
...
<MarkdownSVGImage src={VersioningImg} alt="Versioning diagram" />

## Writing Modules

### Standard Inputs

When deploying modules, the Panfactum Terragrunt setup will automatically provide the
[standard inputs](/docs/main/reference/infrastructure-modules/overview#standard-variables)
to your module during deployment. However, you must define [variables](https://opentofu.org/docs/language/values/variables/)
for these inputs in each module in order to be able to reference them.

We provide a [templating system](/docs/main/reference/iac-templating#standard-vars) to make maintaining these
variables easier.

1. In your `vars.tf`, add the following lines:
...
3. Notice that this block has expanded to include the standard variables. Additionally,
   if new standard variables are ever added in the future, they will be automatically added when
   you next run `pf-update-iac`.
...
When using Panfactum modules, you will want to pass in the
[standard inputs](/docs/main/reference/infrastructure-modules/overview#standard-variables). We
use [templating system](/docs/main/reference/iac-templating#standard-vars) to make maintaining
these calling conventions easier:

1. Add a [template directive](/docs/main/reference/iac-templating#directives) to the module:
...
3. Notice that this block has expanded to include the variables defined be the chosen directive. If new standard variables are ever added in the future, they will be automatically added when
   you next run `pf-update-iac`.

Additionally, you will generally want to ensure that all Panfactum modules in your ecosystem are pinned
to the same version as your development environment. We provide an annotation that will automatically
synchronize your versions:

1. Append `# pf-update` to the `module.source` line:

   ```hcl
   module "pod" {
     source       = "github.com/Panfactum/stack.git//packages/infrastructure/kube_pod?ref=__PANFACTUM_VERSION_MAIN__" # pf-update
   }
   ```

2. Run `pf-update-iac`.

3. Notice that the `ref` has been updated to the commit hash of git version that your devenv is pinned to. If you
   wish to prevent this automatic updating behavior, simply omit the `# pf-update` comment.
