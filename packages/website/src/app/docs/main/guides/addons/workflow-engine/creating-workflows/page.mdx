...
Demonstrate how to use the [wf\_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_spec)
...
[wf\_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_spec), that aims to simplify the process
...
module "workflow_spec" {
  ...
  templates = [
    ...
    },
    {
      ...
      container   = {
        image   = "some-repo/some-image:some-tag"
        command = ["/bin/some-other-command"]
      }
...
1. In the example, `module.workflow_spec` instantiates the Panfactum `wf_spec` module, but critically this module
   does **not** create a Workflow; it only creates the *specification* for a workflow.
...
2. The `templates` array defines all the nodes in the workflow's execution graph. There are many different types of
   ...
   * An [HTTP Template](https://argo-workflows.readthedocs.io/en/latest/http-template/) (for executing HTTP requests)
...
3. You might want to override one or more pod-level defaults for a template (example: provide certain tolerations for some
   pods but not others).  See the `second` template above as an example.
...
5. The `entrypoint` defines which template from the `templates` array will be initially executed when the Workflow is created.
   ...
   Our example above starts by executing the DAG template (`entry`) which in turns will run the `first` Container template
   and then the `second` Container template.
...
see the documentation for [wf\_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_spec).
...
* Can your pods run on `arm64` nodes? If so, did you set `arm_nodes_enabled` to `true`?
* Can your pods tolerate a small chance of disruption? If so, set `spot_nodes_enabled` to `true` to save significant money on workflow execution.
* Can your pods tolerate a medium chance of disruption? If so, set `disruptions_enabled` to `true` to save additional money.
* Can your pods run on [burstable (T-family) instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/burstable-performance-instances.html)? If so, set `burstable_nodes_enabled` to `true` to save additional money.
...
* Do your workflow containers need access to a common set of environment variables? If so, have you set `common_env` and/or `secrets`?
...
* You provide many [prebuilt WorkflowTemplates](/docs/main/guides/addons/workflow-engine/prebuilt-workflows) that you can examine for best practices.
