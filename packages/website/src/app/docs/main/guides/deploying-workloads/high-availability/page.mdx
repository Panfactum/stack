...
* For *all* modules that deploy workloads to Kubernetes, set `instance_type_anti_affinity_required` to `true` to prevent mass
...
## Usage in Panfactum Modules

Many [Panfactum modules](/docs/main/reference/infrastructure-modules/overview) have an `sla_target` input
that can be used to configure the module to achieve the desired SLA level.

However, not all modules can be automatically configured successfully with a single input.
