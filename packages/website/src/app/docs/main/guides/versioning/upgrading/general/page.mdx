...
1. If you use Panfactum IaC submodules in your organization's IaC modules, update the `ref` across all of your modules:

   ```diff-hcl[class="diff-highlight"]
   module "pod_template" {
   -    source       = "github.com/Panfactum/stack.git/packages/infrastructure//kube_pod?ref=old-version"
   +    source       = "github.com/Panfactum/stack.git/packages/infrastructure//kube_pod?ref=new-version"
   }
   ```

2. If you deploy Panfactum modules directly via terragrunt, update the `pf_stack_version` [terragrunt variable](/docs/edge/reference/configuration/terragrunt-variables#variables)
