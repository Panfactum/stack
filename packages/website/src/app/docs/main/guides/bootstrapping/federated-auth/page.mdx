...
<MarkdownAlert severity="info">
  ...
  * [kube\_rbac](/docs/main/reference/infrastructure-modules/kubernetes/kube_rbac) to allow SSO authentication
    with your Kubernetes clusters. Note that you MUST omit the /aws-reserved/sso.amazonaws.com/us-east-2/
    part of the ARN.

  * [aws\_eks](/docs/main/reference/infrastructure-modules/aws/aws_eks) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encrypt\_key](/docs/main/reference/infrastructure-modules/aws/aws_kms_encrypt_key)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](/docs/main/reference/infrastructure-modules/kubernetes/kube_vault) to allow SSO authentication
...
[authentik\_aws\_sso](/docs/main/reference/infrastructure-modules/authentik/authentik_aws_sso).

Let's deploy it now:

1. Add a new `authentik_aws_sso` folder adjacent to your `authentik_core_resources` folder.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/authentik_aws_sso/terragrunt.hcl).
...
to automatically set this up: [aws\_iam\_identity\_center\_permissions](/docs/main/reference/infrastructure-modules/aws/aws_iam_identity_center_permissions).

Let's deploy it now:

1. Add a new `aws_iam_identity_center_permissions` folder to **management** environment in the **global** region.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/management/global/aws_iam_identity_center_permissions/terragrunt.hcl).

3. For the `acccount_access_configuration`, we recommend setting this up according to our [RBAC recommendations](/docs/main/reference/rbac),
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](/docs/main/reference/infrastructure-modules/aws/aws_kms_encrypt_key) module
   * EKS clusters deployed with [aws\_eks](/docs/main/reference/infrastructure-modules/aws/aws_eks)
   * Vault clusters deployed with [kube\_vaults](/docs/main/reference/infrastructure-modules/kubernetes/kube_vault)
...
5. Set up `config.yaml` according to [these reference docs](/docs/main/reference/configuration/aws).

   1. We provide an example file [here.](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/.aws/config.yaml)
...
[authentik\_vault\_sso](/docs/main/reference/infrastructure-modules/authentik/authentik_vault_sso).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/authentik_vault_sso_production/terragrunt.hcl).
...
We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](/docs/main/reference/infrastructure-modules/vault/vault_auth_oidc).

Let's deploy it now:

1. For **every** Vault cluster, add a `vault_auth_oidc` folder adjacent to its `kube_vault` folder.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/vault_auth_oidc/terragrunt.hcl).
...
1. Remove your hardcoded `VAULT_ADDR` and `VAULT_TOKEN` token from your `.env` file. Make sure that you added the
   `vault_addr` in you `region.yaml` as covered in the [inbound networking section.](/docs/main/guides/bootstrapping/inbound-networking#deploy-the-vault-ingress)
