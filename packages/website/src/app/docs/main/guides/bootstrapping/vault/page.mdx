...
We will use the [kube\_vault](/docs/main/reference/infrastructure-modules/kubernetes/kube_vault) infrastructure module to deploy
...
via the [vault\_core\_resources](/docs/main/reference/infrastructure-modules/vault/vault_core_resources) module.
...
Let's now deploy [vault\_core\_resources](/docs/main/reference/infrastructure-modules/vault/vault_core_resources):

1. Create a new directory adjacent to your `kube_vault` module called `vault_core_resources`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/vault_core_resources/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/main/guides/bootstrapping/storage-interfaces"} forwardHref={"/docs/main/guides/bootstrapping/certificate-management"} stepNumber={11} />
