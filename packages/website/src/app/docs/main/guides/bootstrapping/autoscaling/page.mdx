...
We provide a module to deploy the server: [kube\_metrics\_server](/docs/main/reference/infrastructure-modules/kubernetes/kube_metrics_server).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_linkerd` module called `kube_metrics_server`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_metrics_server/terragrunt.hcl).
...
We provide a module to deploy the VPA: [kube\_vpa](/docs/main/reference/infrastructure-modules/kubernetes/kube_vpa).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_metrics_server` module called `kube_vpa`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_vpa/terragrunt.hcl).
...
6. Once the module has successfully deployed, return to all of the previously deployed kubernetes
   modules and set `vpa_enabled` to `true`:
   * `kube_aws_ebs_csi`
   * `kube_cert_manager`
   * `kube_cilium`
   * `kube_linkerd`
...
7. Navigate up to the directory containing all of these modules (this should be the parent of `kube_vpa`). Run
   `terragrunt run-all apply` to apply all of the modules at once. Note that this might cause your Vault proxy
...
8. After all these modules are updated, you should now see many VPA resources in k9s (`:vpa`). After a minute or two, they
...
9. Return to the pod view. Notice that all request columns are populated as well as the memory limit column:
   ...
   * We set **requests** for both CPU and memory so that the Kubernetes scheduler  can
     accurately decide which enough resources available for pods during the pod-to-node assignment phase.
...
Fortunately, a new provide has emerged, [Karpenter](https://karpenter.sh/), which provides both more flexibility, better
...
* You do not need to choose instance types in advance as it will query your cloud providers
  API for the *entire* list of available instance types and prices and then spin up the exact right instance types
  that are the best fit for your workloads at the lowest cost

* If you allow it, it will prioritize utilizing [spot
  instances](https://aws.amazon.com/ec2/spot/) for up to a **90% discount** over list price. Moreover, it will periodically
...
We provide an infrastructure module to deploy it: [kube\_karpenter](/docs/main/reference/infrastructure-modules/kubernetes/kube_karpenter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_vpa` module called `kube_karpenter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_karpenter/terragrunt.hcl).
...
* linux [spot nodes](https://aws.amazon.com/ec2/spot/)

  * labeled with `panfactum.com/class` set to `spot`

  * highest scheduling precedence

  * assigned a [taint](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/) to prevent workloads that cannot tolerate
    arbitrary disruption from accidentally being scheduled on one: `{ key = "spot", value = "true", effect = "NoSchedule" }`

* linux on-demand nodes

  * labeled with `panfactum.com/class` set to `worker` [^1]

[^1]: The EKS-managed nodes created by `aws_eks` have a class of `controller`.

Those are defined in the [kube\_karpenter\_node\_pools](/docs/main/reference/infrastructure-modules/kubernetes/kube_karpenter_node_pools) module.

Let's deploy it now:

1. Create a new directory adjacent to your `kube_karpenter` module called `kube_karpenter_node_pools`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_karpenter_node_pools/terragrunt.hcl).
...
6. Scale `jetstack-cert-manager` back down to `2`. Karpenter will automatically take care of the node
...
Now that Karpenter is successfully running, we can dramatically decrease the nodes in the base EKS node
pool in order to allow Karpenter to handle autoscaling more efficiently (approximately $100 / month cost
savings over the current settings).

In your `aws_eks` modules, we recommend the following changes:

- Set `controller_node_count` to `1`. [^4]
- Set `controller_node_instance_types` to `["t3a.medium"]`.

[^4]: We must keep at least one node around as Karpenter must run on a node that it itself did **not** provision.

Note that this change might cause temporary cluster disruption as the core controllers are repositioned on
Karpenter nodes.
...
<BootstrappingGuideNav backHref={"/docs/main/guides/bootstrapping/service-mesh"} forwardHref={"/docs/main/guides/bootstrapping/inbound-networking"} stepNumber={14} />
