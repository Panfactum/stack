...
3. Add a `module.yaml` that enables the `aws`, `kubernetes`, and `helm` providers.
...
5. In your `region.yaml`, set `extra_inputs.vpa_enabled` to `true` like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/region.yaml).
...
3. For `node_subnets`, we *strongly* recommend using the same subnets you used in the `aws_eks` module for the `controller_node_subnets` unless you have a specific reason not to.
...
the [kube\_karpenter\_node\_pools](/docs/main/reference/infrastructure-modules/kubernetes/kube_karpenter_node_pools) module:
...
We provide an alternative scheduler in [kube_scheduler](/docs/main/reference/infrastructure-modules/kubernetes/kube_scheduler)
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_scheduler/terragrunt.hcl).

3. Add a `module.yaml` that enables the `aws`, `kubernetes`, and `helm` providers.

4. Run `terragrunt apply`.

5. In your `region.yaml`, set `extra_inputs.panfactum_scheduler_enabled` to `true` like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/region.yaml).
