...
We provide a module to deploy this controller: [kube\_reloader](/docs/main/reference/infrastructure-modules/kubernetes/kube_reloader).

Let's deploy the Reloader now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_reloader`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_reloader/terragrunt.hcl).
...
ConfigMaps and Secrets are namespaced resources. As a result, they can only be accessed by other resources deployed
...
will cause them to be automatically synchronized across namespaces.

We provide a module to deploy this controller: [kube\_reflector](/docs/main/reference/infrastructure-modules/kubernetes/kube_reflector).

Let's deploy the Reflector now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_reflector`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_reflector/terragrunt.hcl).

3. Add a `module.yaml` that enables the `aws`, `random`, `helm`, and `kubernetes` providers.

4. Run `terragrunt apply`.

Let's test that the Reflector is working:

1. Create a new, reflected ConfigMap:
...
2. Run `kubectl get configmap -A | grep test-reflector` to verify the ConfigMap was created in all namespaces.

3. Run `kubectl -n reflector delete configmap test-reflector` to delete the original ConfigMap.

4. Re-run `kubectl get configmap -A | grep test-reflector` to verify all the reflected ConfigMaps were also deleted.
...
We provide a module for the Descheduler: [kube\_descheduler](/docs/main/reference/infrastructure-modules/kubernetes/kube_descheduler).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_reloader` module called `kube_descheduler`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_descheduler/terragrunt.hcl).
...
via the [kube\_aws\_ebs\_csi](/docs/main/reference/infrastructure-modules/kubernetes/kube_aws_ebs_csi) module we installed in
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](/docs/main/reference/infrastructure-modules/kubernetes/kube_external_snapshotter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_descheduler` module called `kube_external_snapshotter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_external_snapshotter/terragrunt.hcl).
...
We provide a module to deploy this functionality: [kube\_velero](/docs/main/reference/infrastructure-modules/kubernetes/kube_velero).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_velero/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/main/guides/bootstrapping/inbound-networking"} forwardHref={"/docs/main/guides/bootstrapping/databases"} stepNumber={16} />
