...
Fortunately, the [kube-fledged](https://github.com/senthilrch/kube-fledged) operator provides a solution to this problem.
This controller allows you to define `ImageCache` resources containing lists of images. kube-fledged will automatically ensure
the specified images are pre-pulled to every node, so that they will already be available if a pod needs the image in the future.

We provide a module for deploying this operator: [kube_fledged](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_fledged).

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_fledged`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_fledged/terragrunt.hcl).

3. Run `pf-providers-enable` to enable the required providers.

4. Run `terragrunt apply`

5. In your `region.yaml`, set `extra_inputs.node_image_cache_enabled` to `true` like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/region.yaml).
This will instruct modules that would benefit from pre-pulled images to set up `ImageCache` resources.
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/kube_pvc_autoresizer/terragrunt.hcl).
