...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_cert_issuers).
...
4. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/docs/main/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/docs/main/reference/infrastructure-modules/direct/aws/aws_registered_domains)
      and [aws\_delegated\_zones](/docs/main/reference/infrastructure-modules/direct/aws/aws_delegated_zones) modules
...
5. Run `pf-providers-enable` to enable the required providers.
...
[root CA certificate bundle.](/docs/main/concepts/networking/cryptography#certificate-authorities)
...
We provide a module to deploy this controller: [kube\_reflector](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_reflector).
