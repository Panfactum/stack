...
Deploy the core AWS networking infrastructure such as the [Virtual Private Cloud (VPC)](https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html)
that will contain all of your deployed resources.
...
When using Panfactum, you will need to choose an SLA target when deploying each environment. The configured SLA
target changes how the deployed modules behave. Higher targets will improve uptime but increase your infrastructure
costs.

We provide three different SLA targets: [^90]
...
[^90]: You can read more about achieving uptime requirements for your workloads [here](/docs/main/guides/deploying-workloads/high-availability).
...
`sla_target` in your environment's `environment.yaml`. This overrides the default SLA level for all modules
deployed in that environment. [^91]

[^91]: The `sla_target` can still be overwritten at lower scopes such as regions or even on a per-module basis. For
more information on setting Terragrunt variables, see [the reference docs](/docs/main/reference/configuration/terragrunt-variables#metadata).
...
6. Ensure that your [NAT nodes](/docs/main/concepts/networking/nat) are running and healthy:
...
<MarkdownAlert severity="warning">
    If you deviate from the default settings for the [aws\_vpc](/docs/main/reference/infrastructure-modules/direct/aws/aws_vpc) module,
