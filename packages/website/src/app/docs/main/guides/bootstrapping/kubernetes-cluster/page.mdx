...
<MarkdownAlert severity="warning">
    Once you [deploy autoscaling](/docs/main/guides/bootstrapping/autoscaling), you can reduce
    the size and count of the EKS nodes recommended below, but until then, using less than our recommendations
    can introduce issues in later guide sections.
</MarkdownAlert>

The nodes created by this module are used to run cluster-critical controllers, without which your cluster will not
function properly. [^5] You **must** use at least **three** nodes in order to properly launch and update
all cluster components. The `t3a.large` is the minimum recommended instance size if using three nodes.
...
like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_MAIN__/packages/reference/environments/production/us-east-2/region.yaml). This can save about $50 / month.
