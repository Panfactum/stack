...
Unfortunately, AWS install various utilities to every new EKS cluster such as `coredns` and `kube-proxy` which
will conflict with our resources in later guide steps.

We need to reset the cluster to a clean state before we continue.

We provide a convenience script `pf-eks-reset` to perform the reset. Run this script now.

Note that this will temporarily prevent EC2 nodes from joining the cluster until
the `kube_rbac` module is deployed (below).
