...
These resources are considered non-essential during periods when the cluster is not actively in use and can be safely suspended to reduce AWS charges by ~$3 a day.

<MarkdownAlert severity="info">
  Warning: You are about to suspend the cluster. Please note that the cluster's certificates will expire in 90 days. If the cluster is not resumed before then, the certificates will not be renewed automatically, and the cluster may require manual intervention to recover.
...
### Suspend Resources (`pf-eks-suspend`)

To suspend the AWS resources:

1. Run the `pf-eks-suspend` script.
...
### Resume Resources (`pf-eks-resume`)

To resume the suspended resources:

1. Run the `pf-eks-resume` script.
