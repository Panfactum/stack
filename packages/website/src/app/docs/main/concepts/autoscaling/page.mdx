...
import ScheduleConstraintImg from "./schedule-constraint.svg"
...
* Karpenter will not replace [spot nodes](https://aws.amazon.com/ec2/spot/) with cheaper spot nodes of similar size
  ...
  This can run into issues when using smaller instance types were 15 cheaper instances will not ever exist. As a result,
...
* EKS node groups are not automatically scaled. As a result, we recommend running only a single `t4g.medium` node in the node group. This is
