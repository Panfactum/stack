...
In a traditional datacenter setup, firewall rules are implemented in a centralized system hardware such as routers and L3 switches.
In Kubernetes, there is no centralized router. Instead, routing rules are distributed to
every individual node through the CNI plugins you have installed. This not only improves the efficiency of the network traffic
but also allows for strong security to be maintained even in more diverse deployment scenarios.

Despite this architectural difference, setting up firewall rules is a similar process to what you might find in a traditional
network setup. However,
...
* In a traditional system, firewall rules can only be implemented for traffic that crosses network boundaries. [^41]
...
[^41]: Due to [ARP](https://en.wikipedia.org/wiki/Address_Resolution_Protocol), traffic in the same subnet / VLAN
    does not need to be routed through a router or L3 switch and thus cannot have firewall rules applied.
