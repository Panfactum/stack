...
1. Add the `# pf-generate: <directive>` and `# end-generate` blocks to your `.tf` code:
   ...
   ```hcl
   module "kube_labels" {
     source = "github.com/Panfactum/stack.git//packages/infrastructure/kube_workload_utility?ref=__PANFACTUM_VERSION_MAIN__"
...
[standard input variables](/docs/main/reference/infrastructure-modules/overview#standard-variables) that are automatically
...
module "workflow_perms" {
  source = "github.com/Panfactum/stack.git//packages/infrastructure/kube_sa_auth_workflow?ref=__PANFACTUM_VERSION_MAIN__"
...
We recommend this for [kube\_workload\_utility](/docs/main/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility)
and [aws\_tags](/docs/main/reference/infrastructure-modules/submodule/aws/aws_tags).
