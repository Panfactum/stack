import ModuleHeader from "../../ModuleHeader";

{/* lint disable no-duplicate-headings */}

# AWS Load Balancer Controller

<ModuleHeader name="kube_aws_lb_controller" sourceHref="https://github.com/Panfactum/stack/tree/main/packages/infrastructure/kube_aws_lb_controller" status="stable" type="live" />

[Documentation](https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.7/)

## Providers

The following providers are needed by this module:

* [aws](https://registry.terraform.io/providers/hashicorp/aws/5.39.1/docs) (5.39.1)

* [helm](https://registry.terraform.io/providers/hashicorp/helm/2.12.1/docs) (2.12.1)

* [kubernetes](https://registry.terraform.io/providers/hashicorp/kubernetes/2.27.0/docs) (2.27.0)

## Required Inputs

The following input variables are required:

### <a name="input_eks_cluster_name" /> [eks\_cluster\_name](#input_eks_cluster_name)

Description: The name of the EKS cluster that this is being deployed to

Type: `string`

### <a name="input_subnets" /> [subnets](#input_subnets)

Description: List of subnet names to deploy load balancers into. Must be in at least two different availability zones.

Type: `set(string)`

### <a name="input_vpc_id" /> [vpc\_id](#input_vpc_id)

Description: The ID of the VPC to use for AWS networked resources

Type: `string`

## Optional Inputs

The following input variables are optional (have default values):

### <a name="input_alb_controller_helm_version" /> [alb\_controller\_helm\_version](#input_alb_controller_helm_version)

Description: The version of aws-application-loadbalancer-controller helm chart to deploy

Type: `string`

Default: `"1.7.2"`

### <a name="input_aws_iam_ip_allow_list" /> [aws\_iam\_ip\_allow\_list](#input_aws_iam_ip_allow_list)

Description: A list of IPs that can use the service account token to authenticate with AWS API

Type: `list(string)`

Default: `[]`

### <a name="input_log_level" /> [log\_level](#input_log_level)

Description: The log level for the ALB controller pods

Type: `string`

Default: `"warning"`

### <a name="input_pull_through_cache_enabled" /> [pull\_through\_cache\_enabled](#input_pull_through_cache_enabled)

Description: Whether to use the ECR pull through cache for the deployed images

Type: `bool`

Default: `false`

### <a name="input_vpa_enabled" /> [vpa\_enabled](#input_vpa_enabled)

Description: Whether the VPA resources should be enabled

Type: `bool`

Default: `false`

## Outputs

No outputs.

## Usage

No notes

{/* lint enable no-duplicate-headings */}
