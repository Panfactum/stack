import ModuleHeader from "../../ModuleHeader";

{/* lint disable no-duplicate-headings */}

# Kubernetes Metrics Server

<ModuleHeader name="kube_metrics_server" sourceHref="https://github.com/Panfactum/stack/tree/main/packages/infrastructure/kube_metrics_server" status="stable" type="live" />

This module provides a deployment of the [metrics-server](https://github.com/kubernetes-sigs/external-dns) project.

## Providers

The following providers are needed by this module:

* [aws](https://registry.terraform.io/providers/hashicorp/aws/5.39.1/docs) (5.39.1)

* [helm](https://registry.terraform.io/providers/hashicorp/helm/2.12.1/docs) (2.12.1)

* [kubernetes](https://registry.terraform.io/providers/hashicorp/kubernetes/2.27.0/docs) (2.27.0)

## Required Inputs

No required inputs.

## Optional Inputs

The following input variables are optional (have default values):

### <a name="input_log_verbosity" /> [log\_verbosity](#input_log_verbosity)

Description: The log verbosity (0-9) for the metrics-server pods

Type: `number`

Default: `0`

### <a name="input_metrics_server_helm_version" /> [metrics\_server\_helm\_version](#input_metrics_server_helm_version)

Description: The version of the metrics-server helm chart to deploy

Type: `string`

Default: `"3.12.0"`

### <a name="input_pull_through_cache_enabled" /> [pull\_through\_cache\_enabled](#input_pull_through_cache_enabled)

Description: Whether to use the ECR pull through cache for the deployed images

Type: `bool`

Default: `false`

### <a name="input_vpa_enabled" /> [vpa\_enabled](#input_vpa_enabled)

Description: Whether the VPA resources should be enabled

Type: `bool`

Default: `false`

## Outputs

No outputs.

## Usage

No notes

{/* lint enable no-duplicate-headings */}
