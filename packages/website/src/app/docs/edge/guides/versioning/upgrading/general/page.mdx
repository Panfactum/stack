...
1. Update the provider versions in your first-party IaC modules to match the
   [required provider versions in the new release](/docs/edge/reference/infrastructure-modules/overview#provider-versions):
...
3. Ensure that the terragrunt `version` for that environment is set to `local` in order
   to deploy the local changes from step 1 (review [these docs](/docs/edge/guides/iac/first-party-modules)). [^2]

4. Run `pf-tf-init` in the environment's Terragrunt directory to automatically upgrade your `module.yaml` and `.terraform.lock.hcl` files
   for every module.
