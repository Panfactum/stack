...
<MarkdownAlert severity="info">
  ...
  BuildKit [architecture documentation](/docs/edge/concepts/buildkit#architecture) to gain a better understanding
...
`buildctl` will use the credentials at `$DOCKER_CONFIG/config.json`. We set up devenv to use a repo-local file at `$PF_BUILDKIT_DIR/config.json`,
