...
Argo comes with a companion CLI (`argo`) that we bundle in the Panfactum devenv. This CLI tool uses your kubeconfig
...
<MarkdownAlert severity="warning">
  The Panfactum [kube\_argo](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_argo) does not create an Ingress for the Argo Workflow Server, so the HTTP API is only available
...
[kube\_sa\_auth\_workflow](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_sa_auth_workflow)
...
Using a Workflow to create other Workflows is a powerful pattern that allows you to chain workflows together
into larger execution graphs. Argo refers to this pattern as a [Workflow of Workflows](https://argo-workflows.readthedocs.io/en/latest/workflow-of-workflows/).

Below is an example WorkflowTemplate for a Workflow of Workflows that executes the above example Workflow twice in a series:
...
resources it has created (the `example` Workflows) have succeeded or failed.
