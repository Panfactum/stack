...
configuration to your repository. Specifically, you must create a `$PF_BUILDKIT_DIR/config.yaml`
([reference docs](/docs/edge/reference/configuration/buildkit)).

Let's do this now:

1. You should already have a `$PF_BUILDKIT_DIR` directory in your repository (defaults to `.buildkit`). In it, you
...
3. Update `bastion` to the name of the bastion deployed in the Kubernetes cluster running BuildKit. See your `$PF_SSH_DIR/config.yaml`.
...
5. You can now pull and run this image locally as well: `podman run --rm -it 891377197483.dkr.ecr.us-east-2.amazonaws.com/test:test`.
   ...
   If you run this command from inside the repository, our devenv will automatically take care of registry authentication for you.
