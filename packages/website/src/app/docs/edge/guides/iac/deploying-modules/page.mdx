...
(set by the `$PF_REPO_PRIMARY_BRANCH` [devenv variable](/docs/edge/reference/configuration/repo-variables)) are the values
...
We provide a convenience command, `pf-providers-enable`, that will automatically inspect the sourced
infrastructure module and add the correct values to the `providers` array. [^3]
