...
See these [concept docs](/docs/edge/concepts/autoscaling) for more information.
...
be enabled **only** in production environments. See these [concept docs](/docs/edge/concepts/autoscaling) for more information.
...
* Spot interruptions (if using `spot_nodes_enabled`) or other node shutdown; pods on the node will get approximately two minutes to terminate gracefully.
...
to received requests or closing database connections. If the pod isn't shutdown after a short delay,
