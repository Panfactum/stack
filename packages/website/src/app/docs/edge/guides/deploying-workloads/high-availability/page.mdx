...
Panfactum modules will adjust their default behavior based on the `sla_target` [Terragrunt variable.](/docs/edge/reference/configuration/terragrunt-variables#metadata) [^1] [^80]
That said, there are a handful of actions you must take to achieve any target _in addition to_ setting the `sla_target` variable. We detail those tuning requirements below.
...
* Set `sla_target` to `1`.
* [Set up liveness probes](/docs/edge/guides/deploying-workloads/basics#liveness-and-readiness-probes) for your workloads to ensure that they will be restarted if they enter a failure state.
