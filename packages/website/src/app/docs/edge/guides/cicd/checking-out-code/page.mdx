...
We provide a utility command in the devenv called `pf-wf-git-checkout` that
...
You can use the Panfactum devenv and scripts like this in your workflows by following
[this guide section](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec#using-the-panfactum-devenv).
...
You can find a full example of this in action [here.](https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_EDGE__/packages/infrastructure/wf_dockerfile_build)

<MarkdownAlert severity="warning">
  Note that this performs a shallow clone, so if you need to interact with commits
  other than what is supplied by `--checkout`, you will need to explicitly fetch them
  first.
</MarkdownAlert>
