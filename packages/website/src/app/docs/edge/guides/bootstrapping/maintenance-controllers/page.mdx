...
We provide a module to deploy this controller: [kube\_reloader](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_reloader).
...
We provide a module for deploying this operator: [kube_fledged](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_fledged).

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_fledged`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/kube_fledged/terragrunt.hcl).

3. Run `pf-providers-enable` to enable the required providers.

4. Run `terragrunt apply`

5. In your `region.yaml`, set `extra_inputs.node_image_cache_enabled` to `true` like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/region.yaml).
...
We provide a module to deploy it: [kube\_pvc\_autoresizer](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_pvc_autoresizer).
...
We provide a module for the Descheduler: [kube\_descheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_descheduler).
...
via the [kube\_aws\_ebs\_csi](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_aws_ebs_csi) module we installed in
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_snapshotter).
...
We provide a module to deploy this functionality: [kube\_velero](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_velero).
