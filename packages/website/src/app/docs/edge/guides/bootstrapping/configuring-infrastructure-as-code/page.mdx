...
<MarkdownAlert severity="warning">
  ...
  [concepts documentation](/docs/edge/concepts/iac) before continuing.
</MarkdownAlert>
...
Pick the branch you want to use and make sure it is set as the `PF_REPO_PRIMARY_BRANCH` (per the [docs](/docs/edge/reference/configuration/repo-variables)).
...
Pick the folder you want to use and make sure it is set as the `PF_ENVIRONMENTS_DIR` (per the [docs](/docs/edge/reference/configuration/repo-variables)).
...
* They can each contain *any and every key* found in this [reference document](/docs/edge/reference/configuration/terragrunt-variables).
...
* You have a top-level directory for **every** environment we created in the previous [Preparing AWS](/docs/edge/guides/bootstrapping/preparing-aws) guide.
...
For more information about releases and support, see our [guide to releases.](/docs/edge/guides/versioning/releases)
...
* `aws_secondary_region`: Set this to a *different* region than `aws_region` from above. This value is primarily
  used to create regional backups, so set this to the region where you want your backups to live. [^2]
...
<BootstrappingGuideNav backHref={"/docs/edge/guides/bootstrapping/preparing-aws"} forwardHref={"/docs/edge/guides/bootstrapping/infrastructure-as-code"} stepNumber={4} />
