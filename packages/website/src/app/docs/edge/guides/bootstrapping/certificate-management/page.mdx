...
We provide a module for deploying trust-manager: [kube\_trust\_manager](/docs/edge/reference/infrastructure-modules/kubernetes/kube_trust_manager).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cert_manager` module called `kube_trust_manager`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/kube_trust_manager/terragrunt.hcl).
