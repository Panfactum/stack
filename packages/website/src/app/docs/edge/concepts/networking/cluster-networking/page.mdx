...
import bastionSVG from './bastion.svg'
import ingressSVG from './ingress.svg'
import serviceMeshSVG from './service-mesh.svg'
import serviceSVG from './services.svg'
...
The Panfactum Stack's Cilium deployment ([kube\_cilium](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium)) has a few enhancements over the default Cilium install:
...
In the Panfactum Stack, we use the Ingress-NGINX Controller ([kube\_ingress\_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx))
...
We provide a submodule for defining Ingress manifests for our Ingress-NGINX controller: [kube\_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress).
...
We provide a module for deploying CoreDNS to Panfactum Stack clusters, [kube\_core\_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns),
...
In the Panfactum Stack, we provide the [kube\_external\_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
...
[kube\_bastion](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion).
...
devenv provides a convenience command, `pf-tunnel`, that will automatically create the necessary SSH certificates
...
Stack via [kube\_cert\_manager](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
<MarkdownAlert severity="warning">
  ...
  [bootstrapping guide.](/docs/edge/guides/bootstrapping/dns)
</MarkdownAlert>
