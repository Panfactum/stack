...
We provide both a module to deploy Authentik ([kube\_authentik](/docs/reference/infrastructure-modules/kubernetes/kube_authentik)) and
many modules to connect Authentik to the various systems in the Panfactum stack (e.g., [authentik\_aws\_sso](/docs/reference/infrastructure-modules/authentik/authentik_aws_sso)).
...
We provide a module to do this: [aws\_ses\_domain](/docs/reference/infrastructure-modules/aws/aws_ses_domain).
...
7. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record
   ...
   to set this up. You can use the [aws\_dns\_records](/docs/reference/infrastructure-modules/aws/aws_dns_records) module to
...
branding: [authentik\_core\_resources](/docs/reference/infrastructure-modules/authentik/authentik_core_resources).
...
RBAC reference](/docs/reference/rbac) for more information.
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/authentik_core_resources/terragrunt.hcl).
   ...
   5. `default_groups_enabled` will provision the [default Panfactum RBAC groups](/docs/reference/rbac) (`superusers` will always be deployed).
...
<BootstrappingGuideNav backHref={"/docs/guides/bootstrapping/databases"} forwardHref={"/docs/guides/bootstrapping/federated-auth"} stepNumber={18} />
