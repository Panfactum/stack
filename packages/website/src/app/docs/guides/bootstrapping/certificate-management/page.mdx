...
our [concept docs on network cryptography](/docs/concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/docs/reference/infrastructure-modules/kubernetes/kube_cert_manager).
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/docs/reference/infrastructure-modules/kubernetes/kube_cert_issuers).
...
4. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/docs/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/docs/reference/infrastructure-modules/aws/aws_registered_domains)
      and [aws\_delegated\_zones](/docs/reference/infrastructure-modules/aws/aws_delegated_zones) modules
...
We provide a module for deploying trust-manager: [kube\_trust\_manager](/docs/reference/infrastructure-modules/kubernetes/kube_trust_manager).
...
<BootstrappingGuideNav backHref={"/docs/guides/bootstrapping/vault"} forwardHref={"/docs/guides/bootstrapping/service-mesh"} stepNumber={12} />
