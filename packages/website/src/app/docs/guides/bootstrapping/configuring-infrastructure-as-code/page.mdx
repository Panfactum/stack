...
<MarkdownAlert severity="warning">
  ...
  [concepts documentation](/docs/concepts/iac) before continuing.
</MarkdownAlert>

The Panfactum stack uses the open-source Terraform fork, [OpenTofu](https://opentofu.org/), to define the [100+
infrastructure modules](/docs/reference/infrastructure-modules/overview) that comprise the Panfactum stack. [^1] We
use a [configuration-as-code](/docs/concepts/iac#configuration-as-code) manager for Terraform / OpenTofu called [Terragrunt](https://terragrunt.gruntwork.io/)
...
Pick the branch you want to use and make sure it is set as the `PF_REPO_PRIMARY_BRANCH` (per the [docs](/docs/reference/configuration/repo-variables)).
...
Pick the folder you want to use and make sure it is set as the `PF_ENVIRONMENTS_DIR` (per the [docs](/docs/reference/configuration/repo-variables)).
...
* They can each contain *any and every key* found in this [reference document](/docs/reference/configuration/terragrunt-variables).
...
* You have a top-level directory for **every** environment we created in the previous [Preparing AWS](/docs/guides/bootstrapping/preparing-aws) guide.
...
* `pf_stack_version`: Check [here](/docs/reference/releases) for the full list of available releases. This should
...
For more information about releases and support, see our [guide to releases.](/docs/guides/versioning/releases)
...
<BootstrappingGuideNav backHref={"/docs/guides/bootstrapping/preparing-aws"} forwardHref={"/docs/guides/bootstrapping/infrastructure-as-code"} stepNumber={4} />
