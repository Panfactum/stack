...
We provide a module to deploy the operator: [kube\_cloudnative\_pg](/docs/reference/infrastructure-modules/kubernetes/kube_cloudnative_pg).

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_cloudnative_pg`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_cloudnative_pg/terragrunt.hcl).
...
We provide a module [kube\_pg\_cluster](/docs/reference/infrastructure-modules/kubernetes/kube_pg_cluster) that you can use in your
projects to do this.
...
module called [test\_kube\_pg\_cluster](/docs/reference/infrastructure-modules/kubernetes/kube_pg_cluster) which is a thin
wrapper around [kube\_pg\_cluster](/docs/reference/infrastructure-modules/kubernetes/kube_pg_cluster).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cloudnative_pg` module called `test_kube_pg_cluster`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/test_kube_pg_cluster/terragrunt.hcl).
...
3. You should eventually receive an output like the following:
   ...
   in [inbound networking section](/docs/guides/bootstrapping/inbound-networking).
...
manner. Instead, you can run our [kube\_redis\_sentinel](/docs/reference/infrastructure-modules/kubernetes/kube_redis_sentinel)
directly.
...
<BootstrappingGuideNav backHref={"/docs/guides/bootstrapping/maintenance-controllers"} forwardHref={"/docs/guides/bootstrapping/identity-provider"} stepNumber={17} />
