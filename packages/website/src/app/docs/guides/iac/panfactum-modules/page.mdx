...
You can find the full list [here](/docs/reference/infrastructure-modules/overview).

All source code for the modules are found [here](https://github.com/Panfactum/stack/tree/__currentPanfactumVersion__/packages/infrastructure).

You can directly deploy these modules via your terragrunt configuration code.

## Sourcing via Terragrunt

We use the following [terragrunt variables](/docs/reference/configuration/terragrunt-variables), to automatically configure your `terraform.source`.
...
infrastructure modules](/docs/guides/iac/first-party-modules).
...
```hcl
module "pod_template" {
  source       = "github.com/Panfactum/stack.git/packages/infrastructure//kube_pod?ref=__currentPanfactumVersion__"
