...
our [concept docs on network cryptography](/docs/24-05/concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube\_cert\_manager](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_cert_manager).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_vault` module called `kube_cert_manager`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_cert_manager/terragrunt.hcl).

3. Set `vpa_enabled` to `false`. We will enable it when we deploy the autoscalers.

4. Set `self_generated_certs_enabled` to `true`. We will disable insecure self-signed certificates
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_cert_issuers).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_cert_issuers/terragrunt.hcl).
...
4. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](/docs/24-05/guides/bootstrapping/dns), we configured DNS in the [aws\_registered\_domains](/docs/24-05/reference/infrastructure-modules/aws/aws_registered_domains)
      and [aws\_delegated\_zones](/docs/24-05/reference/infrastructure-modules/aws/aws_delegated_zones) modules
...
We provide a module for deploying trust-manager: [kube\_trust\_manager](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_trust_manager).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cert_manager` module called `kube_trust_manager`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_trust_manager/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/vault"} forwardHref={"/docs/24-05/guides/bootstrapping/service-mesh"} stepNumber={12} />
