...
Install the basic kubernetes cluster networking primitives via the [kube\_cilium](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_cilium)
module.
...
we recommend reviewing the [concept documentation](/docs/24-05/concepts/networking/cluster-networking) for more
information.
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_cilium/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/kubernetes-cluster"} forwardHref={"/docs/24-05/guides/bootstrapping/storage-interfaces"} stepNumber={9} />
