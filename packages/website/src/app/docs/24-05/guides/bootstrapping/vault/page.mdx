...
We will use the [kube\_vault](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_vault) infrastructure module to deploy
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_vault/terragrunt.hcl).
...
7. Note that this deployment may or may not succeed, but you should see three vault instances which are in the unready state:
...
via the [vault\_core\_resources](/docs/24-05/reference/infrastructure-modules/vault/vault_core_resources) module.
...
Let's now deploy [vault\_core\_resources](/docs/24-05/reference/infrastructure-modules/vault/vault_core_resources):

1. Create a new directory adjacent to your `kube_vault` module called `vault_core_resources`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/vault_core_resources/terragrunt.hcl).
...
in the [prior section](/docs/24-05/guides/bootstrapping/storage-interfaces).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/storage-interfaces"} forwardHref={"/docs/24-05/guides/bootstrapping/certificate-management"} stepNumber={11} />
