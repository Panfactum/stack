...
We provide a module to deploy it: [kube\_external\_dns](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_external_dns)

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_external_dns`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_external_dns/terragrunt.hcl).

3. The syntax for `route53_zones` is the same as it was for `kube_cert_issuers`. You may reference [that guide](/docs/24-05/guides/bootstrapping/certificate-management#deploy-issuers)
...
We provide a module to deploy it: [kube\_aws\_lb\_controller](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_aws_lb_controller)

Let's deploy it now:

1. Create a new directory adjacent to your `kube_external_dns` module called `kube_aws_lb_controller`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_aws_lb_controller/terragrunt.hcl).
...
We provide a module to deploy it: [kube\_ingress\_nginx](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_ingress_nginx).

### Deploy the NGINX-Ingress Controller

1. Create a new directory adjacent to your `kube_external_dns` module called `kube_ingress_nginx`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_ingress_nginx/terragrunt.hcl).
...
1. Return the `kube_vault` module you deployed in the [Vault guide section](/docs/24-05/guides/bootstrapping/vault).
...
7. Moreover, notice that NGINX properly secures the site in a standard way by setting security headers for the browser. You
   ...
   This is accomplished by our [kube\_ingress](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_ingress)
...
We provide a bastion deployment module: [kube\_bastion](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_bastion).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_bastion/terragrunt.hcl).
...
4. Update the values to the correct values for your setup. See the [reference docs](/docs/24-05/reference/configuration/ssh) for
   more information.
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/autoscaling"} forwardHref={"/docs/24-05/guides/bootstrapping/maintenance-controllers"} stepNumber={15} />
