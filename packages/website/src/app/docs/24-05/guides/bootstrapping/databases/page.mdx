...
We provide a module to deploy the operator: [kube\_cloudnative\_pg](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_cloudnative_pg).

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_cloudnative_pg`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_cloudnative_pg/terragrunt.hcl).
...
We provide a module [kube\_pg\_cluster](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_pg_cluster) that you can use in your
projects to do this.
...
module called [test\_kube\_pg\_cluster](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_pg_cluster) which is a thin
wrapper around [kube\_pg\_cluster](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_pg_cluster).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cloudnative_pg` module called `test_kube_pg_cluster`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/test_kube_pg_cluster/terragrunt.hcl).
...
6. Switch to the pods view (`:pods`) and notice several new pods will come up after approximately 2-3 minutes:
   ...
   * Two different sets of "pooler" pods are launched. These are [PgBouncer](https://www.pgbouncer.org/) instances,
     one running in read-write mode and the other in read-only. You should establish database connections to these
...
3. You should eventually receive an output like the following:
   ...
   in [inbound networking section](/docs/24-05/guides/bootstrapping/inbound-networking).
...
manner. Instead, you can run our [kube\_redis\_sentinel](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_redis_sentinel)
directly.
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/maintenance-controllers"} forwardHref={"/docs/24-05/guides/bootstrapping/identity-provider"} stepNumber={17} />
