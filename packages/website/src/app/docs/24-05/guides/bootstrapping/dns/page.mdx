...
[aws\_registered\_domains](/docs/24-05/reference/infrastructure-modules/aws/aws_registered_domains) module. [^1]
...
3. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/global/aws_registered_domains/terragrunt.hcl).

4. Enable the `aws` and `time` providers in your `module.yaml`.
...
[^1]: The domains will have the following defaults you can override if desired:
    ...
    refer to the [module documentation](/docs/24-05/reference/infrastructure-modules/aws/aws_registered_domains).
...
This is accomplished via a concept called [subdomain delegation](/docs/24-05/concepts/networking/subdomain-delegation). Panfactum's
[aws\_delegated\_zones](/docs/24-05/reference/infrastructure-modules/aws/aws_delegated_zones) will provide the relevant setup
for this to work.
...
3. Create a `terragrunt.hcl` that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/global/aws_delegated_zones_production/terragrunt.hcl). [^4]
   Use the appropriate `subdomain_identifiers` for your environment.

4. Create a `module.yaml`.

   1. Enable the `aws` and `time` providers.
   ...
   2. Set the `aws_account_id` and `aws_profile` to the appropriate values for the environment. Note that this module deploys
      ...
      and the secondary `aws` provider to deploy the appropriate DNS records in the `production` environment.
...
If you have static records to deploy, you can use our [aws\_dns\_records](/docs/24-05/reference/infrastructure-modules/aws/aws_dns_records)
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/infrastructure-as-code"} forwardHref={"/docs/24-05/guides/bootstrapping/aws-networking"} stepNumber={6} />
