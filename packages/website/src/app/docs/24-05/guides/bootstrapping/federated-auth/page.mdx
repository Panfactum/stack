...
<MarkdownAlert severity="info">
  ...
  * [kube\_rbac](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_rbac) to allow SSO authentication
    with your Kubernetes clusters. Note that you MUST omit the /aws-reserved/sso.amazonaws.com/us-east-2/
    part of the ARN.

  * [aws\_eks](/docs/24-05/reference/infrastructure-modules/aws/aws_eks) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encrypt\_key](/docs/24-05/reference/infrastructure-modules/aws/aws_kms_encrypt_key)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_vault) to allow SSO authentication
...
1. Log into the AWS web console of your management account.
...
[authentik\_aws\_sso](/docs/24-05/reference/infrastructure-modules/authentik/authentik_aws_sso).

Let's deploy it now:

1. Add a new `authentik_aws_sso` folder adjacent to your `authentik_core_resources` folder.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/authentik_aws_sso/terragrunt.hcl).
...
3. Enable the `authentik`, `aws`, `kubernetes`, `random`, and `tls` providers in your `module.yaml`.
...
to automatically set this up: [aws\_iam\_identity\_center\_permissions](/docs/24-05/reference/infrastructure-modules/aws/aws_iam_identity_center_permissions).

Let's deploy it now:

1. Add a new `aws_iam_identity_center_permissions` folder to **management** environment in the **global** region.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/management/global/aws_iam_identity_center_permissions/terragrunt.hcl).

3. For the `acccount_access_configuration`, we recommend setting this up according to our [RBAC recommendations](/docs/24-05/reference/rbac),
   but you have the flexibility to choose what works best for your organization. The "groups" are groups you have created
   in Authentik and have allowed access to AWS in the `authentik_aws_sso` module above.

4. Enable the `aws` provider `module.yaml`.
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](/docs/24-05/reference/infrastructure-modules/aws/aws_kms_encrypt_key) module
   * EKS clusters deployed with [aws\_eks](/docs/24-05/reference/infrastructure-modules/aws/aws_eks)
   * Vault clusters deployed with [kube\_vaults](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_vault)

3. Run `pf-update-aws` to scaffold files in your `$PF_AWS_DIR` directory.

4. Copy the generated `config.example.yaml` file to `config.yaml`.

5. Set up `config.yaml` according to [these reference docs](/docs/24-05/reference/configuration/aws).

   1. We provide an example file [here.](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/.aws/config.yaml)
...
6. Run `pf-update-aws --build`. This will automatically generate your `$PF_AWS_DIR/config` file to support
...
8. Ensure that you adjust your `$PF_KUBE_DIR/config.user.yaml` and terragrunt variables to reflect the
   ...
   access. See [this guide](/docs/24-05/guides/iac/deploying-modules#local-deployments) for more information.

9. Verify that you still have Kubernetes connectivity by running `kubectl cluster-info` for every cluster
   (after running `pf-update-kube` to reflect any changes to your `$PF_KUBE_DIR/config.user.yaml`).

10. Let's finally de-provision your static root user credentials.

    1. Delete `$PF_AWS_DIR/credentials` which contains your static credentials.
...
<MarkdownAlert severity="warning">
  You will only need to complete this section for EVERY Vault cluster you have deployed.
</MarkdownAlert>

### Deploy Vault Provider in Authentik

We have a module for configuring Authentik to serve as the IdP for Vault clusters:
[authentik\_vault\_sso](/docs/24-05/reference/infrastructure-modules/authentik/authentik_vault_sso).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/authentik_vault_sso_production/terragrunt.hcl).
...
3. Enable the `authentik`, `aws`, `kubernetes`, `random`, and `tls` providers in your `module.yaml`. Additionally,
   set the `module` to `authentik_vault_sso`.

4. Run `terragrunt apply`.

### Deploy Vault OIDC Login Configuration

We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](/docs/24-05/reference/infrastructure-modules/vault/vault_auth_oidc).

Let's deploy it now:

1. For **every** Vault cluster, add a `vault_auth_oidc` folder adjacent to its `kube_vault` folder.

2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/vault_auth_oidc/terragrunt.hcl).
...
4. Enable the `vault` provider in your `module.yaml`.
...
1. Remove your hardcoded `VAULT_ADDR` and `VAULT_TOKEN` token from your `.env` file. Make sure that you added the
   `vault_addr` in you `region.yaml` as covered in the [inbound networking section.](/docs/24-05/guides/bootstrapping/inbound-networking#deploy-the-vault-ingress)
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/identity-provider"} forwardHref={"/docs/24-05/guides/bootstrapping/next-steps"} stepNumber={19} />
