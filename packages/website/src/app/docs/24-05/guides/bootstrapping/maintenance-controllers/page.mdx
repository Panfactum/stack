...
We provide a module to deploy this controller: [kube\_reloader](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_reloader).

Let's deploy the Reloader now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_reloader`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_reloader/terragrunt.hcl).
...
We provide a module for the Descheduler: [kube\_descheduler](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_descheduler).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_reloader` module called `kube_descheduler`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_descheduler/terragrunt.hcl).
...
via the [kube\_aws\_ebs\_csi](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_aws_ebs_csi) module we installed in
[a prior section](/docs/24-05/guides/bootstrapping/storage-interfaces).
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_external_snapshotter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_descheduler` module called `kube_external_snapshotter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_external_snapshotter/terragrunt.hcl).
...
We provide a module to deploy this functionality: [kube\_velero](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_velero).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_velero/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/inbound-networking"} forwardHref={"/docs/24-05/guides/bootstrapping/databases"} stepNumber={16} />
