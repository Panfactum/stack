...
We provide a module to deploy the server: [kube\_metrics\_server](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_metrics_server).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_linkerd` module called `kube_metrics_server`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_metrics_server/terragrunt.hcl).
...
We provide a module to deploy the VPA: [kube\_vpa](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_vpa).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_metrics_server` module called `kube_vpa`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_vpa/terragrunt.hcl).
...
We provide an infrastructure module to deploy it: [kube\_karpenter](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_karpenter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_vpa` module called `kube_karpenter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_karpenter/terragrunt.hcl).
...
Those are defined in the [kube\_karpenter\_node\_pools](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_karpenter_node_pools) module.

Let's deploy it now:

1. Create a new directory adjacent to your `kube_karpenter` module called `kube_karpenter_node_pools`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_karpenter_node_pools/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/service-mesh"} forwardHref={"/docs/24-05/guides/bootstrapping/inbound-networking"} stepNumber={14} />
