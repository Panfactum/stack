...
We provide the [kube\_aws\_ebs\_csi](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_aws_ebs_csi) module to deploy the driver.

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_aws_ebs_csi`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_aws_ebs_csi/terragrunt.hcl).
...
We provide the [kube\_secrets\_csi](/docs/24-05/reference/infrastructure-modules/kubernetes/kube_secrets_csi) module to deploy the driver.

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_secrets_csi`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_24_05__/packages/reference/environments/production/us-east-2/kube_secrets_csi/terragrunt.hcl).
...
<BootstrappingGuideNav backHref={"/docs/24-05/guides/bootstrapping/internal-cluster-networking"} forwardHref={"/docs/24-05/guides/bootstrapping/vault"} stepNumber={10} />
