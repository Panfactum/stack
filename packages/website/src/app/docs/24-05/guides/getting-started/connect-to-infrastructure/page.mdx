...
[^1]: The IdP itself should have already been provisioned in the [bootstrapping guide.](/docs/24-05/guides/bootstrapping/identity-provider)

If your organization's administrators have not already [provisioned your account](/docs/24-05/guides/user-management/provisioning-new-user), ask them now. You
will need to follow [this guide](/docs/24-05/guides/user-management/setting-up-new-user) to set up your user for the first time.
...
These accounts and roles are already available for use inside your local repository. They are configured under the `$PF_AWS_DIR` directory (defaults to `.aws`).
...
[^3]: You can see what accounts and regions your clusters are deployed to by examining the `$PF_ENVIRONMENTS_DIR` folder in your
...
1. Copy the `config.user.example.yaml` file in your `$PF_KUBE_DIR` folder (defaults to `.kube`) to `config.user.yaml`.
...
2. Update the `config.user.yaml` to include the clusters you want to access and what AWS profile you want to use for that access. See
   the [reference docs](/docs/24-05/reference/configuration/kubernetes#config.user.yaml) for the specific syntax.
...
* [Learn how to deploy infrastructure using infrastructure-as-code](/docs/24-05/guides/iac/deploying-modules)

* [Customize to local devenv](/docs/24-05/guides/developer-environment/customizing)

* [Connect to databases](/docs/24-05/guides/networking/database-connections)

<MarkdownGuideNav backHref={"/docs/24-05/guides/getting-started/boot-developer-environment"} stepNumber={4} totalSteps={4} progressLabel={"Getting Started Guide:"} />
