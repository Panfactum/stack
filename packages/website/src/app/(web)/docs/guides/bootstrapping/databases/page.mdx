...
We provide a module to deploy the operator: [kube\_cloudnative\_pg](../../reference/terraform-modules/kube_cloudnative_pg).
...
We provide a module [kube\_pg\_cluster](../../reference/terraform-modules/kube_pg_cluster) that you can use in your
projects to do this.
...
module called [test\_kube\_pg\_cluster](../../reference/terraform-modules/kube_pg_cluster) which is a thin
wrapper around [kube\_pg\_cluster](../../reference/terraform-modules/kube_pg_cluster).
...
3. You should eventually receive an output like the following:
   ...
   in [inbound networking section](./inbound-networking).
...
manner. Instead, you can run our [kube\_redis\_sentinel](../../reference/terraform-modules/kube_redis_sentinel)
directly.
...
<BootstrappingGuideNav backHref={"./maintenance-controllers"} forwardHref={"./identity-provider"} stepNumber={17} />
