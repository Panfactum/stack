...
We provide both a module to deploy Authentik ([kube\_authentik](../../reference/terraform-modules/kube_authentik)) and
many modules to connect Authentik to the various systems in the Panfactum stack (e.g., [authentik\_aws\_sso](../../reference/terraform-modules/authentik_aws_sso)).
...
We provide a module to do this: [aws\_ses\_domain](../../reference/terraform-modules/aws_ses_domain).
...
3. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/aws_ses_domain/terragrunt.hcl).

4. Enable the `aws`, `random`, and `time` providers in your `module.yaml`.

5. Run `terragrunt apply`.

6. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record
   ...
   to set this up. You can use the [aws\_dns\_records](../../reference/terraform-modules/aws_dns_records) module to
   deploy the record like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/global/aws_dns_records/terragrunt.hcl).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_authentik/terragrunt.hcl).
...
7. Save the `akadmin_bootstrap_token` value in your `.env` as `AUTHENTIK_TOKEN`. This will be used by the [Authentik
   terraform provider](https://registry.terraform.io/providers/goauthentik/authentik/latest/docs) in subsequent steps
...
9. Sign-in with the username `akadmin` and the `akadmin_bootstrap_password` from your terraform outputs.
...
branding: [authentik\_core\_resources](../../references/terraform-modules/authentik_core_resources).
...
RBAC reference](../../reference/rbac) for more information.
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/authentik_core_resources/terragrunt.hcl).
   ...
   5. `default_groups_enabled` will provision the [default Panfactum RBAC groups](../../reference/rbac) (`superusers` will always be deployed).
...
4. Provision a new API token.
   ...
   7. We recommend using a short-lived, expiring token as the credential will have root access to Authentik and thus
      *every* system in your ecosystem. Generally, you will only need this for initially setting up the Authentik
      terraform modules which will not change frequently.
...
<BootstrappingGuideNav backHref={"./databases"} forwardHref={"./federated-auth"} stepNumber={18} />
