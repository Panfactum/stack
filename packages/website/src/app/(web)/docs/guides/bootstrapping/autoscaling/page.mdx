...
We provide a module to deploy the server: [kube\_metrics\_server](../../reference/terraform-modules/kube_metrics_server).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_linkerd` module called `kube_metrics_server`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_metrics_server/terragrunt.hcl).
...
We provide a module to deploy the VPA: [kube\_vpa](../../reference/terraform-modules/kube_vpa).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_metrics_server` module called `kube_vpa`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_vpa/terragrunt.hcl).
...
9. Return to the pod view. Notice that all request columns are populated as well as the memory limit column:
   ...
   across all pods *proportional to each pod's CPU request.*
...
We provide a terraform module to deploy it: [kube\_karpenter](../../reference/terraform-modules/kube_karpenter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_vpa` module called `kube_karpenter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_karpenter/terragrunt.hcl).
...
Those are defined in the [kube\_karpenter\_node\_pools](../../reference/terraform-modules/kube_karpenter_node_pools) module.

Let's deploy it now:

1. Create a new directory adjacent to your `kube_karpenter` module called `kube_karpenter_node_pools`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_karpenter_node_pools/terragrunt.hcl).
...
Now that autoscaling is active, we can [proceed to set up inbound networking for our cluster.](./external-cluster-networking).
