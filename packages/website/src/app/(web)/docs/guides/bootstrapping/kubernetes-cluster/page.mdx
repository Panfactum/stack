...
using our [aws\_eks](../../reference/terraform-modules/aws_eks) module.
...
We provide a module for configuring this behavior: [aws\_ecr\_pull\_through\_cache](../../reference/terraform-modules/aws_ecr_pull_through_cache).
...
with the rest of the Panfactum stack. See the [module documentation](../../reference/terraform-modules/aws_eks) if
...
you created in the [aws networking guide](./aws-networking). [^3] This ensures the API server is resilient to an AZ outage.
...
In the Panfactum stack, that file is stored in your repo in the `$PF_KUBE_DIR` directory. [^9] By default, this is set
to `.kube`. This should have been created when you ran `pf-update-kube` to resolve the warnings when you first
installed the Panfactum devenv.
...
To your `$PF_KUBE_DIR`, add a `config.user.yaml` file that looks like this: [^7]
...
terragrunt directory under `$PF_ENVIRONMENTS_DIR` and `aws_profile` is the AWS profile you want to use to authenticate
with the cluster (more on this later).

Replace `module` with the appropriate path for the cluster you just launched. For now, use the `aws_profile` you
are using for terragrunt in that environment.

Run `pf-update-kube` to dynamically build your kubeconfig file. [^8]

[^8]: This will create a kubeconfig file at `$PF_KUBE_DIR/config`. The `pf-update-kube` config
    only updates this file to include the clusters defined in your `config.user.yaml` file. If desired, you can manually
    add additional clusters to this file as well. This file will not be committed to version control as it is specific to every
    user.

### Verify Connection

1. Run `kubectx` to list all of the clusters that were set up in the previous section. Selecting one will set your
...
In the Panfactum stack everything is deployed via terraform, including Kubernetes manifests. [^10] By constraining ourselves
...
In order to start using our kubernetes terraform modules, we must first configure the
[kubernetes provider](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs) by setting
some additional [terragrunt variables](../../reference/terragrunt-variables).
...
Kubernetes modules deployed in this region will now appropriately deploy to this cluster.

### RBAC

Up until now, we have been using implicit EKS authentication to communicate with the cluster (the IAM user that created
the cluster automatically has cluster access). We will now deploy the [kube\_rbac](../../reference/terraform-modules/kube_rbac)
...
4. A bootstrapped, default version of this ConfigMap gets created when you first create an EKS cluster. Delete it
   with `kubectl -n kube-system delete configmap aws-auth`.

5. Run `terragrunt apply`.

6. To verify that cluster authentication is still functional, we will need to terminate a node and make sure the replacement node
...
7. The autoscaling group should notice the missing node within a few minutes and launch a new node to replace the terminated node.
...
8. If the node joins successfully, you have successfully setup your initial Kubernetes RBAC.
...
as an unexpected AZ outage. These are defined in the [kube\_priority\_classes](../../reference/terraform-modules/kube_priority_classes)
module.
...
Now that the cluster is running, we will begin working on the [internal networking stack](./internal-cluster-networking).
