...
check out our concept documentation (TODO).

## Deploy Linkerd2

[Linkerd](https://linkerd.io/2.15/overview/) is likely the most popular service mesh for Kubernetes,
and we provide a module for deploying it: [kube\_linkerd](../../reference/terraform-modules/kube_linkerd)
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_linkerd/terragrunt.hcl).
...
Linkerd comes with a companion cli tool, `linkerd`. This tool is included in the Panfactum devenv.
...
1. Using k9s, restart all your deployments not in the `linkerd` namespace:
   ...
   1. Navigate to `:deployments`. Don't forget to change the namespace filter to "all" by pressing `0`.

   2. Select each deployment by pressing `<space>`.

   3. Press `r` to restart.

2. Perform a similar restart to `:statefulsets`.

3. Notice that the new pods have linkerd sidecar containers:
...
Run `linkerd check` again to ensure the control plane is working.
...
At this point, all of the necessary components for powering *internal* cluster networking are configured. However, before
we configure *external* networking, [we should address autoscaling](./autoscaling)
