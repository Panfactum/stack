...
<MarkdownAlert severity="info">
  ...
  * [kube\_rbac](/docs/reference/infrastructure-modules/kube_rbac) to allow SSO authentication
    with your Kubernetes clusters. Note that you MUST omit the /aws-reserved/sso.amazonaws.com/us-east-2/
    part of the ARN.

  * [aws\_eks](/docs/reference/infrastructure-modules/aws_eks) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encrypt\_key](/docs/reference/infrastructure-modules/aws_kms_encrypt_key)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](/docs/reference/infrastructure-modules/kube_vault) to allow SSO authentication
...
[authentik\_aws\_sso](/docs/reference/infrastructure-modules/authentik_aws_sso).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/authentik_aws_sso/terragrunt.hcl).
   ...
   4. Set `aws_scim_enabled` to `false` and omit `aws_scim_url` and `aws_scim_token` for now.
...
to automatically set this up: [aws\_iam\_identity\_center\_permissions](/docs/reference/infrastructure-modules/aws_iam_identity_center_permissions).
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](/docs/reference/infrastructure-modules/aws_kms_encrypt_key) module
   * EKS clusters deployed with [aws\_eks](/docs/reference/infrastructure-modules/aws_eks)
   * Vault clusters deployed with [kube\_vaults](/docs/reference/infrastructure-modules/kube_vault)
...
5. Set up `config.yaml` according to [these reference docs](/docs/reference/aws/config-yaml).
...
[authentik\_vault\_sso](/docs/reference/infrastructure-modules/authentik_vault_sso).
...
We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](/docs/reference/infrastructure-modules/vault_auth_oidc).
