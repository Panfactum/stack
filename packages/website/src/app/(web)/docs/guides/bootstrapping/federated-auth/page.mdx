...
<MarkdownAlert severity="info">
  ...
  * [kube\_rbac](../../reference/terraform-modules/kube_rbac) to allow SSO authentication
    with your Kubernetes clusters. Note that you MUST omit the /aws-reserved/sso.amazonaws.com/us-east-2/
    part of the ARN.

  * [aws\_eks](../../reference/terraform-modules/aws_eks) to allow SSO authentication
    with the cluster KMS encryption key.

  * sops keys (deployed with [aws\_kms\_encryption\_key](../../reference/terraform-modules/aws_kms_encryption_key)) to allow SSO authentication
    with the sops KMS encryption key.

  * [kube\_vault](../../reference/terraform-modules/kube_vault) to allow SSO authentication
...
so we will have to configure several settings manually before we can begin terraforming.
...
[authentik\_aws\_sso](../../reference/terraform-modules/authentik_aws_sso).
...
organizations. Fortunately, AWS *does* expose APIs for this, so we can provide a terraform module
to automatically set this up: [aws\_iam\_identity\_center\_permissions](../../reference/terraform-modules/aws_iam_identity_center_permissions).
...
3. For the `acccount_access_configuration`, we recommend setting this up according to our [RBAC recommendations](../../reference/rbac),
...
2. As AWS KMS keys are special resources that require explicit permission grants, you must re-apply
   ...
   * sops keys deployed with the [aws\_kms\_encrypt\_key](../../reference/terraform-modules/aws_kms_encrypt_key) module
   * EKS clusters deployed with [aws\_eks](../../reference/terraform-modules/aws_eks)
   * Vault clusters deployed with [kube\_vaults](../../reference/terraform-modules/kube_vault)
...
5. Set up `config.yaml` according to [these reference docs](../../reference/aws/config-yaml).
...
8. Ensure that you adjust your `$PF_KUBE_DIR/config.user.yaml` and terragrunt variables to reflect the
   ...
   access. See [this guide](../terraforming/deploying-modules#local-deployments) for more information.
...
[authentik\_vault\_sso](../../reference/terraform-modules/authentik_vault_sso).
...
We also provide a module to configure Vault to work with the above Authentik configuration: [vault\_auth\_oidc](../../reference/terraform-modules/vault_auth_oidc).
...
1. Remove your hardcoded `VAULT_ADDR` and `VAULT_TOKEN` token from your `.env` file. Make sure that you added the
   `vault_addr` in you `region.yaml` as covered in the [inbound networking section.](./inbound-networking#deploy-the-vault-ingress)
...
<BootstrappingGuideNav backHref={"./identity-provider"} forwardHref={"./next-steps"} stepNumber={19} />
