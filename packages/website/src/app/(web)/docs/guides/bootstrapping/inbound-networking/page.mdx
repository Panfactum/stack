...
We provide a module to deploy it: [kube\_external\_dns](../../reference/terraform-modules/kube_external_dns)
...
3. The syntax for `route53_zones` is the same as it was for `kube_cert_issuers`. You may reference [that guide](./certificate-management#deploy-issuers)
...
We provide a module to deploy it: [kube\_aws\_lb\_controller](../../reference/terraform-modules/kube_aws_lb_controller)
...
We provide a module to deploy it: [kube\_ingress\_nginx](../../reference/terraform-modules/kube_ingress_nginx).
...
1. Return the `kube_vault` module you deployed in the [Vault guide section](./vault).
...
7. Moreover, notice that NGINX properly secures the site in a standard way by setting security headers for the browser. You
   ...
   This is accomplished by our [kube\_ingress](../../reference/terraform-modules/kube_ingress)
...
We provide a bastion deployment module: [kube\_bastion](../../reference/terraform-modules/kube_bastion).
...
[let's install a handful of maintenance controllers](./maintenance-controllers) that will ensure things continue
to operate smoothly.

<MarkdownGuideNav backHref={"./autoscaling"} forwardHref={"./maintenance-controllers"} stepNumber={15} totalSteps={19} progressLabel={"Panfactum Bootstrapping Guide:"} />
