...
We provide a module to deploy it: [kube\_external\_dns](/docs/reference/infrastructure-modules/kube_external_dns)

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_external_dns`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_external_dns/terragrunt.hcl).

3. The syntax for `route53_zones` is the same as it was for `kube_cert_issuers`. You may reference [that guide](/docs/guides/bootstrapping/certificate-management#deploy-issuers)
...
We provide a module to deploy it: [kube\_aws\_lb\_controller](/docs/reference/infrastructure-modules/kube_aws_lb_controller)

Let's deploy it now:

1. Create a new directory adjacent to your `kube_external_dns` module called `kube_aws_lb_controller`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_aws_lb_controller/terragrunt.hcl).
...
We provide a module to deploy it: [kube\_ingress\_nginx](/docs/reference/infrastructure-modules/kube_ingress_nginx).

### Deploy the NGINX-Ingress Controller

1. Create a new directory adjacent to your `kube_external_dns` module called `kube_ingress_nginx`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_ingress_nginx/terragrunt.hcl).
...
1. Return the `kube_vault` module you deployed in the [Vault guide section](/docs/guides/bootstrapping/vault).
...
7. Moreover, notice that NGINX properly secures the site in a standard way by setting security headers for the browser. You
   ...
   This is accomplished by our [kube\_ingress](/docs/reference/infrastructure-modules/kube_ingress)
...
We provide a bastion deployment module: [kube\_bastion](/docs/reference/infrastructure-modules/kube_bastion).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_bastion/terragrunt.hcl).
...
4. Update the values to the correct values for your setup. See the [reference docs](/docs/reference/ssh/config-yaml) for
   more information.
...
5. For fun, run `kubectl rollout restart bastion -n bastion` to restart the bastion instances. Notice that the tunnel
...
<BootstrappingGuideNav backHref={"/docs/guides/bootstrapping/autoscaling"} forwardHref={"/docs/guides/bootstrapping/maintenance-controllers"} stepNumber={15} />
