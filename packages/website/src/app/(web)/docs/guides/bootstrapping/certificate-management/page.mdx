...
This module creates two different types of issuers:
...
* An issuer for *internal* certificates signed by your Vault cluster [^1] for use in securing internal network traffic
  such as which the service mesh (next section)
...
Congratulations! We have just verified that the internal certificate management infrastructure is working
...
which will ensure that *all* traffic in the cluster is encrypted with mTLS.](./service-mesh)
