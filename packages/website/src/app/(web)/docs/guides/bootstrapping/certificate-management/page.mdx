...
our [concept docs on network cryptography](../../concepts/networking/cryptography) for the relevant background
information.
...
We provide a module for deploying cert-manager: [kube\_cert\_manager](../../reference/terraform-modules/kube_cert_manager).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_vault` module called `kube_cert_manager`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_cert_manager/terragrunt.hcl).
...
5. Add a `module.yaml` that enables the `kubernetes`, `helm`, and `random` providers.
...
We provide a module to set up the issuers required by the Panfactum stack: [kube\_cert\_issuers](../../reference/terraform-modules/kube_cert_issuers).
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_cert_issuers/terragrunt.hcl).
...
4. Let's walk through the `route53_zones` input:
   ...
   2. In the prior [DNS guide](./dns), we configured DNS in the [aws\_registered\_domains](../../reference/terraform-modules/aws_registered_domains)
      and [aws\_delegated\_zones](../../reference/terraform-modules/aws_delegated_zones) modules
      under `management/global`. Because
      these modules use a separate environment's state bucket, we cannot use them in terragrunt `dependency` blocks.
   ...
   5. Note that you might want to issue certificates and records in the root zones created by the
      `aws_registered_domains` module, especially in your production environment. These zones
      are in the `management` AWS account, so we must ensure that we provide cross-account
      access [^3] via the `additional_account_ids_with_record_access` input. In
      [this Panfactum example](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/management/global/aws_registered_domains/terragrunt.hcl),
      we provide access to our `production` account to manage records under `panfactum.com`.
...
[^3]: This is one of the few exceptions to the environment segregation principle. We recommended
    *only* doing this for production as this should already be your must secure environment.
...
There is one final component to the certificate infrastructure: [trust-manager](https://cert-manager.io/docs/trust/trust-manager/).

trust-manager is an ancillary cert-manger utility that copies CA trust bundles into every cluster namespace
so that they can be consumed by cluster workloads. As our private CA's (Vault) certs are not installed in every operating system
by default, this provides a convenient way to make them available.

We provide a module for deploying trust-manager: [kube\_trust\_manager](../../reference/terraform-modules/kube_trust_manager).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cert_manager` module called `kube_trust_manager`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_trust_manager/terragrunt.hcl).

3. Set `vpa_enabled` to `false`. We will enable it when we deploy the autoscalers.

4. Add a `module.yaml` that enables the `kubernetes` and `helm` providers.

5. Run `terragrunt apply`.

Note that unlike most modules which receive their own namespace, trust-manager will be deployed to the cert-manager
namespace. [^7]

[^7]: Requirement of the trust-manager

## Next Steps

Now that internal certificate management is working, [we will build upon that foundation to deploy a service mesh
which will ensure that *all* network traffic in the cluster is secured with mTLS.](./service-mesh)
