...
for the Deployment or StatefulSet. As a result, pods might continue to run with stale data.
...
We provide a module to deploy this controller: [kube\_reloader](../../reference/terraform-modules/kube_reloader).

Let's deploy the Reloader now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_reloader`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_reloader/terragrunt.hcl).
...
We provide a module for the Descheduler: [kube\_descheduler](../../reference/terraform-modules/kube_descheduler).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_reloader` module called `kube_descheduler`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_descheduler/terragrunt.hcl).
...
All of the cluster's persistent data is stored on [PersistentVolumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/)
backed by [AWS EBS volumes](https://docs.aws.amazon.com/ebs/latest/userguide/ebs-volumes.html). This is managed
via the [kube\_aws\_ebs\_csi](../../reference/terraform-modules/kube_aws_ebs_csi) module we installed in
[a prior section](./storage-interfaces).
...
We provide a module to deploy the external-snapshotter: [kube\_external\_snapshotter](../../reference/terraform-modules/kube_external_snapshotter).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_descheduler` module called `kube_external_snapshotter`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_external_snapshotter/terragrunt.hcl).
...
We provide a module to deploy this functionality: [kube\_velero](../../reference/terraform-modules/kube_velero).

By default, this will create hourly backups of the cluster and its data that will allow you to rollback your entire
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_velero/terragrunt.hcl).
...
5. **We will now simulate a disaster.**
   ...
   4. Let's verify that Vault is truly unavailable. Run `vault token lookup`. You should receive an error like the following:

      ```shell-session
      Error looking up token: Get "https://vault.prod.panfactum.com/v1/auth/token/lookup-self": tls: failed to verify certificate: x509: certificate is valid for ingress.local, not vault.prod.panfactum.com
      ```
...
additional databases](./database-operators) to the cluster for more serious workloads.
