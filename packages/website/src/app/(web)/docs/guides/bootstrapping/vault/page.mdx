...
We will use the [kube_vault](../../references/terraform-modules/kube_vault) terraform module to deploy
...
2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/kube_vault/terragrunt.hcl).

3. For now, set `vpa_enabled` to `false`. We will enable it when we install the autoscalers.

4. For now, set `ingress_enabled` to `false`. We will enable it when we install the ingress subsystem.

5. Add a `module.yaml` that enables the `aws`, `kubernetes`, and `helm` providers.

6. Run `terragrunt apply`.

7. If the module deploys successfully, you will see three vault instances which are in the unready state:

    <MarkdownImage src={unreadyVaultImg} alt="Vault pods are in the unready state" />

    Checking the logs of one of the Vault pods will show error messages that look like this:
...
[^6]: The recovery keys are not the _unseal keys_. We do not utilize static unseal keys in the Panfactum setup
...
One of the major benefits of Vault is that it can be [configured directly with terraform](https://registry.terraform.io/providers/hashicorp/vault/latest/docs)
...
via the [vault_core_resources](../../reference/terraform-modules/vault_core_resources) module.
...
Let's now deploy [vault_core_resources](../../reference/terraform-modules/vault_core_resources):

1. Create a new directory adjacent to your `kube_vault` module called `vault_core_resources`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__currentPanfactumVersion__/packages/reference/environments/production/us-east-2/vault_core_resources/terragrunt.hcl).

3. Vault will issue ssh certificates that allow users in your organization to connect to private network resources.
Those certificates are valid for `ssh_cert_lifetime_seconds`. We recommend setting this to a fairly low value
($\leq$ 8 hours) as long-lived certificates would allow de-provisioned users to continue to access the private network. [^8]

4. Add a `module.yaml` that enables the `aws`, `kubernetes`, `time`, and `vault` providers.

5. Run `terragrunt apply`.

[^8]: Access to the private network is not the only security gate for accessing private systems in the Panfactum stack,
...
in the [prior section](./storage_interfaces).
...
[next section](./certificate-management).
