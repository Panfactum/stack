...
5. Add a `module.yaml` that enables the `aws`, `kubernetes`, `random`, and `helm` providers.
...
3. Add a `module.yaml` that enables the `kubernetes`, `time`, and `vault` providers.
...
<MarkdownGuideNav backHref={"./storage-interfaces"} forwardHref={"./certificate-management"} stepNumber={11} totalSteps={19} progressLabel={"Panfactum Bootstrapping Guide:"} />
