...
<Alert severity="info">
    You should use [this search interface](https://search.nixos.org/packages) to find packages.
</Alert>
...
<Alert severity="warning">
    ...
    `pkgs` refers to the set of packages defined **specific commit** in `nixpkgs`.
    That commit hash is determined by the `inputs.pkgs.url` field in your `flake.nix`.

    Even if you specify a branch such as `nixos-23.11` (via `inputs.pkgs.url = "github:NixOS/nixpkgs/nixos-23.11";`),
    ...
    In general, we recommend pinning the commit directly in the `flake.nix` to avoid ambiguity such
    as with `inputs.pkgs.url = "github:NixOS/nixpkgs/c8e74c2f83fe12b4e5a8bd1abbc090575b0f7611";`.
</Alert>
...
Since all of the `nixpkgs` is versioned atomically, you may find yourself in the following scenario:

> You want a version of utility `A` that is found at `nixpkgs` commit `XXX` but also a version of utility
