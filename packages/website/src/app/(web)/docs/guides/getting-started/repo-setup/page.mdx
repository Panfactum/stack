...
1. Create a nix flake in the root of your repo by generating a `flake.nix` file
...
2. Run `git add flake.nix devenv.nix` to register the flake and devenv file.
...
4. Test that you are able to instantiate the development environment via `nix develop --impure`. If everything is working,
you should see multiple environment variables when you run `printenv | grep DEVENV`.
