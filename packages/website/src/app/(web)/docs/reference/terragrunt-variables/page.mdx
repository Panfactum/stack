...
| Variable           | Type   | Description                                                                                                                                                                                                                                                 | Recommended File   |
| :----------------- | :----- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------- |
| `version`          | string | Configures `include.panfactum.locals.source`. The version of the local modules from `PF_TERRAFORM_DIR` you are using.  Must be a valid [commit ref](https://git-scm.com/book/en/v2/Git-Internals-Git-References) to the stack repo.                         | `environment.yaml` |
| `pf_stack_edition` | string | Configures `include.panfactum.locals.pf_stack_source`. The edition of the Panfactum stack you are using. One of: `community`, `enterprise`, `local`.                                                                                                        | `global.yaml`      |
...
| `module`           | string | If using `include.panfactum.locals.source`, defines which module from `PF_TERRAFORM_DIR` to use. If using, `include.panfactum.locals.pf_stack_source`, defines which module from `packages/terraform` to use. Defaults to the name of the module directory. | `module.yaml`      |
