...
* When you register a domain with AWS, that "registers" the nameservers of your management account's DNS servers as
...
* When you query a delegated domain name (i.e., `dig test.dev.panfactum.com`), the resolution process works as follows: [^1]
  ...
  * Check the nameservers for `panfactum.com` (in the management account) for the [A record](https://www.cloudflare.com/learning/dns/dns-records/dns-a-record/) `test.dev.panfactum.com`. Notice that there are
