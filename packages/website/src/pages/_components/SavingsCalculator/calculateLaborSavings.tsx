...
import {
  ...
  LABOR_HOURS_PER_CUSTOM_WORKLOAD,
...
export function calculateLaborSavings(
  ...
  const baseClusterCost =
    laborHourlyCost * clusterCount * LABOR_HOURS_PER_CLUSTER;
  const baseWorkloadCost =
    laborHourlyCost *
    clusterCount *
    workloadCount *
    LABOR_HOURS_PER_CUSTOM_WORKLOAD;
  const baseDBCost =
    laborHourlyCost * clusterCount * dbCount * LABOR_HOURS_PER_DB_WORKLOAD;
  const baseModuleCost =
    ...
    LABOR_HOURS_PER_STANDARD_MODULE;
  const baseCostOpt2 =
    baseClusterCost + baseWorkloadCost + baseDBCost + baseModuleCost;
        ...
        lineItems={
          baseCostOpt2 > baseCostOpt1
            ? [
                ...
                {
                  content: `${CFP.format(baseWorkloadCost)} (${CFP.format(laborHourlyCost)} hourly labor cost x ${NUMBER_FORMAT.format(LABOR_HOURS_PER_CUSTOM_WORKLOAD)} labor hours per custom workload x ${NUMBER_FORMAT.format(clusterCount * workloadCount)} workload deployments`,
                  operation: "+",
                },
                {
                  content: `${CFP.format(baseDBCost)} (${CFP.format(laborHourlyCost)} hourly labor cost x ${NUMBER_FORMAT.format(LABOR_HOURS_PER_DB_WORKLOAD)} labor hours per database x ${NUMBER_FORMAT.format(clusterCount * dbCount)} database deployment`,
                  operation: "+",
                },
                {
                  content: `${CFP.format(baseModuleCost)} (${CFP.format(laborHourlyCost)} hourly labor cost x ${NUMBER_FORMAT.format(LABOR_HOURS_PER_STANDARD_MODULE)} labor hours per Panfactum module x ${NUMBER_FORMAT.format(clusterCount * moduleCount)} module deployments`,
                  operation: "+",
                },
                null,
                {
