  ...
  return (
    <span class="text-secondary font-semibold xl:col-span-2">
      <Show when={props.perCluster} fallback={`x ${CURRENCY_FORMAT.format(props.price)}`}>
        ...
        >
          <span class="text-balance text-left">x {props.clusterCount || 1} cluster{(props.clusterCount || 1) > 1 ? "s" : ""} x {CURRENCY_FORMAT.format(props.price)} per cluster</span>
         <div class=" w-4 min-w-4 overflow-visible"><InfoIcon/></div>
        </button>
      </Show>
      <Modal
        open={open()}
