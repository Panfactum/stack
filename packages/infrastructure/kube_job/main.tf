...
module "pod_template" {
  ...
  restart_policy                       = "Never" // Required to use podFailurePolicy on Job
...
resource "kubectl_manifest" "job" {
    ...
    spec = {
      ...
      podFailurePolicy = {
        rules = concat(
          ...
          [for rule in var.extra_pod_failure_policy_rules : {
            action = rule.action
            onPodConditions = [for condition in rule.on_pod_conditions : {
              type = condition.type
            }]
            onExitCodes = rule.on_exit_codes != null ? {
              containerName = rule.on_exit_codes.container_name
              operator      = rule.on_exit_codes.operator
              values        = rule.on_exit_codes.values
            } : null
          }]
        )
      }
  ...
  dynamic "wait_for" {
    for_each = var.wait_for_success ? toset(["true"]) : toset([])
    content {
      field {
        key   = "status.succeeded"
        value = var.pod_completions
      }
    }
  }
  ...
  force_new         = true
  depends_on        = [kubectl_manifest.pdb]
}
...
resource "kubectl_manifest" "vpa" {
  ...
  depends_on = [kubectl_manifest.job]
}
