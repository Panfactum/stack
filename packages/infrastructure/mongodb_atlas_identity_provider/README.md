...
- If "Bypass SAML Mode" is enabled, users will be able to bypass Authentik and login directly to Atlas using their
  static usernames and passwords. As a result, we strongly recommend keeping the [SSO Debug Mode](#sso_debug_enabled) input as `false` after initial setup.
...
1. Add a text record to your `production/global/aws_dns/terragrunt.hcl` file that looks
   like this:
      ...
      ::: code-group labels=[production/global/aws_dns/terragrunt.hcl]
...
1. Add a new a `terragrunt.hcl` file that looks like this:
      ...
      ::: code-group labels=[mongodb_atlas_identity_provider/terragrunt.hcl]
