...
module "tf_deploy_workflow" {
  ...
  common_env = {
    ...
    # We need to provide environment variables that would have
    # otherwise been generated by devenv b/c we do not use devenv
    # in the CI pipelines
    PF_REPO_URL            = var.repo_url
    PF_IAC_DIR             = var.iac_dir
    PF_ENVIRONMENTS_DIR    = var.environments_dir
    PF_REPO_PRIMARY_BRANCH = var.repo_primary_branch
    PF_REPO_NAME           = var.repo_name
    ALT_DEVENV_ROOT        = var.alternative_devenv_root
