---
title: "Installing the Workflow Engine"
---
...
Install the management controllers and Web dashboard for [Argo Workflows](https://argoproj.github.io/workflows/)
and [Argo Events](https://argoproj.github.io/argo-events/).
...
Because you will often want these types of external triggers, we install both Argo Workflows _and_ Argo Events
in this guide even though they are separate projects.

## Installing

We provide a module, [kube_argo](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_argo), that
...
2. Add a `terragrunt.hcl` that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/kube_argo/terragrunt.hcl).
...
<Image src={DashboardImg} alt="Argo Workflow dashboard" />
...
<Image src={WorkflowsImg} alt="Recently executed Argo Workflows" />
...
<Image src={WorkflowTemplatesImg} alt="Workflow templates" />
...
<Image
  src={WorkflowTemplateSubmitImg}
  alt="Submitting the test WorkflowTemplate"
/>
...
<Image src={WorkflowExecutionImg} alt="Workflow execution graph" />

7. Click "Logs" to verify the generated pods printed your message:

<Image src={WorkflowLogsImg} alt="Workflow logs" />
