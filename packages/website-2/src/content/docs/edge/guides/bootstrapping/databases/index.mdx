...
[^1]:
    Okay... technically, it is the embeddable [SQLite](https://www.sqlite.org/) which is included in virtually
...
We provide a module to deploy the operator: [kube_cloudnative_pg](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cloudnative_pg).

Let's deploy it now:

1. Create a new directory adjacent to your `aws_eks` module called `kube_cloudnative_pg`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/kube_cloudnative_pg/terragrunt.hcl).
...
We provide a module [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) that you can use in your
projects to do this.

### Test PostgreSQL Cluster

<MarkdownAlert severity="info">
  This section will deploy and tear down a test PostgreSQL cluster. While we
  recommend completing these steps to gain familiarity in working with
  databases, you can skip this without impacting your cluster's functionality.
</MarkdownAlert>

#### Deploy Test Cluster

To demonstrate the capabilities of PostgreSQL running in the Panfactum stack, we have created a demo
module called [test_kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) which is a thin
wrapper around [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster).

Let's deploy it now:

1. Create a new directory adjacent to your `kube_cloudnative_pg` module called `test_kube_pg_cluster`.

2. Add a `terragrunt.hcl` to that directory that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/test_kube_pg_cluster/terragrunt.hcl).
...
5. In k9s, notice that there is a new Cluster resource that will go through multiple statuses as it launches (`:cluster`):
   ...
   <Image
     src={k9sPGClusterCreateImg}
     alt={"PostgreSQL cluster created in k9s"}
   />

6. Switch to the pods view (`:pods`) and notice several new pods will come up after approximately 2-3 minutes:

   <Image src={k9sPGPodsImg} alt={"PostgreSQL pods running in the cluster"} />
...
7. Navigate to the S3 view in the AWS web console. Notice that a new bucket was created for the cluster:
   ...
   <Image src={pgBackupS3Img} alt={"PostgreSQL WAL backups in S3"} />
   ...
   that you have the ability to rollback the database _arbitrary_ points in time. [^3] This is significantly more
...
3. You should eventually receive an output like the following:
   ...
   in [inbound networking section](/docs/edge/guides/bootstrapping/inbound-networking).
...
5. Register a new server:
   ...
   <Image
     src={pgAdminRegisterSeverImg}
     alt={"Register a new server with PgAdmin4"}
   />
...
7. You should now see the main PgAdmin4 dashboard and can begin running commands against the database:
   ...
   <Image
     src={pgAdminDashboardImg}
     alt={"PgAdmin4 dashboard for the test database"}
   />
...
manner. Instead, you can run our [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)
directly.
...
- **Relational:** PostgreSQL ([kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster))
- **Key-value:** Redis ([kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel))
