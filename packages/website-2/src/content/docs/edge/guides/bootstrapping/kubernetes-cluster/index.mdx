...
import pullThroughCacheSvg from "./pull-through-cache.svg?raw";
...
using our [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) module.
...
<MarkdownAlert severity="info">
  To save costs during the bootstrapping process you can follow the [Kubernetes
  Cluster Suspension
  Guide](/docs/edge/guides/bootstrapping/suspend-kubernetes-cluster) to suspend
  resources and resume as you are setting up your cluster.
</MarkdownAlert>
...
<ImageSvg
  src={pullThroughCacheSvg}
  alt="Model for how pull through image caches operate in AWS"
/>

Instead of cluster nodes pulling images directly from a public registry, they will pull them from ECR which is then
configured to download the image from the public registry _only if it does not already contain the image
in its cache_. In this way, most images will only ever need to be downloaded from a public source _once_ during
the initial deployment.

We provide a module for configuring this behavior: [aws_ecr_pull_through_cache](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ecr_pull_through_cache).
...
<MarkdownAlert severity="warning">
  This should only be deployed *once* per environment-region combination. If you
  have multiple clusters in the same region, you do **not** need to deploy the
  pull through cache multiple times.
</MarkdownAlert>
...
[^3]:
    Ensure that you choose an odd number for proper resilience to AZ outages. A requirement of `etcd`, the database
...
[^4]:
    Note that this must be an odd number in order for many high availability algorithms to work in the case of an
...
<Image
  src={eksDashboardImg}
  alt="Kubernetes cluster dashboard in AWS web console"
/>
...
1. Add a `config.user.yaml` file that looks like this: [^7]
   ...
   [^7]:
       This file is specific to every user as different users will have different access levels to the various
...
<MarkdownAlert severity="warning">
  *Every* one of your users will need to set up their own `config.user.yaml`
  file, and we provide instructions for them
...
4. You should see a minimal list of pods that looks like this:
   ...
   <Image src={k9sImg} alt="k9s listing all pods" />
...
[^12]:
    Just like human users, EC2 nodes use AWS IAM roles to authenticate with the EKS Kubernetes API server. These roles
...
the cluster automatically has cluster access). We will now deploy the [kube_rbac](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_rbac)
...
[^11]:
    The `<namespace>/<resource>` syntax is common in the Kubernetes ecosystem. `kube-system/aws-auth`
...
5. To verify that cluster authentication is functional, you should be able your cluster nodes within k9s (`:nodes`):
   ...
   <Image
     src={k9sNodesImg}
     alt="k9s showing node successfully joining the cluster"
   />
...
as an unexpected AZ outage. These are defined in the [kube_priority_classes](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_priority_classes)
module.
