---
title: "Cryptography in Networking"
---

import ImageSvg from "@/components/solid/markdown/MarkdownImgSvg";
import tlsMotivationSvg from "./tls-motivation.svg?raw";
...
are _two_ mathematically related, but not identical keys used in cryptographic operations (as opposed to one).
...
- Digital signatures: One key signs arbitrary data to produce a signature, and another entity can use the _other_ key
...
cryptography to introduce the concept of _certificates_.
...
X.509 also introduces the concept of _certificate authorities_ (CAs) which can be used to verify
...
When you use the internet, your traffic is routed across many _untrusted_ computers (routers) before finally
...
<ImageSvg src={tlsMotivationSvg} alt="MITM Attacker" />
...
where you assume any intermediate computer _could_ be an attacker and implement the appropriate safeguards.
...
[^8]:
    How to do this varies heavily with your particular operating system and setup. In the Panfactum stack,
...
which serves as its _own_ CA. Obviously, this provides no additional trust value and is used
...
want _any_ computer to be able to connect to your server. However, in many situations you only
want _particular_ clients to be able to connect.

Using _mutual_ TLS, both the client and server present their
...
- A typical VPN setup will allow access to _all_ networked resources in the private network by default.
...
- A typical VPN setup will route _all_ network traffic from the remote machine to the private network. This introduces problems:
...
- Utilize a point-to-point proxy solution such as [ssh tunneling](https://www.ssh.com/academy/ssh/tunneling-example) which enables
  users to expose _individual_ private network resources on local computer ports.
