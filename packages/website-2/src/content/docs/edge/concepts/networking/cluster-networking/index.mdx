...
import ImageSvg from "@/components/solid/markdown/MarkdownImgSvg";
...
import servicesSvg from "./services.svg?raw";
...
Kubernetes provides _some_ out-of-the-box functionality for L3/4 networking. This includes two
core capabilities:
...
<ImageSvg src={servicesSvg} alt="Services in Kubernetes" />
...
The Panfactum Stack's Cilium deployment ([kube_cilium](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium)) has a few enhancements over the default Cilium install:
...
then become the _only_ containers that communicate directly with one another and thus create a mesh that captures
and orchestrates all network traffic.

<ImageSvg src={serviceMeshSvg} alt="Service mesh diagram" />
...
15 minutes of setup time (if using the Panfactum stack), you _automatically_ get the following without _any_ additional
work:
...
Ultimately, you do not _need_ a service mesh, but you _should_ choose to use one if you are working with Kubernetes in a professional setting.
...
You can see our implementation in the [kube_linkerd](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_linkerd) terraform module.
...
Both Cilium and Linkerd primarily deal with _intra-_ and _inter_-cluster networking. That said, a key additional
...
[^10]:
    Why not expose your workloads directly to the public internet? There are a few reasons, but the most important
...
- An [Ingress controller](https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/) resource which receives traffic
  ...
  for your entire cluster which receives _all_ inbound traffic and then routes traffic to other pods based on your custom
  routing rules (e.g., HTTP `/api/v1` goes to `foo-service` and `/api/v2` goes to `bar-service`). [^11] [^12]

[^11]:
    Why not connect your workloads directly to the hardware load balancer? While that is certainly possible,
...
[^12]:
    In the future, Ingresses might be replaced with the upcoming [Gateway API](https://gateway-api.sigs.k8s.io/),
...
In the Panfactum Stack, we use the Ingress-NGINX Controller ([kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx))
...
<ImageSvg src={ingressSvg} alt="Inbound network diagram" />
...
We provide a submodule for defining Ingress manifests for our Ingress-NGINX controller: [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress).
...
We provide a module for deploying CoreDNS to Panfactum Stack clusters, [kube_core_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns),
...
[^20]:
    Note that EKS also deploys CoreDNS to newly provisioned clusters. However, their implementation is lacking in several
...
In the Panfactum Stack, we provide the [kube_external_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_dns)
module which will provision DNS records in [AWS Route 53.](https://aws.amazon.com/route53/) The specific instructions
for configuring this are covered in [the bootstrapping guide.](/docs/edge/guides/bootstrapping/inbound-networking#deploy-externaldns) [^21]
...
[kube_bastion](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion).
...
<ImageSvg src={bastionSvg} alt="Bastion network diagram" />
...
[^32]:
    `remotehost:80` can be _any_ address accessible from inside the cluster. This upstream server does _not_
...
For more information on SSH and why we chose this architecture over a VPN-based system, see [these concept docs.](/docs/edge/concepts/networking/cryptography#vpns)
...
A _firewall_ is any component on the network path that can drop packets based on preconfigured rules. In most datacenters,
firewall rules are implemented in a centralized system hardware such as routers and L3 switches. This is called a _network
firewall._ In AWS, this is implemented through [Security Groups](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html)
and [Network ACLs.](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html)

Kubernetes adds the concept of _host firewalls_ where additional routing rules are distributed to
...
[^41]:
    Due to [ARP](https://en.wikipedia.org/wiki/Address_Resolution_Protocol), traffic in the same subnet / VLAN
    does not need to be routed through a router or L3 switch and thus cannot have network firewall rules applied.

<MarkdownAlert severity="warning">
  The current version of the Panfactum Stack supports Network Policies but does
  not yet implement them in our modules. As a result, you cannot turn on
  deny-by-default networking in the Panfactum Stack. This is coming in a future
  release.
</MarkdownAlert>
...
TLS and the associated [X.509 certificate infrastructure](/docs/edge/concepts/networking/cryptography#x.509-certificates)
...
Stack via [kube_cert_manager](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager).
...
<MarkdownAlert severity="warning">
  This certificate provisioning process will only work if you have set up DNS in
  accordance with our [bootstrapping
