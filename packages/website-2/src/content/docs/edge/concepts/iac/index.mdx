...
- Plugin-oriented: Terraform provides a standard, stable interface for adding new infrastructure providers, allowing the wider
  ...
  to define _all_ infrastructure using a single toolchain, greatly simplifying infrastructure management.
...
[^2]:
    This was due to their definition and banning of ["competitive offerings"](https://www.hashicorp.com/license-faq#what-is-considered-competitive)
...
- [CloudFormation](https://aws.amazon.com/cloudformation/): An AWS-specific IaC format. While Cloudformation does
  ...
  in CloudFormation that cannot be provided in terraform. This greatly reduces its usefulness unless you are _only_ using AWS APIs which very few organizations do in practice.
...
_Infrastructure-as-Code_ is typically packaged as reusable [modules](https://opentofu.org/docs/language/modules/) that can be parameterized during deployment by providing different
...
for a deployed module is provided via code, we call that _configuration-as-code_ (CaC). [^4]

[^4]:
    Configuration can also extend beyond module inputs. In general, anything that programmatically configures the behavior of an IaC module
