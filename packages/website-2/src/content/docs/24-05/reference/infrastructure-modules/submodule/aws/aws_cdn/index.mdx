...
<ModuleHeader
  name="aws_cdn"
  sourceHref="https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_EDGE__/packages/infrastructure/aws_cdn"
  status="beta"
  type="submodule"
/>
...
[^2]:
    We default to `redirect-to-https` for backwards-compatibility, but it is more secure to use `https-only`. See
...
We provide an input, `redirect_rules`, that allows you to specify redirects that will be applied _before_ requests
...
<MarkdownAlert severity="warning">
  By enabling CORS handling in CloudFront, the CORS headers set by your origin
...
[aws_s3_public_website](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_s3_public_website) module.
...
then client can connect from anywhere _but_ the United States.
