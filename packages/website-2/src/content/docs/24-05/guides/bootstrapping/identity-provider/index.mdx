...
- Single Sign-on: Using a federated auth protocol to facilitate a _single_ means of _authentication_ for your users; does not necessarily
  facilitate _authorization_
...
<MarkdownAlert severity="warning">
  You should only configure ONE root identity provider for your organization.
  Only complete this guide section ONCE for your production (or most secure)
  environment.
</MarkdownAlert>
...
we _strongly_ recommend self-hosting. Your IdP will control _all_ access to your systems, and leaving this control
...
We provide both a module to deploy Authentik ([kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)) and
many modules to connect Authentik to the various systems in the Panfactum stack (e.g., [authentik_aws_sso](/docs/edge/reference/infrastructure-modules/direct/authentik/authentik_aws_sso)).
...
We provide a module to do this: [aws_ses_domain](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ses_domain).
...
3. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/aws_ses_domain/terragrunt.hcl).
...
6. Log into the AWS web console and navigate to the SES service. Under "Get set up," notice that your SES configuration
   ...
   <Image src={SESSandboxImg} alt="SES Sandbox mode" />
...
7. (Optional) This module does not configure your DMARC record as you would typically only have one DMARC record
   ...
   to set this up. You can use the [aws_dns_records](/docs/edge/reference/infrastructure-modules/direct/aws/aws_dns_records) module to
   deploy the record like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/global/aws_dns_records/terragrunt.hcl).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/kube_authentik/terragrunt.hcl).
...
8. Navigate to the `domain` you configured in step 2 in a web browser. You should receive a login prompt like this (may have different branding):
   ...
   <Image src={authentikBootstrapLoginImg} alt="Initial Authentik login page" />
...
10. After successful sign-in, click the "Admin Interface" button in the top-right (if not already in the admin dashboard):
    ...
    <Image src={authentikAdminBtnImg} alt="Authentik admin interface button" />

11. You should reach a dashboard that looks like this:

    <Image src={authentikAdminInterfaceImg} alt="Authentik admin interface" />
...
branding: [authentik_core_resources](/docs/edge/reference/infrastructure-modules/direct/authentik/authentik_core_resources).
...
2. Add a new `terragrunt.hcl` file that looks like [this](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/authentik_core_resources/terragrunt.hcl).
   ...
   3. `superusers_require_webauthn` controls whether members of the `superusers` group require hardware tokens such as [Yubikeys](https://www.yubico.com/) to
      authenticate. We _strongly_ recommend setting this to `true`.

   4. Optionally, you can replace the default Panfactum branding with your own `logo_svg_b64` and
      `favicon_ico_b64` files.

   5. `default_groups_enabled` will provision the [default Panfactum RBAC groups](/docs/edge/reference/rbac) (`superusers` will always be deployed).
...
3.  Reset the new user's password.
    ...
    2.  On the Overview tab, click the "Email recovery link" button in the sidebar:
        ...
        <Image src={emailRecoveryImg} alt="Email recovery link button" />
...
[^3]:
    If you do not receive an email or if clicking on the link results in a "Request has been denied" error
