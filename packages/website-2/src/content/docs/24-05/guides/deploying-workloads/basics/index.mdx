---
title: "Deploying Workloads: The Basics"
---
...
2. [Create a first-party IaC module](/docs/edge/guides/iac/first-party-modules#writing-modules)
   ...
   | :----------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------------------------------------- |
   | [Namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) | [kube_namespace](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_namespace)       | A logical container for Kubernetes resources. Most resources must be assigned a namespace.                              |
   | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)        | [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)     | Orchestrates stateless workloads such as webservers.                                                                    |
   | [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)      | [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set) | Orchestrates for stateful workloads such as databases.                                                                  |
   | [CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/)            | [kube_cron_job](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job)         | Periodically runs one-shot processes on a schedule such as system maintenance scripts.                                  |
   | [DaemonSet](https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/)          | [kube_daemon_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_daemon_set)     | Runs a pod on every tolerated node in the cluster.                                                                      |
   | [Argo Workflow](https://argoproj.github.io/workflows/)                                     | [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflows/wf_spec)                      | An advanced pattern available through our [Workflow Engine Addon](/docs/edge/guides/addons/workflow-engine/installing). |

3. [Deploy the IaC module](/docs/edge/guides/iac/first-party-modules#deploying-modules) by configuring it in the appropriate environment directory and running `terragrunt apply`.
...
- [IaC Module](https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_EDGE__/packages/reference/infrastructure/pf_website)
- [Terragrunt Deployment](https://github.com/Panfactum/stack/tree/__PANFACTUM_VERSION_EDGE__/packages/reference/environments/production/us-east-2/pf_website)
- [CI / CD Integration](https://github.com/Panfactum/stack/blob/__PANFACTUM_VERSION_EDGE__/packages/reference/infrastructure/demo_cicd/website_image_builder.tf)
...
[^2]: In fact, our submodules utilize a shared interface for defining pods: [kube_pod](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pod).
...
[^4]:
    `tail -f /dev/null` is used because this is a noop that prevents the command from terminating. If the container terminated, some
    controllers like a Deployment would continually try to restart it.

<MarkdownAlert severity="info">
  For public images, we strongly recommend that you use the ECR pull-through
  caches for the `image_registry` (setup as a [step in the bootstrapping
  guide](/docs/edge/guides/bootstrapping/kubernetes-cluster#configure-pull-through-cache)).
...
[module docs](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pod) for more information).
...
pod autoscaler. [^5] However, you _can_ set minimums and maximums to the autoscaling ranges via `minimum_memory`, `maximum_memory`,
...
- Every container has a ready-only root filesystem. If you need to write to disk, we recommend using temporary directories (see below)
  or a [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set). However, you
...
- `instance_type_anti_affinity_required`: If true, pods of the same controller will be prevented from running on the same
  ...
  - Spot disruptions: Spot disruptions often impact _all_ spot nodes of a single instance type, so you would
...
See our [high availability guide](/docs/edge/guides/deploying-workloads/high-availability) for more information.
...
[^90]:
    Spot tolerations are added to _all_ pods by default, even those not created by Panfactum submodules. You can add the label `"panfactum.com/spot-enabled" = "false"` to
    the pod to prevent spot tolerations from being injected.

[^91]:
    ARM64 tolerations are added to _all_ pods by default, even those not created by Panfactum submodules. You can add the label `"panfactum.com/arm-enabled" = "false"` to
    the pod to prevent spot tolerations from being injected.
...
<MarkdownAlert severity="info">
  Node classes are **not** mutually exclusive. You can have multiple enabled at
  once to combine their benefits.
</MarkdownAlert>
...
All of our controller submodules such as [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)
...
<MarkdownAlert severity="warning">
  Note that PDBs will NOT protect pods that have not yet reached the `Running`
  state (e.g., the pod is initializing). We have added custom logic that
  prevents most evictions for recently created pods to avoid disrupting pods
  that have not reached a healthy state, but your pods must fully initialize
  within five minutes to avoid potential issues.
</MarkdownAlert>
...
to received requests or closing database connections. During this time the pod will not be able to _receive_
...
That bound depends on _how_ the node was terminated.
...
- **The node was terminated via the host operating system shutting down:** For spot instances, the upper bound
  ...
  and 59.5 minutes for non-critical pods. Additionally, non-critical pods will terminate _before_ critical pods due to the [node shutdown sequencing](https://kubernetes.io/docs/concepts/cluster-administration/node-shutdown/#graceful-node-shutdown).
...
- `common_secrets`: A key-value mapping of secret values. The values will be stored in a Kubernetes
  ...
  reference](/docs/edge/reference/rbac)).
...
[^8]:
    Unless you are using [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set)
...
Pod created by Panfactum submodules will automatically be labeled with our [standard resource tags](/docs/edge/reference/resource-tags).
...
define precedence for pod preemption. Our [kube_priority_classes](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_priority_classes)
...
(e.g., [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)).
...
<MarkdownAlert severity="warning">
  Keep in mind that if you roll your own modules for deploying resources on
  Kubernetes, you will be responsible for the dozens of enhancements that are
  built into our submodules by default. This includes everything from security
  to autoscaling to high availability and much more.
</MarkdownAlert>
