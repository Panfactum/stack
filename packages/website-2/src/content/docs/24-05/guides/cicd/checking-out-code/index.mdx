...
[this guide section](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec#using-the-panfactum-devshell).
...
<MarkdownAlert severity="warning">
  Note that this performs a shallow clone, so if you need to interact with
  commits other than what is supplied by `--checkout`, you will need to
  explicitly `git fetch` them first.
</MarkdownAlert>
