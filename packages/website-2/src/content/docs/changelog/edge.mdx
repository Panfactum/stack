...
_To upgrade your Panfactum stack version, please follow the instructions in the [upgrade guide.](/docs/edge/guides/versioning/upgrading/general)_
...
- [wf_dockerfile_build](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build) now supports
...
- The `kube_rbac` module has been deprecated and will be removed in the next release. Please destroy any deployments of
  it **after upgrading [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks)**.

  Kubernetes access control has now been moved to the [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) module using
...
- The `kube_priority_classes` module has been consolidated with [kube_policies](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_policies)
  in order to remove a superfluous onboarding step. Please destroy any deployments of it **immediately before upgrading
...
- Upgrades Authentik in [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)
...
- Addresses issue in [wf_dockerfile_build](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build)
...
- [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) and [kube_karpenter_node_pools](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter_node_pools) can
...
- This release changes the way that public ingress TLS certificates are provisioned in order to avoid
  ...
  - [kube_cert_issuers](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_issuers)
  - [kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx). **To avoid service disruptions,
...
- Adds `bootstrap_cluster_creator_admin_privileges` input to [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) to provide
...
- This release contains a major version upgrade to Linkerd.
  ...
  **To upgrade with no downtime, you MUST update the modules in the following order:**
  ...
  - [kube_policies](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_policies)
  - [kube_kyverno](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_kyverno)
  - [kube_cilium](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium)
  - [kube_linkerd](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_linkerd)
...
- The NATS backend for [kube_argo_event_bus](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) has been replaced
  with our enhanced NATS module, [kube_nats](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_nats). This provides improved
  ...
  also need to delete any associated `EventSource` or `Sensor` resources _before_ deleting the `EventBus` or the `EventBus`
...
- The `images` input of [kube_node_image_cache](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) has
  been updated to take a list of image configuration options rather than a list of image strings.

  Additionally, `node_image_cached_enabled` has been removed as a top-level input from Panfactum submodules (e.g., [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment))
  as image cache settings can now be configured on a per-container basis.

### Changes

- Added support for the [NATS Jetstream message broker](https://docs.nats.io/nats-concepts/jetstream) via a new submodule, [kube_nats](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_nats).
...
- [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) now launches with `arm64` nodes when `bootstrap_mode_enabled` is `true` as we have resolved the remaining issues
  that have prevented `arm64` from being used during bootstrapping.

- [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) now has [EKS access entries](https://docs.aws.amazon.com/eks/latest/userguide/access-entries.html) enabled.

- [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) now has [ARC Zonal Shift](https://docs.aws.amazon.com/eks/latest/userguide/zone-shift.html) enabled if running nodes in multiple subnets.

- [kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx) now has [ARC Zonal Shift](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/zonal-shift.html) enabled.

- [kube_vault](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault) now schedules pods exclusively on `arm64` nodes
  in order to support the integration of external secret plugins.

### Added

- The [kube_node_image_cache_controller](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_node_image_cache_controller) has
...
- The [kube_descheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_descheduler) will now
...
- Images provided to and/or used by Panfactum submodules (e.g.,
  ...
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster), etc.) are now
  cached by default.

- Additional annotations and labels can now be added to the controllers created via [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment),
  [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set),
  [kube_daemon_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_daemon_set), and
...
- Adds support for dynamically generated labels in [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec)
  via `labels_from_parameters` and `labels_from`.

- [kube_argo_event_source](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_source) now creates a ServiceAccount and output's its name. This can
  be used to assign AWS (or other permissions) to the EventSource pods.

- Adds the ability to configure temporary storage space size in [wf_tf_deploy](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_tf_deploy).

### Fixed

- The [kube_node_image_cache_controller](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_node_image_cache_controller)
  will now deduplicate images that are added to the cache by [kube_node_image_cache](/docs/edge/reference/infrastructure-modules/submodules/kubernetes/kube_node_image_cache).

- We have adjusted the Kyverno settings to improve overall stability of the mutation engine.

- Resolved slow Vault startup times for Vault databases larger than 100MB in [kube_vault](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault).

- BuildKit cache PVCs are now excluded from Velero backups as they consume a lot of storage and are safe to delete.

- Fixed root user access provisioning in [kube_rbac](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_rbac).
...
- Fixed mount permissions in [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec).
...
- `kube_fledged` has been removed in favor of a new node-local image caching mechanism built by Panfactum on top of Kyverno. The new mechanism has the following
  ...
  [kube_node_image_cache](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_node_image_cache) module. Additionally,
  we provide a new input to our submodules such as [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment) called `node_image_cached_enabled` that when enabled
...
- `kube_reflector` has been removed in favor of a new syncing mechanism built by Panfactum on top of Kyverno.
  ...
  - To sync ConfigMaps, use [kube_sync_config_map](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_sync_config_map).
  - To sync Secrets, use [kube_sync_secret](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_sync_secret).
...
- Vertical pod autoscaling now works for both the PostgreSQL clusters and Pgbouncer deployments created by the
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) submodule. The following variables
  have been removed:
...
- We have upgrade the CNPG operator in [kube_cloudnative_pg](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cloudnative_pg) to 1.24 (up from 1.23).
  This adds additional stability improvements during failover events.

  After performing this upgrade, you MUST use the new [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
  submodule as well. Old versions are no longer compatible.

- We have upgraded the default PostgreSQL version in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) to 16.4 (up
...
- Adds a new submodule, [kube_daemon_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_daemon_set), for creating
...
<MarkdownAlert severity="warning">
  A bug has been discovered in this release that can cause a complete cluster
  crash due to the introduction of the new Kyverno policy engine. Please skip
  this release and use `edge.24-11-13` instead.
</MarkdownAlert>
...
- Added a new module, [aws_s3_public_website](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_s3_public_website), to enable users to serve files directly from an S3 bucket via CloudFront.
- [aws_cdn](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_cdn) can now handle CORS headers on behalf of the origin servers
- [aws_cdn](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_cdn) now uses 10x more efficient [CloudFront functions](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/cloudfront-functions.html)
  for request / response mutations.

### Fixed

- Deploying modules that use Helm charts hosted in ECR (e.g., [kube_karpenter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter))
...
- To add further protection against Karpenter provisioning extremely large nodes, we have
  two variables for [kube_karpenter_node_pools](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter_node_pools), `max_node_memory_mb` and `max_node_cpu`,
...
- Removes memory limits on the Cilium node agent in [kube_cilium](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium) as
...
- Upgrades [kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_nginx_ingress) so that it can run on nodes with a large number of CPU cores.
...
- [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) no longer allows `rewrite_rules`
...
- [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) no longer allows `domains`
  ...
  for _all_ domains in your system.

### Added

- A new module, [kube_aws_cdn](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_aws_cdn), has been
  ...
  as well as reduced server costs. [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress)
  has been updated to support CDN settings.

  Additionally, a non-Kubernetes CDN module, [aws_cdn](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_cdn),
  has also been created.

- A new module, [aws_dns_zones](/docs/edge/reference/infrastructure-modules/direct/aws/aws_dns_zones), has been
  created that allows you to create Route53 zones that have a non-AWS registrar.

- Adds the `acl_aws_logs_delivery_enabled` input to [aws_s3_private_bucket](/docs/main/reference/infrastructure-modules/submodule/aws/aws_s3_private_bucket)
  which makes it easier to use the bucket for AWS log delivery purposes.

- Adds support for Cloudflare in [kube_external_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_external_dns) and [kube_cert_issuers](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_issuers)

### Changed

- `tls_1_2_enabled` now defaults to `true` in [kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx) in order
  to support CDNs like CloudFront which do not yet support TLSv1.3.

### Fixed

- The internal logic of [aws_dns_records](/docs/edge/reference/infrastructure-modules/direct/aws/aws_dns_records) has been
  updated so that each record is managed independently of the others. This fixes an issue where adding or removing
  records would cause _all_ records to be created. However, this update will cause all records to be recreated
  one last time.
...
- `pf-wf-git-checkout` no longer automatically appends a `.git` to the end of given repo URLs as this is incompatible
  ...
  to [wf_tf_deploy](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_tf_deploy) and
  [wf_dockerfile_build](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build) should
...
- Fixes the ami instance type mismatch when `bootstrap_mode_enabled` is enabled in the [aws_eks](/docs/main/reference/infrastructure-modules/direct/aws/aws_eks) module.
- Fixes issues that prevented bootstrapping scripts from running with new `pf-tf-init` logic.
- Adjusts the defaults for [kube_reflector](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_reflector) so that installation does not fail in the bootstrapping guide.
...
- Fixes an issue where voluntary disruption windows created by the [kube_disruption_window_controller](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_disruption_window_controller)
...
- The [kube_secrets_csi](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_secrets_csi) has been deprecated
...
- [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)'s and [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)'s `superuser_username`
...
- [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) and
  [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel) now support
...
- The `ports` input on [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)
  and [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set) has been moved
  to a container-level field rather than a top-level field to better align with the Kubernetes API.

### Added

- Adds a new submodule, [kube_service](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_service), for
  ...
  Additionally, integrates `kube_service` into [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment)
  and [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set) for automatic Service creation.

- Adds `extra_storage_classes` input to the [kube_aws_ebs_csi](/docs/main/reference/infrastructure-modules/direct/kubernetes/kube_aws_ebs_csi)
  module.

### Fixed

- Addressed issue in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
...
- Before applying this release, the `buildkit-amd64` and `buildkit-arm64` StatefulSets in the `buildkit` namespace
  must be removed (if [kube_buildkit](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_buildkit) is deployed).
...
- In preparation for our upcoming release, we cleaned up a handful of naming conventions which impact the inputs and outputs of several modules:
  ...
  - In [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment), [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set), [kube_cron_job](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job), [kube_pod](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pod), [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec), and [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility):
  ...
  - In [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) and [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel):
  ...
  - In [kube_constants](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_constants), a few outputs have been updated:
...
- We have removed a handful of options from [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment), [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set), [kube_cron_job](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job), [kube_pod](/docs/main/reference/infrastructure-modules/submodule/kubernetes/kube_pod), [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec), and [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility) that we
...
- When we introduced the concept of the `enhanced_ha_enabled` input, it was designed as a cost-saving switch for direct modules
  ...
  As a result, we have replaced the `enhanced_ha_enabled` input in all _submodules_ with more granular tuning knobs that have
  clearer behavior. This impacts the following submodules: [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster),
  [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel),
  [kube_vault_proxy](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_vault_proxy),
  [kube_argo_event_bus](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_bus),
  and [kube_argo_event_source](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_source).
...
- Previously we were conservative about enabling certain features by default in some of our submodules in order to ensure our modules
  ...
  - The following flags are now enabled by default in [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment), [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set), [kube_cron_job](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job), [kube_pod](/docs/main/reference/infrastructure-modules/submodule/kubernetes/kube_pod), [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_spec), [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster), [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel), and [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility):
    - `spot_nodes_enabled`
    - `arm_nodes_enabled`
    - `vpa_enabled`
    - `panfactum_scheduler_enabled`
  - The following flags are now enabled by default in [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment):
    - `az_spread_preferred`
  - The following flags are now enabled by default in [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set):
    - `az_spread_required`
...
- Remove the `RemoveDuplicates` strategy in [kube_descheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_descheduler)
...
- We removed the ability to disable S3 backups in
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster). The backups
...
- Added native support for restoring from database backups to the [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
  submodule.

- Added automatic creation of an immediate base backup to the
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) to ensure that
...
- Mitigated a rare scenario where disruption in the middle of a database failover would result in
  ...
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) submodule.
...
- Addressed a couple of issues with the [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)
  module:
  - [authentik_core_resources](/docs/edge/reference/infrastructure-modules/direct/authentik/authentik_core_resources) will no longer
...
- Correctly sets PgBouncer permissions on new PostgreSQL cluster creation in
  [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster).

## edge.24-08-22

### Breaking Changes

- The default behavior of [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)
  ...
  only benefit would be if _all_ Redis nodes simultaneously failed with a non-graceful shutdown (an incredibly unlikely scenario).
...
- `token_lifetime_seconds` has been changed to `token_lifetime_hours` in [vault_auth_oidc](/docs/edge/reference/infrastructure-modules/direct/vault/vault_auth_oidc) to avoid a perpetual diff issue
  present in the Vault provider.

- Removed the daily backups from [kube_velero](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_velero)
  as they were undocumented and had no realistic use case.

### Added

- Adds a new submodule, [kube_disruption_window_controller](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_disruption_window_controller),
  which can be used to specify time-based disruption windows for disruption-sensitive workloads (e.g., databases). Disruption
  window capabilities have also been added to [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
  and [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel).
...
- Addressed issue where `pg_smart_shutdown_timeout` cannot be set to 0 in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
  without having CNPG reset it to 180.

- Fixed an issue in [kube_velero](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_velero) where stale
  EBS snapshots were not being deleted.

- Added stricter disruption prevention to the Velero server in [kube_velero](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_velero)
...
- `pg_shutdown_timeout` has been renamed to `pg_smart_shutdown_timeout` to better indicate its purpose
  in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster). Additionally,
...
- Adds the concept of passthrough parameters to [wf_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_spec).

- Makes `tf_apply_dir` a Workflow parameter in [wf_tf_deploy](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_tf_deploy)
...
- Adds the ability to use `templateRef` to [compose Workflows](https://argo-workflows.readthedocs.io/en/latest/workflow-templates/#referencing-other-workflowtemplates)
  in [wf_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build).

### Fixed

- Fixed the working directory in
  [wf_tf_deploy](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_tf_deploy)
  and [wf_dockerfile_build](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build)
  to be inside the cloned repository.

- Addressed OOM errors when using resource templates with [wf_spec](/docs/main/reference/infrastructure-modules/submodule/workflow/wf_spec).

## edge.24-08-13

### Breaking Changes

- `pg_storage_increase_percent` has been changed to `pg_storage_increase_gb` in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster).
...
- `pg_storage_gb` has been changed to `pg_initial_storage_gb` in [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster).
  This better indicates that this value is _only used during the initial database provisioning_ and has no effect thereafter.

- `node_vpc_id`, `node_subnets`, and `node_security_group_id` have been moved from [kube_karpenter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter)
  to [kube_karpenter_node_pools](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter_node_pools) in order to simplify the logic
...
- Adds new enhancements to the [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) module:
...
- Added deterministic ordering to additional resources in [authentik_core_resources](/docs/main/reference/infrastructure-modules/direct/authentik/authentik_core_resources).
...
- `amd64` nodes are now used when `bootstrapping_enabled` is `true` in [aws_eks](/docs/main/reference/infrastructure-modules/direct/aws/aws_eks) in order
...
- Updates the Panfactum image version in [kube_constants](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_constants) to
...
- To support the new addons, we are upgrading the following infrastructure modules to Beta status:
  ...
  - [kube_argo](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_argo): For deploying the Argo controllers
  - [kube_argo_event_bus](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_bus): For deploying an Argo EventBus
  - [kube_argo_event_source](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_source): For deploying an Argo EventSource
  - [kube_argo_sensor](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_sensor): For deploying an Argo Sensor
  - [wf_spec](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/wf_spec): For creating an Argo Workflow specification
  - [wf_tf_deploy](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/wf_tf_deploy): For creating an Argo WorkflowTemplate that deploys IaC modules
  - [wf_dockerfile_build](/docs/edge/reference/infrastructure-modules/submodule/workflow/wf_dockerfile_build): For creating an Argo WorkflowTemplate that builds container images from Dockerfiles
...
- We have made a small, breaking refactor of [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks)
...
- By default, PVCs created by controllers such as StatefulSets can not be updated through their controller as their template (`volumeClaimTemplates`)
  ...
  built a workaround to this ([kube_pvc_annotator](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pvc_annotator))
  ...
  2. The following StatefulSets need to be deleted **in this order** AND **with `kubectl delete --cascade=orphan`** AND **immediately restored**
     ...
     - Any Redis clusters StatefulSets you have provisioned with [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)
...
- Adds [kube_fledged](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_fledged) to the core stack. The
...
- Adds the [kube_pvc_annotator](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pvc_annotator) submodule
...
- Adds `persistence_backups_enabled` (default: `true`) to [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel)
...
- The input format to [aws_ecr_repos](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ecr_repos) has been
...
- The following resources will no longer be tagged with the Panfactum version and commit hash as updates cause unnecessary
  ...
  - EC2 instances in EKS node groups generated by [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks)
  - EC2 instances serving as NAT hosts in [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc)
  - KMS replica keys in [aws_kms_encrypt_key](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_kms_encrypt_key)
  - Pods created in [kube_bastion](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion)

### Added

- [kube_buildkit](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_buildkit) has graduated to beta
...
- [aws_ecr_repos](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ecr_repos) now supports custom image
  expirations rules and both pull and push permissions.

- [aws_ecr_public_repos](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ecr_public_repos) has been added
  to support created public ECR repositories.

- Adds ARM support in [kube_bastion](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion) and
  [kube_pvc_autoresizer](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_pvc_resizer). All core cluster
...
- (Alpha) Adds [kube_argo_sensor](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_sensor) and
  [kube_argo_event_source](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_argo_event_source) submodules
  for deploying these core components of the [Argo Events](https://argoproj.github.io/argo-events/) system.

- (Alpha) Adds the [kube_workflow_spec](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workflow_spec)
  submodule to help in defining production-ready [Argo Workflows.](https://argo-workflows.readthedocs.io/en/latest/)

### Fixed

- [kube_aws_ebs_csi](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_aws_ebs_csi) has been adjusted
  to ensure that PVCs are detached from nodes during node shutdown, preventing unnecessary delays in moving PVCs between
  nodes.

- [kube_core_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns) no longer accidentally includes the Vault provider.

- [kube_ingress_nginx](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_ingress_nginx) will no longer
...
- [kube_karpenter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter) has upgraded the Karpenter
...
- The `persistence_enabled` option was removed from [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel).
...
- [aws_ecr_pull_through_cache_addresses](/docs/edge/reference/infrastructure-modules/submodule/aws/aws_ecr_pull_through_cache_addresses)
...
- [kube_deployment](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_deployment),
  [kube_stateful_set](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_stateful_set),
  [kube_cron_job](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_cron_job),
  and [kube_pod](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pod) have graduated to Beta status. They
  are now safe to use.
...
- Adds phone number validation in [aws_account](/docs/edge/reference/infrastructure-modules/direct/aws/aws_account).

- Adds `cors_enabled` (default: `false`) input variable to [kube_vault](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault)
...
- Addresses an issue in [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)
  that prevented the SSO login pop-up from working.

- Implements custom CORS handling logic in [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress)
...
- Adds [kube_scheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_scheduler), an alternative Kubernetes
...
- Adds `panfactum_scheduler_enabled` (default: `false`) input to most infrastructure modules. When enabled, will
  use the scheduler provided by [kube_scheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_scheduler)
  instead of the less-efficient EKS scheduler.

- If `panfactum_scheduler_enabled` is `true`, the [kube_descheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_descheduler)
  will automatically remove pods from low utilization nodes to allow the [kube_scheduler](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_scheduler)
  to bin-pack them on other nodes.

### Fixed

- Addresses a bug in the previous release that left [kube_karpenter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter)
  not deployable.
...
- Updates [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) with many new variables
  for configuring PgBouncer. New variables are prefixed with `pgbouncer_`.

- Adds support for `path_prefix` to [kube_vault_proxy](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_vault_proxy) ([@mschnee](https://github.com/mschnee))
...
- Adds a [Spot Data Feed](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/spot-data-feeds.html)
  to the [aws_account](/docs/edge/reference/infrastructure-modules/direct/aws/aws_account) module.

- Adds the [kube_open_cost](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_open_cost) module
  for calculating the cost of workloads running on Kubernetes.

### Fixed

- Addressed issue in [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc) where NAT nodes wouldn't restart
  if NAT setup failed with an exit code other than `1`.

- Increased the memory floor of the Authentik server in [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik)
  to avoid OOM issues.

- Updates [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik) to allow showing
  [Gravatar](https://gravatar.com/) profile images.

- Updates [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik) to provide the necessary Permissions-Policy headers to allow
  use of WebAuthn devices.

- Correctly applies pod labels in [kube_aws_lb_controller](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_aws_lb_controller).

- Removes node preferences defaults from [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility)
...
- Fixes incorrect SCIM property mapping in [authentik_aws_sso](/docs/edge/reference/infrastructure-modules/direct/authentik/authentik_aws_sso).

- Aligns pod labels, affinities, topologySpreadConstraints, and tolerations in
  [kube_linkerd](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_linkerd) to conventions used in all
  other modules.
...
- Updates [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc) to support new command `pf-vpc-network-test` that
...
- [kube_trust_manager](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_trust_manager) has been deprecated as it's functionality was redundant with
  [kube_reflector](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_reflector). We are keeping the module
...
- [aws_registered_domains](/docs/edge/reference/infrastructure-modules/direct/aws/aws_registered_domains) can now set the contact type
  for each contact.

- Allow users to reference availability zones by single character (e.g., `a`) in addition to the full name (e.g., `us-east-2a`) in the
  [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc) module.
...
- Addressed issue in [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc) that caused a temporary, harmless error to crash
...
- The reloader deployment must be deleted before the next apply of [kube_reloader](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_reloader). No
  inputs have changed.

- The alpha module `kube_labels` has been removed in favor of the labels provided by [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility).

- VPC flow logs in [aws_vpc](/docs/edge/reference/infrastructure-modules/direct/aws/aws_vpc) are now disabled by default as they
...
- Added [kube_workload_utility](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_workload_utility) to make it easier to create uniform, production-hardened
...
- Added [kube_constants](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_constants) that export static configuration values that can be useful
  when creating resources that run on clusters in the Panfactum stack.

- [kube_cert_manager](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cert_manager) will now automatically delete Certificate secrets if the Certificate is deleted.

- [aws_ses_domain](/docs/edge/reference/infrastructure-modules/direct/aws/aws_ses_domain) now takes an optional input `smtp_allowed_cidrs`
...
- [kube_authentik](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_authentik) now respects `log_level` input.
...
- Addressed an issue that caused updates to the local devenv to take
  ...
  take about 45 seconds at minimum. Note that this _only_ impacts rebuilds and _not_ normal direnv load times which should
  still be instant.
...
- Added extra validation for the terragrunt variable `extra_tags`. Invalid characters will now be replaced with `.`
  for _both_ keys _and_ values for _both_ Kubernetes labels and AWS tags.

- Fixed some core components that were using _all_ Kubernetes labels for `labelSelector` matching rules which prevented
...
- The default for `vault_storage_size_gb` in [kube_vault](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault)
...
- (Alpha) Added the [Loki](https://grafana.com/docs/loki/latest/)
  logging backend via [kube_logging](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_logging)
  and the [Alloy](https://grafana.com/docs/alloy/latest/) log collector via [kube_alloy](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_alloy).

- The [PVC Autoresizer](https://github.com/topolvm/pvc-autoresizer) has been added via the
  [kube_pvc_autoresizer](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_pvc_autoresizer) module in order
...
- Added validation for phone number format in
  [aws_registered_domains](/docs/edge/reference/infrastructure-modules/direct/aws/aws_registered_domains). ([@wesbragagt](https://github.com/wesbragagt))
...
- We have removed the EKS CoreDNS addon and replaced it with the [kube_core_dns](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_core_dns)
  module in order to provide better guarantees about the behavior of DNS in the Panfactum stack. In order to migrate:

  1. Add the `dns_service_ip` input to [aws_eks](/docs/edge/reference/infrastructure-modules/direct/aws/aws_eks) deployments
  ...
  4. Add the `cluster_dns_service_ip` input to your [kube_karpenter_node_pools](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter_node_pools)
...
- We have stabilized the label selectors in [kube_pod](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pod) but
  this requires one final label update for already-deployed Deployments. This will cause re-applies
  of [kube_bastion](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_bastion) to fail (and any first-party modules that rely on [kube_deployment](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_deployment)).
  To resolve, you must first manually delete the `bastion/bastion` deployment (and all other deployments created by [kube_deployment](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_deployment)).

- [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster) has two
  ...
  to have idle resources. This is a _breaking_ change as `pgbouncer_read_only_enabled` is set to `false` by default.

### Added

- (Alpha) We've added a monitoring stack [kube_monitoring](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_monitoring)
...
- (Alpha) [kube_cilium](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium) now has a new debugging
...
- (Alpha) [kube_linkerd](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_cilium) now deploys [Linkerd Viz](https://linkerd.io/2.15/features/dashboard/)
...
- (Alpha) We've added the [Argo Workflow](https://argoproj.github.io/workflows/) engine to the stack
  via the [kube_argo](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_argo) module. This will
...
- A new module, [kube_vault_proxy](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_vault_proxy), that
...
- [kube_redis_sentinel](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_redis_sentinel) has a new flag,
...
- [kube_ingress](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_ingress) now takes an `extra_configuration_snippet`
...
- Added the standard [Restricted Reader](/docs/edge/reference/rbac) role to Vault instances (`rbac-restricted-reader`)
  and updated [vault_auth_oidc](/docs/edge/reference/infrastructure-modules/direct/vault/vault_auth_oidc) to take `restricted_reader_groups`. Since cluster resources authenticate with SSO
...
- [kube_karpenter](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_karpenter) now only deploys
...
- [kube_vpa](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vpa) has a new `history_length_hours` (default `24`)
...
- [kube_vault](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_vault) now takes `vault_domain`
...
- New [kube_reflector](/docs/edge/reference/infrastructure-modules/direct/kubernetes/kube_reflector) module for deploying
...
- `pg_shutdown_timeout` variable to [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
  to control how long the postgres instances will wait for active connections to close before shutting down.

### Fixed

- Fixed an issue where simultaneous, graceful shutdown of _all_ postgres nodes in a [kube_pg_cluster](/docs/edge/reference/infrastructure-modules/submodule/kubernetes/kube_pg_cluster)
