...
import scheduleConstraintSvg from './schedule-constraint.svg?raw'

import ConsumptionEfficiencyChart from './ConsumptionEfficiencyChart'
...
A key benefit of autoscaling is maximizing _resource utilization_. Importantly, there are two core
...
We call the first term _consumption_ efficiency, the second term _provisioning_ efficiency, and the result _net_ efficiency / overall utilization.
...
[^2]:
    Requested resources are the resources the workload requested be made available _before_ it started running
...
[^1]:
    How far back the VPA looks for percentile calculations is configurable, and in the Panfactum stack we have it prioritize data
...
<ConsumptionEfficiencyChart client:only="react" />
...
<ImageSvg src={scheduleConstraintSvg} alt="Impact of scheduling constraints" />
...
- Using "preferred" scheduling constraints such as `ScheduleAnyway` (topology spread) or `preferredDuringSchedulingIgnoredDuringExecution`
  ...
  This can prevent Karpenter from triggering node removal even if pods _could_ be scheduled on other nodes. Karpenter makes
...
- All autoscalers respect [PodDisruptionBudgets (PDBs)](https://kubernetes.io/docs/concepts/workloads/pods/disruptions/) and will
  not resize / evict pods if this would violate a PDB. As a result, ensure you design your workloads so that pods _can_ be disrupted
