...
export function ListBlock({ icon, title, content }: ListBlockProps) {
  return (
    <div className="flex flex-col items-center gap-xl text-center">
      ...
      <h3 className="text-primary_on-brand text-xl font-semibold">{title}</h3>
...
export function FeatureListBlock({ icon, title, content }: ListBlockProps) {
  return (
    <div className="flex flex-col items-center gap-xl text-center max-w-[384px]">
      <div className="w-[48px] h-[48px] flex items-center justify-center border border-primary rounded-md p-[14px] shadow bg-primary">
        {getIcon(icon)}
      </div>

      <h3 className="text-primary text-lg md:text-xl font-semibold">{title}</h3>
