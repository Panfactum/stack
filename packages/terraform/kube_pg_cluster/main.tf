...
module "kube_labels_pooler_rw" {
  ...
  region           = var.region
  is_local         = var.is_local
  extra_tags       = merge(var.extra_tags, local.pooler_rw_match_labels)
}

module "kube_labels_pooler_r" {
  source = "../kube_labels"
...
module "constants_pooler_r" {
  source = "../constants"

  matching_labels = local.pooler_r_match_labels
...
resource "vault_database_secret_backend_role" "writer" {
  backend = "db"
  name    = "writer-${var.pg_cluster_namespace}-${local.cluster_name}"
  db_name = vault_database_secret_backend_connection.postgres.name
  creation_statements = [
    // We have to re-run the generic grant commands
    // on every login to make sure we have picked up new objects
...
resource "vault_database_secret_backend_connection" "postgres" {
  ...
  allowed_roles = [
    ...
    "writer-${var.pg_cluster_namespace}-${local.cluster_name}",
    ...
    spec = {
      ...
      template = {
        ...
        spec = {
          ...
          priorityClassName         = module.constants.database_priority_class_name
          topologySpreadConstraints = module.constants_pooler_rw.topology_spread_zone_preferred
          tolerations               = module.constants_pooler_rw.burstable_node_toleration_helm // TODO: Make conditional
          affinity = merge(
            module.constants_pooler_rw.pod_anti_affinity_helm,
            {
              podAffinity = {
...
resource "kubernetes_manifest" "connection_pooler_r" {
  manifest = {
    apiVersion = "postgresql.cnpg.io/v1"
    kind       = "Pooler"
    metadata = {
    ...
    spec = {
      ...
      template = {
        ...
        spec = {
          ...
          priorityClassName         = module.constants.database_priority_class_name
          topologySpreadConstraints = module.constants_pooler_r.topology_spread_zone_preferred
          tolerations               = module.constants_pooler_r.burstable_node_toleration_helm // TODO: Make conditional
          affinity = merge(
            module.constants_pooler_r.pod_anti_affinity_helm,
            {
              podAffinity = {
                // Try to schedule poolers on the same nodes as db instances to reduce network latency
                preferredDuringSchedulingIgnoredDuringExecution = [
...
resource "kubernetes_manifest" "pdb_pooler_rw" {
  manifest = {
    ...
    spec = {
      selector = {
        matchLabels = local.pooler_rw_match_labels
      }
      maxUnavailable = 1
    }
  }
...
resource "kubernetes_manifest" "pdb_pooler_r" {
  manifest = {
    apiVersion = "policy/v1"
    kind       = "PodDisruptionBudget"
    metadata = {
    ...
    spec = {
      selector = {
        matchLabels = local.pooler_r_match_labels
      }
      maxUnavailable = 1
    }
  }
