locals {
  bastion_image_builder_name = "bastion-builder"
}
...
resource "kubernetes_role_binding" "bastion_builder" {
  metadata {
    generate_name      = local.nix_image_builder_name
    namespace = "buildkit"
    labels    = module.bastion_image_builder_workflow.labels
  }
  subject {
    kind      = "ServiceAccount"
    name      = module.bastion_image_builder_workflow.service_account_name
...
data "aws_iam_policy_document" "bastion_builder_ecr" {
  statement {
    sid = "PrivateECR"
    effect = "Allow"
    actions = [
    ...
    resources = [
      "arn:aws:ecr:us-east-2:891377197483:repository/bastion"
    ]
  }
  statement {
    sid = "PrivateECRAuth"
  ...
  statement {
    ...
    resources = [
      "arn:aws:ecr-public::891377197483:repository/bastion"
    ]
  }
  statement {
    sid = "PublicECRAuth"
...
resource "kubernetes_config_map" "bastion_image_builder_scripts" {
  metadata {
    name = "${local.bastion_image_builder_name}-scripts"
    labels = module.bastion_image_builder_workflow.labels
    namespace = local.namespace
  }
  data = {
    "build.sh" = file("${path.module}/bastion_image_builder/build.sh")
    "clone.sh" = file("${path.module}/bastion_image_builder/clone.sh")
    "merge-manifests.sh" = file("${path.module}/bastion_image_builder/merge-manifests.sh")
    "copy-to-public.sh" = file("${path.module}/bastion_image_builder/copy-to-public.sh")
  }
}

# This configuration is required by skopeo
resource "kubernetes_config_map" "bastion_image_builder_containers" {
  metadata {
    name = "${local.bastion_image_builder_name}-containers"
    labels = module.bastion_image_builder_workflow.labels
    namespace = local.namespace
  }
  data = {
    "policy.json" = file("${path.module}/bastion_image_builder/policy.json")
  }
}
...
module "bastion_image_builder_workflow" {
  ...
  extra_aws_permissions = data.aws_iam_policy_document.bastion_builder_ecr.json
  default_resources = {
    requests = {
      memory = "25Mi"
      cpu = "25m"
  ...
  templates = [
    {
      name = "build-images"
     # affinity = module.bastion_image_builder_workflow.affinity TODO
...
resource "kubectl_manifest" "bastion_workflow_template" {
  yaml_body = yamlencode({
    ...
    spec = module.bastion_image_builder_workflow.workflow_spec
  })
